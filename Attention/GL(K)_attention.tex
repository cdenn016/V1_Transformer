...
\end{equation}
%
which is a Gated Linear Unit: a linear function of the input gated by a Boltzmann distribution over KL energies computed from the same input. This GLU structure is not an architectural choice but rather is the form that variational inference necessarily takes when the sufficient statistics are Gaussian. The neural network is the computational substrate however the information geometric dynamics are primary and wholly independent.
