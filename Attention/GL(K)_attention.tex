...
\begin{table}[h]
\centering
\caption{Summary of notation. Symbols are grouped by category: agents and beliefs, gauge geometry, variational quantities, and Transformer correspondences.}
\label{tab:notation}
\small
...
$q_i = \mathcal{N}(\mu_i, \Sigma_i)$ & $\mathcal{P}(\mathbb{R}^K)$ & Belief distribution of agent $i$ (context-dependent) \\
$p_i = \mathcal{N}(\mu_p^i, \Sigma_p^i)$ & $\mathcal{P}(\mathbb{R}^K)$ & Prior distribution (from token embedding) \\
$s_i, r_i$ & $\mathcal{P}(\mathbb{R}^K)$ & Model-fiber belief and prior (generative model) \\
$K$ & $\mathbb{Z}^+$ & Embedding (fiber) dimension \\
...
$\phi_i \in \mathfrak{g}$ & $\mathbb{R}^{K \times K}$ & Gauge frame parameters (Lie algebra element) for agent $i$ \\
$\Omega_{ij} = \exp(\phi_i)\exp(-\phi_j)$ & $G$ & Parallel transport operator from frame $j$ to frame $i$ \\
...
$\beta_{ij}$ & $[0,1]$, $\sum_j \beta_{ij} = 1$ & Attention weight (source-selection posterior) \\
$\gamma_{ij}$ & $[0,1]$ & Model-fiber coupling weight \\
$\alpha$ & $\mathbb{R}^+$ & Prior precision (self-consistency weight) \\
