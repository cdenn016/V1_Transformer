...
Standard transformers utilize attention sub-layers with feedforward networks (FFNs). These are typically two-layer MLPs with a non-linear activation mechanism such as GeLU \citep{hendrycks2016gaussian}. Within the gauge-theoretic framework, the FFN corresponds to the iterative VFE dynamics that adjust beliefs between attention computations. We now show that the nonlinear activation function is not an architectural choice but a consequence of the variational principle: the VFE gradient \emph{derives} a Gated Linear Unit (GLU) \citep{dauphin2017language}.

\paragraph{Message passing as a gated linear unit.}

Consider the message-passing contribution of neighbor $j$ to the mean update (Eq.~\ref{eq:mu_gradient_general}):
%
\begin{equation}
...
\end{equation}
%
The message $e_{ij} \equiv \mu_i - \Omega_{ij}\mu_j$ is \emph{linear} in the beliefs. The attention gate $\beta_{ij} = \operatorname{softmax}_j\!\bigl(-D_{\mathrm{KL}}(q_i \| \Omega_{ij}q_j)/\tau\bigr)$ is a \emph{Boltzmann weight}: each neighbor's contribution is weighted by $\exp(-E_{ij}/\tau)/Z_i$, where the ``energy'' $E_{ij} = D_{\mathrm{KL}}(q_i \| \Omega_{ij}q_j)$ depends on the message magnitude through the Gaussian KL:
%
\begin{equation}
...
\end{equation}
%
Each message contribution therefore has the structure:
%
\begin{equation}
...
\end{equation}
%
which is a \emph{Gated Linear Unit}: a linear function of the input gated by a Boltzmann distribution over KL energies computed from the same input. The gate is not assumed --- it is the thermodynamic weight that the variational principle assigns to each neighbor. This is derived, not designed.

\paragraph{Binary case: Boltzmann reduces to sigmoid.}
For $N = 2$ neighbors, the Boltzmann distribution over two states reduces to the logistic sigmoid $\sigma$, since $\exp(-E_1/\tau)/(\exp(-E_1/\tau) + \exp(-E_2/\tau)) = \sigma((E_2 - E_1)/\tau)$:
%
\begin{equation}
...
\end{equation}
%
This is the SiLU/Swish \citep{ramachandran2018searching} structure $f(x) = x \cdot \sigma(g(x))$, with the gate argument determined by the relative KL energies.  The sigmoid is thus not the general case but the $N = 2$ limit of the Boltzmann gate. For general $N$, each message is gated by its Boltzmann weight --- the sigmoid is recovered only when two neighbors compete.

\paragraph{Boltzmann gate, Gaussian family, and connection to GELU.}
The Boltzmann gate has a specific functional form determined by the energy landscape. In the isotropic limit $\Sigma_j = \sigma^2 I$, the unnormalized gate for neighbor $j$ is $\exp(-E_{ij}/\tau) = \exp(-\|e_{ij}\|^2 / 2\sigma^2\tau)$ --- a Gaussian function of the prediction error. The Boltzmann distribution over KL energies is therefore a \emph{Gaussian mixture gate}: each neighbor's weight is a Gaussian in the prediction-error space, normalized by the partition function $Z_i = \sum_k \exp(-\|e_{ik}\|^2/2\sigma^2\tau)$. Compare this with the standard activation functions, all of which are Gated Linear Units $f(x) = x \cdot g(x)$:
%
\begin{align}
\text{SiLU}(x) &= x \cdot \sigma(x) && \text{logistic sigmoid gate (binary Boltzmann)}, \notag \\
\text{GELU}(x) &= x \cdot \Phi(x) && \text{Gaussian CDF gate}, \label{eq:activation_comparison} \\
\text{VFE}(e) &= e \cdot \exp(-\|e\|^2/\tau')/Z && \text{Boltzmann gate (Gaussian energy)}. \notag
\end{align}
%
All three belong to the \emph{Gaussian family} of gated linear units. The logistic sigmoid $\sigma(x) = 1/(1+e^{-x})$ is the two-state Boltzmann distribution. The Gaussian CDF $\Phi(x) = \frac{1}{2}\operatorname{erfc}(-x/\sqrt{2})$ is the integral of the Gaussian PDF $\propto \exp(-x^2/2)$ that appears as the VFE gate's unnormalized weight. These are not merely analogous --- they are members of the same exponential family, related by the number of competing states ($N = 2$ for sigmoid, $N > 2$ for softmax) and the functional form of the energy ($E \propto x$ for SiLU, $E \propto x^2$ for VFE).

The VFE derives from first principles that the activation should be a Boltzmann-gated linear unit with Gaussian energy. The specific gate functions used in practice (sigmoid for SiLU, Gaussian CDF for GELU) are special cases or close relatives. ReLU is the zero-temperature ($\tau \to 0$) limit: the Boltzmann distribution concentrates on the lowest-energy state, and the gate becomes a step function $\Theta$, giving $x \cdot \Theta(x) = \max(0, x)$.
