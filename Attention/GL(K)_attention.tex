...
This parallels the mean gradient: $\Sigma_{p,i}^{-1}$ is the prior precision (analogous to weight decay for $\mu$), $\sum_j \beta_{ij}(\Omega_{ij}\Sigma_j\Omega_{ij}^\top)^{-1}$ is attention-weighted precision aggregation that pulls agent $i$'s precision toward its gauge-transported neighbors, and the final term $\\$ $\sum_j (\partial \beta_{ij}/\partial \Sigma_i)\, D_{\mathrm{KL}}(q_i \,\|\, \Omega_{ij} q_j)$ is gradient flow through the attention weights (the covariance analogue of the non-linear activation term in the $\mu$ gradient).
