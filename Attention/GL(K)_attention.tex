...
\begin{figure*}[t]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{attention/figs/attention_step_100000_head0k=90.png}
    \caption{Head~0: broad contextual attention with diffuse coverage and strong early-position weighting.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{attention/figs/attention_step_100000_head5k=90.png}
    \caption{Head~5: sharper attention with prominent vertical stripes at specific high-frequency token positions.}
  \end{subfigure}
  \caption{Learned per-head KL-divergence attention patterns ($\log_{10}\beta_{ij}$) for the $\mathrm{GL}(10)$ gauge VFE model ($K=90$) at convergence (step 100{,}000) on a representative WikiText-103 sequence. The model develops selective, non-uniform, head-specialized attention through gauge transport and KL-divergence dynamics alone, without any learned attention projections ($W_Q$, $W_K$, $W_V$).}
...
\hline
Gauge VFE ($\mathrm{GL}(10)$, $K\!=\!90$) & 1 & 58.8M & 91.5 & 90.6 \\
