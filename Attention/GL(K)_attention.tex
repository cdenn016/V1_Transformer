...
In the gauge-theoretic framework, this has a straightforward interpretation. The full gauge transport between agents $i$ and $j$ is $\Omega_{ij} = \exp(\phi_i)\exp(-\phi_j) \in \mathrm{GL}(K)$. RoPE corresponds to a specific factorization of this transport into a learned, position-independent component and a fixed, position-dependent component:
...
Standard transformers share this property. The learned projections $W_Q, W_K$ define an effective transport $\Omega = W_Q W_K^\top$ that is position-independent, i.e. a constant, and therefore flat, gauge transformation. Dot-product attention operates in this flat bundle limit.
