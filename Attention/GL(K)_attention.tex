...
The expanded baselines in Table~\ref{tab:glk_results} reveal a parameter-vs-embedding tradeoff that clarifies the role of gauge structure. Each gauge VFE configuration is paired with two standard transformer baselines: one embedding-matched (same $d_{\text{model}}$) and one parameter-matched (comparable total parameters). For the $K\!=\!90$ gauge VFE (58.8M params, test PPL~74.9), the embedding-matched standard transformer ($d_{\text{model}} = 90$, 4.6M params) achieves only PPL~118.6, while the parameter-matched baseline ($d_{\text{model}} = 1{,}280$, 84.2M params) reaches PPL~48.5 but requires $14\times$ the embedding dimension. The gauge VFE thus outperforms a standard transformer at matched embedding dimension by $1.58\times$, demonstrating that gauge transport and KL-divergence attention extract substantially more representational capacity from a given embedding dimension than dot-product attention with MLPs. The parameter-matched baselines for the $K\!=\!10$ configuration show the same pattern at smaller scale: the embedding-matched standard transformer ($d_{\text{model}} = 10$, 0.5M params) achieves only PPL~548, while the parameter-matched baseline ($d_{\text{model}} = 116$, 6.0M params) reaches PPL~119.3. This consistent pattern across scales suggests that the gauge-theoretic structure provides an effective mechanism for compensating for low embedding dimensionality through geometric structure, at the cost of higher total parameter count invested in per-token covariance matrices $\Sigma_i$ and gauge frame parameters $\phi_i$.
