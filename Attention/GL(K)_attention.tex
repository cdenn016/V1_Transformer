...
\paragraph{$\mathrm{GL}(K)$ metric correction.}
For orthogonal gauge transport ($\Omega_{ij} \in \mathrm{SO}(K)$), the above formula is exact since the inner product is preserved under orthogonal maps. For the full $\mathrm{GL}(K)$ gauge, however, the pushforward of the mean under a non-orthogonal transformation requires a metric correction. The KL divergence $D_{\mathrm{KL}}(q_i \,\|\, \Omega_{ij} q_j)$ involves the transported covariance $\tilde{\Sigma}_{ij} = \Omega_{ij} \Sigma_j \Omega_{ij}^\top$, and the Mahalanobis inner product $(u - v)^\top \tilde{\Sigma}^{-1}(u - v)$ defines the natural metric in the transported frame. The corresponding message aggregation that respects this metric is:

\begin{equation}
m_i = \sum_j \beta_{ij} \,\Omega_{ij}^{-\top} \mu_j,
\label{eq:glk_metric_aggregation}
\end{equation}

where $\Omega_{ij}^{-\top} = (\Omega_{ij}^{-1})^\top = (\Omega_{ij}^\top)^{-1}$ is the inverse transpose. For $\mathrm{SO}(K)$, $\Omega^{-\top} = \Omega$ and this reduces to the standard formula. The covariance aggregation analogously becomes $\tilde{\Sigma}_{\mathrm{agg},i} = \sum_j \beta_{ij}[\Omega_{ij}^{-\top} \Sigma_j \Omega_{ij}^{-1} + \Omega_{ij}^{-\top}\mu_j \mu_j^\top \Omega_{ij}^{-1}] - m_i m_i^\top$, using the law of total variance under the mixture weights $\beta_{ij}$. This metric-corrected aggregation ensures that the value computation is consistent with the Mahalanobis geometry used to compute the attention weights, as required by the $\mathrm{GL}(K)$ invariance of the KL divergence (Theorem~\ref{thm:glk_invariance}).
