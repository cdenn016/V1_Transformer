...
\paragraph{Connection to the gauge transformer's per-head covariance.}
In the unreduced gauge framework, the per-head attention temperature is not a separate parameter but arises from the per-head belief covariance $\Sigma^{(a)}$. In the isotropic limit $\Sigma^{(a)} = \sigma_a^2 I$, the KL divergence scales as $1/(2\sigma_a^2)$, so the covariance \emph{is} the temperature (Section~\ref{sec:multihead}). The scalar $\kappa_a$ in Eq.~\ref{eq:per_head_temperature} parameterizes this isotropic component: $\kappa_a \propto \sigma_a^2$. This degree of freedom is intrinsic to the multi-head decomposition --- each $\mathrm{GL}(d_{\text{head}})$ block carries its own covariance structure --- and does not need to be introduced as an additional parameter.

Standard attention implicitly encodes per-head temperatures through the weight scales $\|W_Q^{(a)}\|, \|W_K^{(a)}\|$, which absorb $\sigma_a^{-2}$ into the learned projections. When these implicit temperatures are heterogeneous (as in RoBERTa), the single-$\tau$ KL approximation --- which corresponds to fixing $\sigma_a^2 = \sigma^2$ for all heads --- loses information. The gauge framework retains the per-head covariance structure, allowing each head to operate at its own point on the rate--distortion curve: small $\sigma_a^2$ (sharp attention, high rate, low distortion) for heads performing precise source selection, large $\sigma_a^2$ (diffuse attention, low rate, high distortion) for heads performing contextual averaging.
...
The temperature dispersion diagnostic (Section~\ref{sec:temp_dispersion}) reveals that standard transformers implicitly encode per-head temperatures through the weight scales $\|W_Q^{(a)}\|, \|W_K^{(a)}\|$, and that the heterogeneity of these implicit temperatures varies by a factor of $2.4\times$ across architectures (CV $= 0.331$ for BERT-large to $0.787$ for RoBERTa). In the Lie algebra decomposition $\mathfrak{gl}(d_{\text{head}}) = \mathfrak{sl}(d_{\text{head}}) \oplus \mathbb{R}$ (Section~\ref{sec:off_diagonal_mixing}), the per-head temperature corresponds to the $\mathbb{R}$ (trace) component --- the overall scaling degree of freedom within each head's gauge block.

In the gauge framework, this degree of freedom is not introduced as an additional parameter: it is the per-head belief covariance $\Sigma^{(a)}$, which is intrinsic to the multi-head decomposition (Section~\ref{sec:multihead}). In the isotropic limit, $\sigma_a^2$ \emph{is} the attention temperature for head $a$. Standard transformers absorb $\sigma_a^{-2}$ into the learned projections $W_Q^{(a)}, W_K^{(a)}$, entangling the temperature with the projection geometry. The gauge framework retains it as an explicit, learnable quantity. The empirical finding that RoBERTa's heads require different temperatures to achieve optimal $\alpha$--$\beta$ agreement (per-head optimal $\tau$: median $= 25$, std $= 15.3$) demonstrates that this covariance structure carries real information that a single global temperature discards.
