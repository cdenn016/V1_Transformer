...
\begin{figure*}[t]
  ...
  \end{subfigure}
  \caption{Learned per-head KL-divergence attention patterns ($\log_{10}\beta_{ij}$) for the $\mathrm{GL}(10)$ gauge VFE model ($K=90$) at convergence (step 60{,}000) on a representative WikiText-103 sequence. The model develops selective, non-uniform, head-specialized attention through gauge transport and KL-divergence dynamics alone, without any learned attention projections ($W_Q$, $W_K$, $W_V$).}
...
\begin{figure*}[t]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Head~4: broad contextual attention with diffuse coverage and strong early-position weighting.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{attention/figs/attention_step_060000_layer0_head8.png}
    \caption{Head~8: sharper attention with prominent vertical stripes at specific high-frequency token positions.}
  \end{subfigure}
  \caption{Learned per-head KL-divergence attention patterns ($\log_{10}\beta_{ij}$) for the $\mathrm{GL}(10)$ gauge VFE model ($K=90$) at convergence (step 60{,}000) on a representative WikiText-103 sequence. The model develops selective, non-uniform, head-specialized attention through gauge transport and KL-divergence dynamics alone, without any learned attention projections ($W_Q$, $W_K$, $W_V$).}
  \label{fig:glk_attention}
\end{figure*}
