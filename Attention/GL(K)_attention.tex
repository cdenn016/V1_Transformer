...
\subsubsection{Natural Gradient Dynamics.}

In the limit $\lambda_{p,i} = \lambda_p$ the Euclidean gradient of the free energy with respect to $\mu_i$ is:
...
Since beliefs $q_i$ live on the Gaussian statistical manifold, the geometrically correct dynamics uses the \emph{natural gradient} --- the Euclidean gradient preconditioned by the inverse Fisher information metric $G_\mu^{-1} = \Sigma_i$ (see Appendix~\ref{sec:retraction} for derivation). The natural gradient flow is:
...
The natural gradient cleanly recovers the standard transformer update rule: the alignment term $\sum_j \beta_{ij}\,\Omega\mu_j$ maps directly to the value aggregation $\sum_j \alpha_{ij} V_j$ without residual $\sigma^{-2}$ factors. This is consistent with the E-step algorithm (Algorithm~1), which applies Fisher preconditioning $\tilde{\nabla}_\mu \leftarrow \Sigma_i \cdot \nabla_\mu \mathcal{F}$.
