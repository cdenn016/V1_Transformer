...
For example, a uniform attention prior $\pi_k = 1/N$, the prior factors completely cancel yielding:
...
The structural terms $\log\det(\Omega_{ij}\Omega_{ij}^\top)$, $\mathrm{Tr}((\Omega_{ij}\Omega_{ij}^\top)^{-1})$, and $d_k$ depend on the gauge transport but not on the belief means $\mu_i, \mu_j$ directly. Under Limit~2 ($\Omega_{ij} = \Omega$), they become constant across all agent pairs and cancel under softmax thereby producing:
...
which remains finite and equals half the squared distance between means as measured in the key's reference frame. This rescaled quantity therefore has a well-defined limit as $\sigma \to 0$ and represents the natural information-geometric measure of dis-similarity within the deterministic regime. This should be understood as the regime where $\sigma$ is small compared to characteristic scales of $\mu_i$, such that $\sigma^{-2}$ will be absorbed into the learned projections (see below).
...
After our limits the KL divergence reduces to (dropping constant terms that cancel under softmax):

\begin{equation}
s_{ij} = D_{\mathrm{KL}}(q_i \| \Omega q_j) = \frac{1}{2\sigma^2}\|\Omega^{-1}\mu_i - \mu_j\|^2 + C,
\end{equation}

where $C$ collects the remaining terms (log-determinant, trace, $d_k$) that are independent of the belief means and ultimately cancel under softmax. Expanding the squared norm we have:
...
with relative fluctuations $O(1/\sqrt{d_k}) \to 0$ as $d_k$ increases. Note: this $\sigma_0$ has nothing to do with $\sigma$
...
Since beliefs $q_i$ live on the Gaussian statistical manifold, the geometrically correct dynamics uses the natural gradient. The Euclidean gradient must be pre-conditioned by the inverse Fisher information metric which, for MVGs is $G_\mu^{-1} = \Sigma_i$ (see Appendix~\ref{sec:retraction} for derivation). The natural gradient flow is:
...
(Appendix~\ref{app:covariance_dynamics}), so covariance alignment
...
\subsubsection{Per-Head Temperature Dispersion and the RoBERTa Anomaly}
\label{sec:temp_dispersion}

The multi-model validation (Table~\ref{tab:multi_model}) reveals a $0.24$ spread in grand-mean correlation across architectures that all share $d_{\text{head}} = 64$. If the temperature prediction $\tau = 2\sqrt{d_{\text{head}}} = 16$ (squared-distance form; the factor of $2$ from the $\frac{1}{2}$ in the Gaussian KL) were the sole determinant of the $\alpha$--$\beta$ correspondence, all five models would achieve similar correlations at the empirical optimum $\tau = 19.0$. The observed variation, with RoBERTa at $\bar{r} = 0.612$ and ALBERT at $\bar{r} = 0.851$, demands explanation.

We investigated four hypotheses through a targeted diagnostic analysis across all five architectures (30 passages each): (1)~key-norm heterogeneity, where higher coefficient of variation (CV) of $\|K_j\|$ amplifies the residual norm-bias term; (2)~per-model optimal temperature, where the true optimal $\tau$ deviates from 19.0; (3)~angular versus norm decomposition of logit variance; and (4)~per-head effective temperature dispersion, where heads operate at heterogeneous implicit temperatures.
...
\paragraph{Connection to the gauge transformer's per-head $\kappa$.}
This finding provides direct empirical motivation for the per-head temperature $\kappa_a$ in the unreduced gauge framework (Eq.~\ref{eq:per_head_temperature}). In the gauge transformer, each head's attention weight is:
\begin{equation}
\beta_{ij}^{(a)} = \operatorname{softmax}_j\!\left(-\frac{D_{\mathrm{KL}}(q_i^{(a)} \,\|\, \Omega_{ij}^{(a)} q_j^{(a)})}{\kappa_a \sqrt{d_{\text{head}}}}\right),
\end{equation}
where $\kappa_a$ is a learnable per-head parameter. Standard attention implicitly encodes per-head temperatures through the weight scales $\|W_Q^{(a)}\|, \|W_K^{(a)}\|$, but absorbs them into the learned projections. When these implicit temperatures are heterogeneous (as in RoBERTa), the single-$\tau$ KL approximation --- which corresponds to setting $\kappa_a = \kappa$ for all heads --- loses information. The gauge transformer's explicit per-head $\kappa_a$ is the principled correction: it allows each head to find its own operating point on the rate--distortion curve of the information bottleneck, with low $\kappa_a$ (sharp attention) corresponding to high rate / low distortion and high $\kappa_a$ (diffuse attention) corresponding to low rate / high distortion.
...
\paragraph{Per-head temperature dispersion as a diagnostic of the constant-gauge limit.} The temperature dispersion analysis (Section~\ref{sec:temp_dispersion}) resolves the cross-model variation observed in the multi-model validation. The $\alpha$--$\beta$ correlation at a fixed $\tau$ is a measure of how close a pretrained model operates to the constant-gauge limit in which standard attention and KL attention coincide. Models whose heads operate at homogeneous implicit temperatures (ALBERT: CV $= 0.429$; BERT-base: CV $= 0.367$) are well-approximated by a single $\tau$, while models with heterogeneous per-head temperatures (RoBERTa: CV $= 0.787$, nearly $2\times$ BERT-base) show systematically lower correlations. This finding provides direct empirical motivation for the gauge framework's per-head $\kappa_a$ (Eq.~\ref{eq:per_head_temperature}): the learned per-head temperature is not a theoretical nicety but addresses a measurable deficiency in the single-temperature approximation. The ALBERT anomaly --- highest correlation despite the most suboptimal global $\tau$ --- is explained by its weight-sharing architecture, which constrains the gauge transport to be layer-constant and thereby produces uniformly behaved heads.

\paragraph{The unreduced framework as a language model.} The $\mathrm{GL}(10)$ gauge VFE achieves test perplexity 74.9 on WikiText-103 (Table~\ref{tab:glk_results}), without MLPs, pointwise activation functions, or learned attention projections. This represents 87\% of the performance of a matched single-layer standard transformer (PPL 65.0) despite the gauge VFE using 58.8M parameters compared to the transformer's 17.6M, suggesting that the remaining performance gap is attributable to the neural components (MLPs, learned projections, activation functions) rather than to a fundamental limitation of the gauge-theoretic formulation. That gauge-covariant variational inference alone, with no neural architecture beyond a linear output projection, produces a functioning language model supports the explanatory claim that transformers succeed in part because they approximate the underlying mathematical structure of distributed variational inference.
...
\paragraph{Per-head temperature as a measurable architectural degree of freedom.}
The temperature dispersion diagnostic (Section~\ref{sec:temp_dispersion}) reveals that standard transformers implicitly encode per-head temperatures through the weight scales $\|W_Q^{(a)}\|, \|W_K^{(a)}\|$, and that the heterogeneity of these implicit temperatures varies by a factor of $2.4\times$ across architectures (CV $= 0.331$ for BERT-large to $0.787$ for RoBERTa). In the Lie algebra decomposition $\mathfrak{gl}(d_{\text{head}}) = \mathfrak{sl}(d_{\text{head}}) \oplus \mathbb{R}$ (Section~\ref{sec:off_diagonal_mixing}), the per-head temperature corresponds to the $\mathbb{R}$ (trace) component --- the overall scaling degree of freedom within each head's gauge block. Standard transformers absorb this into the learned projections; the gauge framework makes it explicit via $\kappa_a$. The empirical finding that RoBERTa's heads require different temperatures to achieve optimal $\alpha$--$\beta$ agreement (per-head optimal $\tau$: median $= 25$, std $= 15.3$) suggests that architectures which explicitly parameterize per-head temperature scaling --- rather than entangling it with the projection geometry --- may achieve better control over the information bottleneck at each head. Each head's $\kappa_a$ controls its operating point on the rate--distortion curve: low $\kappa_a$ (sharp attention, high rate, low distortion) for heads performing precise source selection, and high $\kappa_a$ (diffuse attention, low rate, high distortion) for heads performing contextual averaging.
...
\end{equation}
is pure gauge by construction. To see what this implies, consider the discrete analog of curvature: the holonomy around a closed loop of agents $i \to j \to k \to i$,
\begin{equation}
...
\label{eq:holonomy}
\end{equation}
The consecutive inverse--exponential pairs cancel identically: $e^{-\phi_j}e^{\phi_j} = I$. The holonomy vanishes for every closed loop, regardless of the learned gauge frames $\phi_i$. The bundle is automatically flat. This is not an approximation or a simplifying assumption rather it is a theorem of the architecture. Any transport of the form $\Omega_{ij} = g_i\,g_j^{-1}$ for vertex-local group elements $g_i$ satisfies the cocycle condition $\Omega_{ij}\,\Omega_{jk} = \Omega_{ik}$ and therefore has vanishing discrete curvature.
...
Both architectures are thus restricted to the same geometric regime: flat bundles with trivial holonomy. In the most general discrete gauge theory on a token graph, the transport $\Omega_{ij}$ on each edge would be an independent group element, and the holonomy around closed loops would be unconstrained. A system with non-trivial holonomy would exhibit path-dependent meaning transport: relating token $i$ to token $k$ directly via $\Omega_{ik}$ would yield a different result than relating them via an intermediate token $j$ through $\Omega_{ij}\,\Omega_{jk}$. The flat bundle limit eliminates this path-dependence entirely.

\textbf{Conjecture (flat bundle optimality).} \emph{The compositional core of natural language---the semantic content that supports reliable, context-independent communication---is approximately path-independent in the sense of gauge transport. Transformers are architecturally matched to this regime.}

This conjecture involves three logically distinct claims. First, a claim about language: the bulk of semantic transport between contexts is path-independent, so that relating token $i$ to token $k$ directly yields the same result as relating them via an intermediate token $j$. Second, a motivating argument: communicative systems face functional pressure toward path-independence, since if the meaning transported from speaker $A$ to speaker $C$ depended on which intermediate speakers relayed the message, reliable communication would degrade. Third, a claim about architecture-task alignment: transformers succeed in part because their flat bundle structure matches this property of language. We regard the first and third claims as substantive and testable; the second is a plausibility argument for why language might have this structure, not a derivation.

An important caveat is that path-independence alone is insufficient to explain transformer success. Bag-of-words models are trivially path-independent yet perform poorly, because they lack content-dependent, position-aware structure. The relevant claim is that transformers implement \emph{content-dependent} path-independent transport---the learned gauge frames $\phi_i$ provide rich, context-sensitive coordinate systems while the cocycle condition $\Omega_{ij}\,\Omega_{jk} = \Omega_{ik}$ ensures that the resulting transport remains flat. It is this combination, not flatness alone, that the conjecture identifies as the operative structure.

Natural language also exhibits phenomena where semantic transport appears genuinely path-dependent: irony, pragmatic inference, context-dependent ambiguity, and culturally situated meaning are cases where the ``same'' utterance acquires different meaning depending on the interpretive path through context. These phenomena are abundant and functional, not pathological, which means the conjecture should be understood as applying to the compositional-semantic core of language rather than to all aspects of meaning. This motivates a testable prediction: if a non-flat gauge architecture (one in which $\Omega_{ij}$ on each edge is an independent group element, unconstrained by the cocycle condition) were trained on tasks requiring pragmatic or contextual inference, the learned holonomy $H_{ijk} = \Omega_{ij}\,\Omega_{jk}\,\Omega_{ki}$ should deviate measurably from the identity on precisely those token triples where meaning transport is context-dependent. Measuring learned holonomy in such architectures would provide an independent, non-circular test of whether the flat bundle approximation breaks down where predicted, and would identify the geometric boundaries of transformer-based language modeling.
...
The gauge VFE language model uses 58.8M parameters compared to the matched standard transformer's 17.6M, yet achieves worse perplexity (74.9 vs 65.0). The parameter overhead arises from the per-token covariance matrices $\Sigma_i$ and gauge frame parameters $\phi_i$, which encode information that standard transformers absorb into shared weight matrices. Whether architectural modifications (parameter sharing across tokens, low-rank covariance parameterizations) can close this efficiency gap while preserving the gauge-theoretic structure is an open question. The language modeling validation also uses a single dataset (WikiText-103) and a single language (English); extending to additional corpora and typologically diverse languages would strengthen the generality of the results.
...
\textbf{Note on appendix structure.} Appendix~A reviews standard differential geometry and gauge theory to make the paper self-contained for readers from machine learning backgrounds. Readers familiar with fiber bundles, gauge connections, and curvature may skip directly to Appendix~B (Covariance Dynamics), Appendix~C (Gauge frame gradients and numerical methods) Appendix~D (Variational Gradient Descent: Implementation and Numerical Methods), Appendix~E(Conditional Uniqueness of Forward KL),  Appendix~F (Symmetry breaking and 2D base manifold experiments).
