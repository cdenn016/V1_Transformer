...
\paragraph{Gauge Frames.} The gauge frame coefficients $\phi \in \mathbb{R}^{900}$ (9 heads $\times$ 100 dimensions per $\mathfrak{gl}(10)$ head) present a contrasting picture. Unlike the belief embeddings, the gauge frames develop negative silhouette scores ($-0.105$), indicating active anti-clustering: tokens of the same category are \emph{less} similar in gauge frame space than tokens of different categories. The inter/intra class distance ratio is 1.055 (near unity). Yet per-dimension ANOVA analysis reveals significant individual-dimension structure: 62\% of 100 sampled gauge frame dimensions are statistically significant (geometric mean $p = 7.7 \times 10^{-4}$, mean $F = 4.7$). The first three principal components explain only 6.8\% of the variance across 900 dimensions. This dissociation (per-dimension category dependence with negative global clustering) suggests that gauge frame structure is distributed across many dimensions in a way that serves the transport computation $\Omega_{ij} = \exp(\phi_i)\exp(-\phi_j)$ rather than encoding token categories in geometrically compact clusters.
