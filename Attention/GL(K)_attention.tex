...
\end{equation}
%
which is a \emph{Gated Linear Unit}: a linear function of the input gated by a Boltzmann distribution over KL energies computed from the same input. Crucially, this GLU structure is not an architectural choice --- it is the form that variational inference necessarily takes when the sufficient statistics are Gaussian. The neural network is the computational substrate; the information-geometric dynamics are primary.

\paragraph{Binary limit: sigmoid as a special case.}
The general Boltzmann gate operates over all $N$ neighbors simultaneously, with each contribution weighted by its KL energy. In the special case where only $N = 2$ sources compete for influence on agent $i$, the Boltzmann distribution over two states reduces to the logistic sigmoid $\sigma$, since $\exp(-E_1/\tau)/(\exp(-E_1/\tau) + \exp(-E_2/\tau)) = \sigma((E_2 - E_1)/\tau)$:
%
\begin{equation}
...
\end{equation}
%
This recovers the SiLU/Swish \citep{ramachandran2018searching} structure $f(x) = x \cdot \sigma(g(x))$, with the gate argument determined by relative KL energies. This binary case corresponds to the FFN sublayer, where the hidden representation is split into two competing pathways --- precisely two sources whose relative informational surprise determines the gate. The sigmoid is thus not the general activation but the $N = 2$ limit of the Boltzmann gate; the general case is the full softmax over all $N$ neighbors.

\paragraph{Boltzmann gate, Gaussian family, and connection to GELU.}

The information-geometric origin of the gate determines its functional form. In the isotropic limit $\Sigma_j = \sigma^2 I$, the unnormalized gate for neighbor $j$ is $\exp(-E_{ij}/\tau) = \exp(-\|e_{ij}\|^2 / 2\sigma^2\tau)$, a Gaussian function of the prediction error. The Boltzmann distribution over KL energies is therefore a \emph{Gaussian mixture gate}: each neighbor's weight is a Gaussian in prediction-error space, normalized by the partition function $Z_i = \sum_k \exp(-\|e_{ik}\|^2/2\sigma^2\tau)$. The activation functions used in practice are specific instantiations of this information-geometric gate under different limits:
%
\begin{align}
\text{SiLU}(x) &= x \cdot \sigma(x) && \text{logistic sigmoid gate (binary Boltzmann, $N\!=\!2$)}, \notag \\
\text{GELU}(x) &= x \cdot \Phi(x) && \text{Gaussian CDF gate}, \label{eq:activation_comparison} \\
\text{VFE}(e) &= e \cdot \exp(-\|e\|^2/\tau')/Z && \text{Boltzmann gate (general $N$, Gaussian energy)}. \notag
\end{align}
%
All three belong to the \emph{Gaussian family} of gated linear units, connected by the underlying information geometry. The logistic sigmoid $\sigma(x) = 1/(1+e^{-x})$ is the two-state Boltzmann distribution. The Gaussian CDF $\Phi(x) = \frac{1}{2}\operatorname{erfc}(-x/\sqrt{2})$ is the integral of the Gaussian PDF $\propto \exp(-x^2/2)$ that appears as the VFE gate's unnormalized weight. These are members of the same exponential family, related by the number of competing information sources ($N = 2$ for sigmoid, $N > 2$ for softmax) and the functional form of the energy ($E \propto x$ for SiLU, $E \propto x^2$ for VFE).

The information-geometric framework thus derives from first principles that the gate must be a Boltzmann weight with Gaussian energy. The specific gate functions used in practice (sigmoid for SiLU, Gaussian CDF for GELU) are computational instantiations of this deeper structure under particular limits. ReLU is the zero-temperature ($\tau \to 0$) limit: the Boltzmann distribution concentrates on the lowest-energy state, and the gate becomes a step function $\Theta$, giving $x \cdot \Theta(x) = \max(0, x)$.
