...
\paragraph{Standard attention as a degenerate limit.} The BERT validation (Section~5.2) demonstrates quantitative agreement between gauge-aligned KL attention and standard dot-product attention across 105 diverse passages and all 144 heads ($\bar{r} = 0.804$ at optimal $\tau$; Figure~\ref{fig:temperature_sweep}), with the entropy of the two distributions closely matched ($H(\beta)/H(\alpha) = 1.076$; Section~\ref{sec:entropy_analysis}). The multi-model validation (Section~\ref{sec:multi_model}) extends this to five architectures ($\bar{r} > 0.6$ for all, $\bar{r} = 0.851$ for ALBERT), and the sequence-length analysis (Section~\ref{sec:seqlen}) confirms robustness from $N = 16$ to $N = 512$ tokens. Beyond the correlation itself (which partly reflects the algebraic relationship between squared distance and dot product under approximately constant norms), the framework generates predictions of varying discriminative power. The most distinctive is the temperature prediction: the optimal $\tau$ matches the dimensional prediction $\tau \approx 2\sqrt{d_k}$ to within 19\%, a quantitative result derived from concentration of measure before any empirical fitting. The key-norm bias has the predicted sign (mean absolute correlation $\bar{\rho} = 0.256$, a modest but correctly directed effect) and its temperature dependence matches theory. Deeper layers exhibit stronger correlations, a trend consistent with increasingly structured representations, though this observation is also compatible with alternative explanations such as increasing low-rank structure in deeper layers.
