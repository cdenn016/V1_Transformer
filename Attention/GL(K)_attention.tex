...
The functional $f(\beta) = \sum_j \beta_{ij}(E_{ij} + \log\beta_{ij} - \log\pi_j)$ is strictly convex on the open simplex: its Hessian is the diagonal matrix $\mathrm{diag}(1/\beta_k)$ with strictly positive entries. The inequality constraints $\beta_j \geq 0$ are non-binding at any interior stationary point because $\log\beta_k \to -\infty$ at the boundary, so the interior KKT system reduces to the equality-Lagrangian above and any solution to it is the unique global minimum \citep{Boyd2004, Wainwright2008}.
...
\end{equation}
This differs from the gradient of the reduced free energy $\mathcal{F}_{\mathrm{red}}$ (Eq.~\ref{eq:free_energy_final}) by the covariance term $-\tau^{-1}\mathrm{Cov}_{\beta^*}(E_{ij}, \partial E_{ij}/\partial q_i)$ of Eq.~\ref{eq:autograd_envelope_gap}; the two vector fields coincide at joint stationary points and differ off-equilibrium by the softmax-gradient nonlinearity identified at Section~\ref{sec:ffn_nonlinearity}.
...
where $\tilde{\nabla}$ denotes natural gradients projected via the Fisher-Rao metric. The autograd gradient differs from $\nabla\mathcal{F}_{\mathrm{red}}$ by the covariance term of Eq.~\ref{eq:autograd_envelope_gap}, coinciding at joint stationarity. This corresponds to variational inference within the forward pass.
