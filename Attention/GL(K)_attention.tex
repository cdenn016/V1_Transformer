...
The preceding expressions assumed isotropic covariances $\Sigma_i = \sigma^2 I$ and a shared gauge connection $\Omega$. We now state the fully general gradient, valid for arbitrary covariances $\Sigma_i$, edge-dependent gauge connections $\Omega_{ij}$, and general prior precision $\Sigma_{p,i}^{-1}$ which we implement in our experiments.

Differentiating the full free energy \eqref{eq:free_energy_final} with respect to $\mu_i$ and applying the product rule through the attention weights gives:

\begin{equation}
\boxed{
\begin{aligned}
\frac{\partial \mathcal{F}}{\partial \mu_i}
&= \Sigma_{p,i}^{-1}\left(\mu_i - \mu_{\text{prior}}\right) \\
&\quad + \sum_j \left[
    \beta_{ij}\,
...
The structure parallels the isotropic case term-by-term, with three generalizations: (i) the prior precision $\lambda_p I$ is replaced by the full matrix $\Sigma_{p,i}^{-1}$; (ii) the isotropic transported precision $\sigma^{-2}(\Omega\Omega^\top)^{-1}$ is replaced by the edge-dependent transported precision $(\Omega_{ij}\Sigma_j\Omega_{ij}^\top)^{-1}$, which accounts for both the neighbor's covariance structure and the geometry of the gauge transport along each edge; and (iii) the quadratic mean-mismatch $\frac{1}{2\sigma^2}\|\Omega^{-1}\mu_i - \mu_j\|^2$ is replaced by the full KL divergence $D_{\mathrm{KL}}(q_i \,\|\, \Omega_{ij}q_j)$, which additionally couples to the covariance mismatch between $\Sigma_i$ and the transported covariance $\Omega_{ij}\Sigma_j\Omega_{ij}^\top$. The full KL cannot be reduced to a purely mean-dependent expression because the covariance contributions (trace and log-determinant terms) are $j$-dependent through $\Omega_{ij}$ and $\Sigma_j$, and therefore do not cancel under the constraint $\sum_j \partial\beta_{ij}/\partial\mu_i = 0$.
