...
The structure parallels the mean gradient: $\Sigma_{p,i}^{-1}$ is the prior precision (analogous to weight decay for $\mu$), and $\sum_j \beta_{ij}(\Omega_{ij}\Sigma_j\Omega_{ij}^\top)^{-1}$ is attention-weighted precision aggregation that pulls agent $i$'s precision toward its gauge-transported neighbors.

At equilibrium $(\partial \mathcal{F}_i / \partial \Sigma_i = 0)$, the precision satisfies the fixed-point equation:
