...
We validate the unreduced theory by training a $\mathrm{GL}(K)$ gauge transformer on WikiText-103 using only KL divergences, gauge transport, and natural gradient dynamics (without learned attention projections, MLPs, or pointwise activation functions such as ReLU or GELU). The model achieves test perplexity 71.6 without neural components, outperforming a standard transformer at matched embedding dimension by $1.66\times$ (PPL~118.6 at $d_{\text{model}} = 90$) while approaching the performance of a parameter-matched standard transformer (PPL~48.5 at $d_{\text{model}} = 1{,}280$, requiring $14\times$ the embedding dimension). The learned gauge frames develop a dual structure: categorical separation in a low-dimensional subspace with within-category diversification in the bulk. We separately validate the degenerate-limit predictions on frozen BERT across 105 diverse passages, confirming quantitative agreement between KL-based and dot-product attention (grand mean $r = 0.804$ at the predicted optimal temperature, 95\% CI $[0.771, 0.838]$, with cross-passage SE $< 0.02$ for all 144 heads). Both architectures operate in the flat bundle limit with trivially vanishing holonomy: a geometric regime we hypothesize is matched to the approximately path-independent semantic transport that compositional language requires.
\end{abstract}
...
Importantly, the proof extends to all $f$-divergences: i.e. for any convex function $f$ such that $f(1) = 0$,
...
Finally, we shall demonstrate that the query and key projection matrices $W_Q, W_K \in \mathrm{GL}(d_k)$ in standard transformers are themselves related to gauge transformations. The constant gauge limit corresponds not to the absence of gauge structure, but to a constant gauge transformation satisfying $W_Q W_K^\top = \sigma^{-2}\Omega^{-\top}$ shared among all token pairs.
...
For gauge transport of the form $\Omega_{ij} = g_i\,g_j^{-1}$ with vertex-local group elements $g_i \in G$, the holonomy around any closed loop vanishes:
\begin{equation}
...
This result is a theorem of the architecture, not an approximation. For our parameterization $\Omega_{ij} = \exp(\phi_i)\exp(-\phi_j)$, the holonomy vanishes for every closed loop regardless of the learned gauge frames $\phi_i$. The gauge bundle is automatically flat: semantic transport from token $i$ to token $k$ is path-independent, in the sense that the direct route via $\Omega_{ik}$ and the indirect route via $\Omega_{ij}\,\Omega_{jk}$ yield identical results. Architectures with non-trivial holonomy would require promoting $\Omega_{ij}$ to an independent edge variable unconstrained by the cocycle condition; we return to the implications of this geometric constraint in Section~\ref{sec:flat_bundle}.
...
The first term regularizes beliefs toward the prior and the second couples beliefs to observations. An analogous expression holds for a model channel (Appendix~\ref{app:model_channel}). We now answer the question: how do agents communicate?
...
Solving and normalizing we find the softmax form (importantly) weighted by attention priors $\pi_k$:
...
Without loss of generality we may include a temperature parameter $\tau > 0$ by rescaling the alignment energy $E_{ik} \to E_{ik}/\tau$. Additionally, as discussed in the appendix, the forward KL divergence is the unique $f$-divergence that preserves exponential-family closure under this linear coupling structure and yields a consistent dual interpretation for the attention weights (see Appendix~A).
...
By combining the single-agent free energy \eqref{eq:single_agent_fep} with the alignment terms derived above and summing over all agents, we obtain the global gauge-covariant variational free energy at $c^* \in \mathcal{C}$:
...
We note that this is the partially minimized free energy: the attention weights $\beta_{ij}^*$ have already been set to their optimal softmax values \eqref{eq:mixture_softmax_general} by minimizing the full alignment free energy $\mathcal{F}_{\text{align}}$ \eqref{eq:mixture_energy_entropy} with respect to $\beta$. The full (un-minimized) alignment energy includes an attention entropy/prior term $\tau\sum_{i,j}\beta_{ij}\log(\beta_{ij}/\pi_j)$ (cf.\ Eq.~\ref{eq:mixture_energy_entropy}), which is essential for it is this term that makes the $\beta$-optimization yield the softmax rather than an arg-min. When $\beta_{ij}$ is set to its optimal value $\beta_{ij}^* = \softmax_j(-E_{ij}/\tau + \log\pi_j)$, the entropy term evaluates to $-\tau\sum_i\log Z_i$ (where $Z_i = \sum_j \pi_j\exp(-E_{ij}/\tau)$ is the partition function), which is absorbed into $\mathcal{F}[\{q_i\}]$ as a function of beliefs alone. This absorbed log-partition function does not appear as an explicit loss term in the implementation, but it contributes to the gradients $\partial\mathcal{F}/\partial\mu_i$ through the chain rule via $\partial\beta^*/\partial\mu_i$ representing a fundamental non-linearity in the theory as we discuss further below.

The envelope theorem implies consistency since:

$$
\partial\mathcal{F}/\partial\mu_i\big|_{\beta=\beta^*} =\frac{d}{d\mu_i}\mathcal{F}[\{q_i\}, \beta^*(q)]
$$ 

since the cross-term

$$
\sum_j(\partial\mathcal{F}/\partial\beta_{ij})(\partial\beta^*_{ij}/\partial\mu_i)
$$

vanishes at the optimal attention. The gradient expressions in Section~4.3, which include $\partial\beta_{ij}/\partial\mu_i$ terms, are therefore correct as written under this partially minimized convention (similarly for $\Sigma$ and $\phi$) .
...
Multi-agent communication within this gauge-covariant formulation allows the FEP to be satisfied. Importantly, there is no necessitation of any informational substrate (such as neural networks, transmission lines, etc) within our formalism. Next, we show that this formulation connects attention, transformers, and architecture to variational inference without neural networks, MLPs, and other standard ad-hoc architectures.
...
The gauge-covariant attention $\beta_{ij} = \operatorname{softmax}(-D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j)/\tau)$ derived in the preceding section is already a complete attention mechanism. Standard transformer attention is a special case, obtained by taking two successive limits: isotropic covariances (which separates the KL into a geometric bias and a Mahalanobis distance) and constant gauge transport (which eliminates the bias and yields the familiar $QK^\top / \sqrt{d_k}$ form). Our experiments (Section~\ref{sec:glk_lm}) implement the general model without these limits.
...
%
\begin{equation}
\Omega_{ij} = R_i \, M_{ij} \, R_j, \qquad M_{ij} = \exp(\phi_i)\exp(-\phi_j) \in \mathrm{SO}(K),
\label{eq:ok_transport}
\end{equation}
%
which covers both components of $\mathrm{O}(K)$ since $\det(\Omega_{ij}) = (\prod_k s_i^{(k)})(\prod_k s_j^{(k)})\det(M_{ij})$ ranges over $\pm 1$.  Orthogonality $\Omega_{ij}\Omega_{ij}^\top = R_i M_{ij} \underbrace{R_j R_j}_{I} M_{ij}^\top R_i = R_i \underbrace{M_{ij}M_{ij}^\top}_{I} R_i = \underbrace{R_i^2}_{I} = I$ follows from the fact that each factor is independently orthogonal, ensuring $S(\Omega_{ij}) = 0$ and preservation of isotropic covariance under transport.  The Mahalanobis distance decomposes cleanly: since $\Omega_{ij}^{-1} = R_j M_{ij}^\top R_i$ and each $R$ is its own inverse,
%
\begin{equation}
...
Such a factorization always exists for any invertible $\Omega$ since for $\Omega^{-\top} \in \mathrm{GL}(d_k)$, any matrix $M \in \mathrm{GL}(d_k)$ can be written as $M = AB^\top$ for suitable $A, B$ (e.g., via SVD: $M = U\Lambda V^\top = (U\Lambda^{1/2})(V\Lambda^{1/2})^\top$).

Importantly this identification concerns the bilinear form $M = W_Q W_K^\top$ that determines the attention logits rather than the individual matrices $W_Q$ and $W_K$ separately. In standard transformer implementations, the per-head projection matrices $W_Q^a, W_K^a \in \mathbb{R}^{d_{\text{model}} \times d_{\text{head}}}$ are rectangular (with $d_{\text{model}} \gg d_{\text{head}}$), and neither is individually invertible in the $d_{\text{model}}$-dimensional space. The gauge-theoretic identification is within the $d_{\text{head}}$-dimensional projected subspace where the effective bilinear form $M^a = (W_Q^a)^\top W_K^a \in \mathbb{R}^{d_{\text{head}} \times d_{\text{head}}}$ acts.

The standard rectangular shape of $W_Q^a$ arises from a further restriction: decomposing $\Omega$ into a block-diagonal subgroup $\mathrm{GL}(d_{\text{head}})^H \subset \mathrm{GL}(d_k)$, where each head $a$ operates within an independent $d_{\text{head}}$-dimensional subspace. The rectangular shape encodes both the orthogonal projection onto block $a$ and the $\mathrm{GL}(d_{\text{head}})$ transport within that block. We develop and discuss this identification in detail in Section~\ref{sec:multihead}. For now we return to the attention derivation.
...
\begin{equation}
\mathcal{F}_{\text{vacuum}}[\{q_i\}] = \sum_i \alpha_iD_{\mathrm{KL}}(q_i \| p_i) + \sum_{i,j} \beta_{ij} D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j).
\end{equation}
...
The natural gradient recovers the standard transformer update rule. The alignment term $\sum_j \beta_{ij}\,\Omega\mu_j$ maps directly to the value aggregation $\sum_j \alpha_{ij} V_j$ without spurious $\sigma^{-2}$ factors. This is consistent with the E-step algorithm we implement, which applies Fisher preconditioning $\tilde{\nabla}_\mu \leftarrow \Sigma_i \cdot \nabla_\mu \mathcal{F}$.
...
Differentiating the full free energy \eqref{eq:free_energy_final} with respect to $\mu_i$ and applying the product rule through both the attention weights $\beta_{ij}(\mu_i)$ and the adaptive prior coupling $\alpha_i(\mu_i)$ gives:
...
Similarly, the complete theory also includes gradient dynamics for the covariance $\Sigma_i$ and gauge frame $\phi_i$. The covariance gradient reveals a novel uncertainty channel absent in standard transformers.  Differentiating the free energy with respect to $\Sigma_i$ and applying the product rule through both the attention weights and the adaptive prior coupling $\alpha_i(\Sigma_i)$ (neglecting the observation term, which contributes an $O(\Sigma_i^{-1})$ correction in the high-precision regime; see Supplementary Appendix~B for the full derivation) yields:
...
\hline
$\mu^{(\ell+1)} \!=\! \mu^{(\ell)} - \tilde{\eta}\,\Sigma\nabla\mathcal{F}$ & Natural gradient flow & Residual connection & D \\
...
\hline
\end{tabular}
\caption{Comprehensive correspondence between the gauge-theoretic VFE framework and transformer architecture. \textbf{Status column:} D = derived from the variational principle (mathematically exact or follows from explicit limits); S = \emph{structural} correspondence (the framework explains the component's role but does not uniquely predict its specific form).}
...
The message $e_{ij} \equiv \mu_i - \Omega_{ij}\mu_j$ is linear in the beliefs. The attention gate $\beta_{ij} = \operatorname{softmax}_j\!\bigl(-D_{\mathrm{KL}}(q_i \| \Omega_{ij}q_j)/\tau\bigr)$ is manifestly a Boltzmann weight: each neighbor's contribution is weighted by $\exp(-E_{ij}/\tau)/Z_i$, where the energy $E_{ij} = D_{\mathrm{KL}}(q_i \| \Omega_{ij}q_j)$ depends on the message magnitude through the Gaussian KL as:
%
\begin{equation}
...
ReLU is the zero-temperature ($\tau \to 0$) limit: the Boltzmann distribution concentrates on the lowest-energy state and the gate becomes a step function $\Theta$, giving $x \cdot \Theta(x) = \max(0, x)$.

\paragraph{Softmax-gradient correction.}

Beyond the GLU structure of the message-passing term itself, the full VFE gradient contains an additional nonlinear correction from differentiating $\beta_{ij}$ with respect to $\mu_i$:
...
\hline
\end{tabular}
\caption{Multi-model validation at $\tau = 19.0$ across 20 passages. All models use $d_{\text{head}} = 64$. The $\alpha$--$\beta$ correspondence holds across architectures, with $\bar{r} > 0.6$ for all five models and $\bar{r} > 0.8$ for ALBERT and BERT-base. RoBERTa's lower correlation is explained by per-head temperature dispersion rather than pretraining differences (see Section~\ref{sec:temp_dispersion}).}
\label{tab:multi_model}
\end{table}

The correspondence holds across all five architectures ($\bar{r} > 0.6$ in every case), with ALBERT achieving the highest agreement ($\bar{r} = 0.851$). ALBERT's parameter-sharing across layers forces all layers to use the same projection matrices, constraining the gauge transport to be constant across layers and thereby regularizing attention patterns toward the distance-based structure predicted by the gauge theory. RoBERTa's lower correlation ($\bar{r} = 0.612$) is not attributable to its pretraining objective per se; rather, a targeted diagnostic analysis (Section~\ref{sec:temp_dispersion}) identifies per-head temperature dispersion as the dominant explanatory variable: RoBERTa's heads operate at nearly $2\times$ the temperature heterogeneity of BERT-base, and a single universal $\tau$ cannot simultaneously approximate all heads.

The theoretical prediction $\tau \approx 2\sqrt{d_{\text{head}}}$ remains the best single-parameter approximation: all five models share $d_{\text{head}} = 64$, and the dimensional scaling correctly predicts the order of magnitude. However, the cross-model variation in correlation at this fixed $\tau$ reveals that the \emph{per-head} temperature structure --- predicted by the gauge framework's learnable $\kappa_a$ (Eq.~\ref{eq:per_head_temperature}) --- carries additional information that the single-$\tau$ approximation discards.


\subsubsection{Per-Head Temperature: Prediction and Confirmation}
\label{sec:temp_dispersion}

The multi-head decomposition of Section~\ref{sec:multihead} predicts that each attention head carries its own effective temperature through the per-head belief covariance $\Sigma^{(a)}$: in the isotropic limit, $\sigma_a^2$ is the attention temperature for head $a$ (Eq.~\ref{eq:per_head_temperature}). Standard transformers absorb $\sigma_a^{-2}$ into $W_Q^{(a)}, W_K^{(a)}$, making this structure implicit. A single-$\tau$ KL approximation therefore corresponds to the assumption $\sigma_a^2 = \sigma^2$ for all heads, and models whose heads operate at heterogeneous implicit temperatures should show systematically worse agreement with this approximation. The multi-model validation provides a direct test: all five architectures share $d_{\text{head}} = 64$, yet the grand-mean $\alpha$--$\beta$ correlation at $\tau = 19.0$ spans a $0.24$ range from RoBERTa ($\bar{r} = 0.612$) to ALBERT ($\bar{r} = 0.851$).

We investigated four hypotheses through a targeted diagnostic analysis across all five architectures (30 passages each): (1)~key-norm heterogeneity, where higher coefficient of variation (CV) of $\|K_j\|$ amplifies the residual norm-bias term; (2)~per-model optimal temperature, where the true optimal $\tau$ deviates from 19.0; (3)~angular versus norm decomposition of logit variance; and (4)~per-head effective temperature dispersion, where heads operate at heterogeneous implicit temperatures. The gauge framework predicts that Hypothesis~4 should be the dominant variable.

\paragraph{Temperature dispersion as the dominant explanatory variable.}
In standard dot-product attention, the score $s_{ij} = Q_i \cdot K_j / \sqrt{d}$ implicitly encodes a per-head temperature through the learned weight scales $\|W_Q^{(a)}\|$ and $\|W_K^{(a)}\|$. Heads with larger-norm projections produce higher-variance logits and therefore sharper attention distributions (lower effective temperature), while heads with smaller-norm projections produce softer distributions (higher effective temperature). We quantify this implicit temperature heterogeneity by computing the coefficient of variation (CV) of per-head attention-logit standard deviation across all (layer, head) pairs for each model.
...
\hline
\end{tabular}
\caption{Per-head temperature dispersion diagnostic across five architectures ($d_{\text{head}} = 64$, 30 passages). Key-norm CV: coefficient of variation of $\|K_j\|$ across tokens (Hypothesis~1). $\tau_{\text{opt}}$: temperature maximizing the mean $\alpha$--$\beta$ Pearson $r$ (Hypothesis~2). $r$@$\tau^*$: correlation at the model-specific optimal $\tau$. $r$@19: correlation at the universal prediction $\tau = 19.0$. Temp disp (CV): coefficient of variation of per-head logit standard deviation across all (layer, head) pairs (Hypothesis~4). Temperature dispersion is the strongest predictor of the cross-model correlation deficit.}
\label{tab:temp_dispersion}
\end{table}

Table~\ref{tab:temp_dispersion} reveals that Hypothesis~4 (per-head temperature dispersion) is the dominant explanatory variable. RoBERTa exhibits the highest temperature dispersion (CV $= 0.787$, nearly $2\times$ that of BERT-base) and the lowest $\alpha$--$\beta$ correlation ($r = 0.611$). The ranking by temperature dispersion anti-correlates with the ranking by correlation: models with more heterogeneous implicit per-head temperatures show systematically worse agreement with a single-temperature KL approximation.

\paragraph{Ruling out alternative hypotheses.}
Key-norm CV (Hypothesis~1) does not explain the pattern: RoBERTa actually has the \emph{lowest} key-norm CV (0.105), indicating more uniform key-vector norms than any other model. The norm-fraction $R^2$ (Hypothesis~3) is similar across models ($0.09$--$0.19$) and does not differentiate RoBERTa from the other architectures. The per-model optimal $\tau$ (Hypothesis~2) shows that RoBERTa's optimum ($\tau = 29.0$) deviates from the dimensional prediction, but this alone is insufficient: ALBERT achieves the highest correlation ($r = 0.853$) despite its optimal $\tau = 35.0$ being even further from 19.0.

\paragraph{The ALBERT anomaly as a control case.}
ALBERT's weight-sharing across all 12 layers forces every layer to use the same $W_Q, W_K$ projection matrices. In the gauge-theoretic framework, this constrains the gauge transport to be constant across layers --- effectively a flat connection in which the per-head temperature structure is identical at every depth. This produces low temperature dispersion (CV $= 0.429$) despite a suboptimal global $\tau$, explaining why a single-temperature KL approximation remains accurate. ALBERT thus serves as a natural control: shared weights enforce the constant-gauge limit under which standard attention and KL attention are most tightly linked.

\paragraph{Per-head optimal temperature distributions.}
The per-head optimal $\tau$ distributions further support this analysis. RoBERTa's heads span the widest range (median $= 25.0$, std $= 15.3$), while BERT-base heads cluster more tightly around the prediction (median $= 21.0$, std $= 10.6$). This confirms that RoBERTa's heads have learned to operate at maximally heterogeneous effective temperatures, and a single universal $\tau$ cannot simultaneously approximate all of them.

\paragraph{Confirmation of the per-head covariance prediction.}
The results confirm the prediction derived in Section~\ref{sec:multihead}: the per-head attention temperature is not a separate parameter but arises from the per-head belief covariance $\Sigma^{(a)}$. In the isotropic limit $\Sigma^{(a)} = \sigma_a^2 I$, the KL divergence scales as $1/(2\sigma_a^2)$, so the covariance \emph{is} the temperature, parameterized by $\kappa_a \propto \sigma_a^2$ in Eq.~\ref{eq:per_head_temperature}. This degree of freedom is intrinsic to the multi-head decomposition --- each $\mathrm{GL}(d_{\text{head}})$ block carries its own covariance structure --- and does not need to be introduced as an additional architectural choice.

Standard attention implicitly encodes per-head temperatures through the weight scales $\|W_Q^{(a)}\|, \|W_K^{(a)}\|$, which absorb $\sigma_a^{-2}$ into the learned projections. The theory predicts that when these implicit temperatures are heterogeneous, the single-$\tau$ KL approximation will degrade, and the degree of degradation should track the coefficient of variation of per-head temperatures. This is precisely what is observed: the ranking by temperature dispersion anti-correlates with the ranking by $\alpha$--$\beta$ agreement across all five architectures. The gauge framework retains the per-head covariance structure, allowing each head to operate at its own point on the rate--distortion curve: small $\sigma_a^2$ (sharp attention, high rate, low distortion) for heads performing precise source selection, large $\sigma_a^2$ (diffuse attention, low rate, high distortion) for heads performing contextual averaging.
...
Gauge VFE ($\mathrm{GL}(10)$, $K\!=\!90$, 9 heads) & 1 & 90 & 58.8M & 78.7 & 74.9 \\
...
\end{equation}
where $\delta_\ell \sim \mathcal{N}(0, \sigma_{\mathrm{layer}}^2)$ captures between-layer variation and $\epsilon_h \sim \mathcal{N}(0, \sigma_{\mathrm{head}}^2)$ captures within-layer variation. Each head's observed correlation enters the likelihood with its empirically measured standard error, propagated to $z$-space via the delta method. The model places weakly informative priors: $\mu_{\mathrm{grand}} \sim \mathcal{N}(0.8,\, 0.5)$, $\sigma_{\mathrm{layer}} \sim \mathrm{HalfNormal}(0.5)$, $\sigma_{\mathrm{head}} \sim \mathrm{HalfNormal}(0.3)$.

The posterior grand mean correlation is $\bar{r}_{\mathrm{post}} = 0.867$ (94\% HDI $[0.808,\, 0.915]$). The upward shift from the raw mean of 0.804 reflects hierarchical shrinkage: the model partially pools the minority of low-$r$ heads toward the population mean, recognizing that extreme values from heads with higher measurement uncertainty are likely regression-to-the-mean artifacts. The within-layer standard deviation ($\hat{\sigma}_{\mathrm{head}} = 0.530$ in $z$-space) exceeds the between-layer standard deviation ($\hat{\sigma}_{\mathrm{layer}} = 0.357$), indicating that most of the variance in $\alpha$--$\beta$ agreement is attributable to functional specialization across heads within the same layer rather than to systematic differences between layers. The posterior fraction of heads exceeding $r > 0.8$ is $0.647$ (94\% HDI $[0.639,\, 0.653]$), consistent with the observed fraction of 93/144 $= 0.646$.

The layer-level posteriors reveal structure invisible in the aggregate statistics. Layers 4--8 and 11 achieve posterior mean correlations exceeding 0.91, while layers 0--3 cluster near 0.77. This gradient is consistent with the expectation that deeper layers develop more structured semantic representations, for which the isotropic-covariance KL approximation is tighter, though it may also reflect increasing low-rank structure in the learned projections.

\subsubsection{Temperature Posterior}

Rather than reporting the empirical optimum $\tau = 19.0$ as a point estimate, we fit a Bayesian response-surface model to the 10-point temperature sweep data. The correlation as a function of $\tau$ is modeled as a concave quadratic in log-space, $r(\tau) = r_{\max} - \kappa(\log\tau - \log\tau_{\mathrm{opt}})^2$, with priors $\log\tau_{\mathrm{opt}} \sim \mathcal{N}(\log 16, 0.3)$ centered on the theoretical prediction $\tau = 2\sqrt{d_k} = 16$, $r_{\max} \sim \mathcal{N}(0.81, 0.05)$, and $\kappa \sim \mathrm{HalfNormal}(0.02)$. The observation-level variance combines estimated standard errors from the bootstrap confidence intervals with a residual noise term.

The posterior for the optimal temperature is $\hat{\tau}_{\mathrm{opt}} = 21.2$ (94\% HDI $[11.6,\, 31.5]$), with the theoretical prediction $\tau = 16$ falling well within the credible interval. The posterior deviation from theory is $+32\%$ (94\% HDI $[-27\%,\, +97\%]$), meaning the data cannot distinguish between the theoretical prediction and the empirical point estimate at $\tau = 19$; both are consistent with the posterior. The breadth of the HDI reflects the flatness of the response curve near the optimum (Figure~\ref{fig:temperature_sweep}): the correlation varies by only 0.005 between $\tau = 15$ and $\tau = 25$, providing limited information to localize the peak precisely. This is itself informative: the theory predicts a broad optimum arising from smooth dimensional corrections rather than a sharp resonance, and the posterior width confirms this.

\subsubsection{Key-Norm Bias Effect Sizes}

The frequentist analysis reports mean absolute correlations ($\overline{|\rho_\alpha|} = 0.139$, $\overline{|\rho_\beta|} = 0.256$) without formal comparison of effect magnitude. To quantify the difference, we fit a hierarchical model with population-level means and per-head random effects for each attention type, placing weakly informative priors $\mu_\alpha \sim \mathcal{N}(0, 0.5)$ and $\mu_\beta \sim \mathcal{N}(-0.5, 0.5)$ on the signed population means.

The posterior population mean for dot-product key-norm correlation is $\hat{\mu}_\alpha = -0.144$ (94\% HDI $[-0.195,\, -0.096]$), with absolute magnitude $|\hat{\mu}_\alpha| = 0.144$, consistent with the reported $\overline{|\rho_\alpha|} = 0.139$. For KL-distance attention, the posterior population mean is $\hat{\mu}_\beta = -0.475$ (94\% HDI $[-0.507,\, -0.447]$). The consistently negative sign across all 144 heads matches the theoretical prediction that the $-\lambda\|K_j\|^2$ bias term suppresses attention to high-norm keys. The standardized effect size is Cohen's $d = 1.43$ (94\% HDI $[1.08,\, 1.88]$), a large effect by conventional criteria, and the posterior probability that KL-distance attention exhibits stronger key-norm sensitivity than dot-product attention is $P(|\mu_\beta| > |\mu_\alpha|) = 1.000$ across all posterior samples.

\subsubsection{Multi-Model and Sequence-Length Analyses}

A partial-pooling model across the five validated architectures yields a population mean correlation of $\hat{\mu}_{\mathrm{pop}} = 0.758$ (94\% HDI $[0.653,\, 0.853]$). The shrinkage is negligible (maximum $|\Delta r| < 0.001$), confirming that the five models are sufficiently well-separated that partial pooling adds little information beyond the individual estimates. ALBERT ($\hat{r} = 0.850$, 94\% HDI $[0.835,\, 0.865]$) and BERT-base ($\hat{r} = 0.803$, 94\% HDI $[0.781,\, 0.825]$) remain clearly above the population mean, while RoBERTa ($\hat{r} = 0.612$, 94\% HDI $[0.601,\, 0.624]$) remains clearly below.

For sequence-length degradation, we model the correlation as $r(N) = r_0 - \beta_{\mathrm{decay}} \log_2(N/32)$, finding a degradation rate of $\hat{\beta}_{\mathrm{decay}} = 0.016$ per doubling (94\% HDI $[0.006,\, 0.024]$). The posterior predictive correlation at $N = 512$ is $\hat{r} = 0.731$ (94\% HDI $[0.704,\, 0.759]$), and extrapolation to $N = 1024$ gives $\hat{r} = 0.715$ (94\% HDI $[0.681,\, 0.752]$), indicating that the $\alpha$--$\beta$ correspondence remains above 0.7 well beyond the training sequence lengths of typical BERT models. The implied half-life, defined as the number of doublings from $N = 32$ required to reach $r = 0.5$, has posterior mean 27.6 doublings (94\% HDI $[10.5,\, 35.0]$), corresponding to sequence lengths far beyond any practical transformer context window.
...
\paragraph{Standard attention as a degenerate limit.} The BERT validation (Section~5.2) demonstrates quantitative agreement between gauge-aligned KL attention and standard dot-product attention across 105 diverse passages and all 144 heads ($\bar{r} = 0.804$ at optimal $\tau$; Figure~\ref{fig:temperature_sweep}), with the entropy of the two distributions closely matched ($H(\beta)/H(\alpha) = 1.076$; Section~\ref{sec:entropy_analysis}). A hierarchical Bayesian model accounting for the nested structure of heads within layers yields a shrinkage-corrected grand mean of $\bar{r}_{\mathrm{post}} = 0.867$ (94\% HDI $[0.808,\, 0.915]$; Section~\ref{sec:bayesian_validation}), with the upward shift from 0.804 reflecting partial pooling of the minority of low-correlation heads. The multi-model validation (Section~\ref{sec:multi_model}) extends this to five architectures ($\bar{r} > 0.6$ for all, $\bar{r} = 0.851$ for ALBERT), and the sequence-length analysis (Section~\ref{sec:seqlen}) confirms robustness from $N = 16$ to $N = 512$ tokens. Beyond the correlation itself (which partly reflects the algebraic relationship between squared distance and dot product under approximately constant norms), the framework generates predictions of varying discriminative power. The most distinctive is the temperature prediction: the Bayesian posterior for the optimal temperature is $\hat{\tau}_{\mathrm{opt}} = 21.2$ (94\% HDI $[11.6,\, 31.5]$), encompassing both the theoretical prediction $\tau = 2\sqrt{d_k} = 16$ and the empirical point estimate $\tau = 19$, with the breadth of the credible interval reflecting the flat response plateau predicted by smooth dimensional corrections. The key-norm bias has the predicted sign, with a Bayesian effect size of Cohen's $d = 1.43$ (94\% HDI $[1.08,\, 1.88]$) confirming that KL-distance attention exhibits substantially stronger key-norm sensitivity than dot-product attention ($P(|\mu_\beta| > |\mu_\alpha|) = 1.000$). Deeper layers exhibit stronger correlations, a trend consistent with increasingly structured representations, though this observation is also compatible with alternative explanations such as increasing low-rank structure in deeper layers.

\paragraph{Per-head temperature as a confirmed prediction.} The per-head temperature analysis (Section~\ref{sec:temp_dispersion}) provides the most discriminative empirical test of the framework. The theory predicts that each head carries an intrinsic attention temperature through its belief covariance $\sigma_a^2$ (Section~\ref{sec:multihead}), and that models with heterogeneous implicit per-head temperatures should show degraded agreement with any single-$\tau$ KL approximation. This prediction is confirmed across all five architectures: the ranking by temperature dispersion anti-correlates with the ranking by $\alpha$--$\beta$ agreement, with the degree of degradation tracked by the coefficient of variation of per-head logit standard deviation (CV $= 0.367$ for BERT-base vs.\ $0.787$ for RoBERTa). ALBERT serves as a natural control: its weight-sharing constrains the gauge transport to be layer-constant, producing uniformly behaved heads and the highest single-$\tau$ correlation ($\bar{r} = 0.851$) despite the most suboptimal global temperature.

\paragraph{The unreduced framework as a language model.} The $\mathrm{GL}(15)$ gauge VFE achieves test perplexity 71.6 on WikiText-103 (Table~\ref{tab:glk_results}), without MLPs, pointwise activation functions, or learned attention projections. Embedding-matched and parameter-matched baselines (Table~\ref{tab:glk_results}) clarify the nature of this result. At matched embedding dimension ($d_{\text{model}} = 90$), a standard transformer achieves only PPL~118.6 with 4.6M parameters, while the parameter-matched standard transformer (84.2M params, $d_{\text{model}} = 1{,}280$) achieves PPL~48.5 but requires $14\times$ the embedding dimension. The gauge VFE (81.4M params) thus outperforms standard transformers at comparable embedding dimensions by $1.66\times$, but underperforms when the standard transformer can invest its parameters in a much larger embedding space. This indicates that gauge transport and KL-divergence attention provide an effective mechanism for compensating for low embedding dimensionality through geometric structure, though the per-token covariance and gauge frame parameters incur a parameter overhead relative to shared weight matrices. That gauge-covariant variational inference alone, with no neural architecture beyond a linear output projection, produces a functioning language model supports the explanatory claim that transformers succeed in part because they approximate the underlying mathematical structure of distributed variational inference.
...
\begin{align}
g_1 &= \|\Sigma_i - \sigma^2 I\| / \sigma^2 & &\text{(anisotropy: deviation from isotropic covariance)}, \label{eq:g1_def} \\
g_2 &= \|\Omega_{ij} - \Omega\| / \|\Omega\| & &\text{(gauge variation: deviation from constant transport)}, \label{eq:g2_def} \\
g_3 &= \|H_{ijk} - I\| & &\text{(holonomy: deviation from flat bundle)}, \label{eq:g3_def}
\end{align}
...
\label{eq:rg_scaling}
\end{equation}
where the exponents follow from the central limit theorem (averaging $n$ independent traceless perturbations $\Delta_i$ reduces the norm by $1/\sqrt{n}$), the law of large numbers applied to pairwise transport (averaging over $n^2$ pairs), and the product structure of holonomy (three transport factors each decaying as $1/n$).  The linearised RG matrix at the fixed point is diagonal:
\begin{equation}
...
Let $\mathcal{T}(K, N, g_1, g_2, g_3)$ denote the family of gauge VFE models with belief dimension $K$, sequence length $N$, and coupling constants $(g_1, g_2, g_3)$ as defined in Eqs.~\eqref{eq:g1_def}--\eqref{eq:g3_def}. Let $R_n: \mathcal{T} \to \mathcal{T}$ be the coarse-graining map~\eqref{eq:meta_agent_beliefs}. Then:
\begin{enumerate}
...
\item[(iii)] \textbf{Universality.} All finite-coupling models flow to the transformer limit under repeated coarse-graining, with $\|g(\zeta) - g^*\| \sim b^{y_1 \zeta}$ where $b$ is the rescaling factor and $\zeta$ the RG step.
\item[(iv)] \textbf{Emergent anisotropy.} The coarse-graining map generates anisotropy from within-cluster variance: even at $g_1 = 0$, $\Sigma_A = \sigma^2 I + \mathrm{Var}_A(\mu)$ is generically anisotropic. Transformers absorb this structure into $W_Q, W_K$; the gauge VFE tracks it explicitly in $\Sigma_i$.
\item[(v)] \textbf{Efficiency gap.} The absorbed degrees of freedom scale as $O(K^2)$, predicting an $O(\sqrt{K})$ sample-efficiency advantage for the gauge VFE over parameter-matched transformers.
\end{enumerate}
\end{conjecture}

The conjecture has several immediate consequences. First, the \emph{ordering} of the three limits in Section~\ref{sec:transformer_limit} is not arbitrary but dictated by the RG: holonomy vanishes fastest ($y_3 = -2$), then gauge variation ($y_2 = -1$), then anisotropy ($y_1 = -\tfrac{1}{2}$), matching the natural hierarchy Limit~3 $\to$ Limit~2 $\to$ Limit~1.  Second, the crossover exponent $\varphi = y_2/y_1 = 2$ quantifies how much faster gauge variation decays relative to anisotropy.  Third, the critical exponents $\nu = -1/y_1 = 2$ (correlation length) and $\eta = 0$ (anomalous dimension) place the fixed point in a Gaussian (mean-field) universality class, consistent with the CLT basis of the scaling argument.

\paragraph{Emergent anisotropy and LayerNorm.}
The generation of anisotropy under coarse-graining~\eqref{eq:meta_agent_beliefs} provides a geometric interpretation of LayerNorm: it acts as a \emph{projection back toward the isotropic submanifold} at each layer, counteracting the emergent anisotropy that the RG flow produces.  Without LayerNorm, the transformer would drift away from the fixed point at each layer; with it, the network is repeatedly returned to a neighborhood of $g_1 = 0$.  The gauge VFE, by contrast, tracks the full anisotropic covariance $\Sigma_i$ and requires no such projection.

\paragraph{Compute crossover.}
Both architectures share the same scaling exponents (universality class), but differ in per-step computational cost: the gauge VFE requires approximately $10$--$30\times$ the FLOPs of standard attention due to matrix exponentials and per-pair KL divergences.  The efficiency gap from part~(v) of the conjecture predicts a crossover compute budget $C^* = O(K^2 V)$ (where $V$ is the vocabulary size) below which the gauge VFE's stronger inductive bias compensates for its higher per-step cost, and above which the transformer's cheaper iterations dominate.

\subsection{Limitations}

Our BERT validation tested the flat bundle limit where all frames are globally aligned and the connection is trivial. This is the appropriate limit for comparison with standard transformers, which lack explicit gauge structure, but it does not probe the regime where the framework is most distinctive. The multi-model validation (Section~\ref{sec:multi_model}) extends the results to five architectures (BERT-base, BERT-large, DistilBERT, RoBERTa, ALBERT), all sharing $d_{\text{head}} = 64$ and English-language pretraining. The temperature dispersion diagnostic (Section~\ref{sec:temp_dispersion}) resolves the principal anomaly in the multi-model comparison --- RoBERTa's lower $\alpha$--$\beta$ correlation --- by identifying per-head temperature heterogeneity as the dominant explanatory variable, a finding that is itself predicted by the framework's per-head $\kappa_a$ structure. Extending to autoregressive architectures (GPT, LLaMA), non-64 head dimensions, larger model scales, and non-English languages remains future work.
...
The gauge VFE language model uses 81.4M parameters yet achieves worse perplexity than a parameter-matched standard transformer (PPL~71.6 vs.\ 48.5 at 84.2M). However, the embedding-matched baselines (Table~\ref{tab:glk_results}) reveal that comparing at matched embedding dimension tells a different story: at $d_{\text{model}} = 90$, a standard transformer achieves only PPL~118.6 (4.6M params), and the gauge VFE outperforms it by $1.66\times$. The parameter overhead arises from the per-token covariance matrices $\Sigma_i$ and gauge frame parameters $\phi_i$, which encode uncertainty and geometric structure that standard transformers absorb into larger shared embedding and weight matrices. Whether architectural modifications (parameter sharing across tokens, low-rank covariance parameterizations) can close this efficiency gap while preserving the gauge-theoretic structure is an open question. Preliminary results on WikiText-ja (Japanese) confirm that the gauge VFE framework generalizes beyond English; full cross-lingual validation remains future work.

The gauge VFE is computationally more expensive than standard attention, primarily due to the matrix exponential $\exp(\phi_i \cdot G)$ required for transport operators and the need for per-pair KL divergences rather than batched dot products. We emphasize that this is an explanatory theory: the goal is not to produce state-of-the-art language models but to provide a mathematical foundation for understanding why attention-based architectures work. Just as statistical mechanics explains thermodynamics without replacing steam engines, the gauge-theoretic framework explains transformers without replacing them. The analogy extends further: statistical mechanics also predicted new phenomena (Brownian motion, the fluctuation-dissipation theorem) that were subsequently confirmed. The temperature prediction $\tau \approx 2\sqrt{d_k}$ (in the squared-distance form) is a genuine analogue: the $\sqrt{d_k}$ arises from concentration of measure and the factor of $2$ from the $\frac{1}{2}$ prefactor in the Gaussian KL divergence, yielding a zero-free-parameter prediction that matches the empirical optimum to within 19\%. The key-norm bias prediction (that residual key-norm variance introduces a systematic, temperature-dependent correction to attention weights) is similarly derived from the theory rather than fitted, though its modest effect size ($\bar{\rho} = 0.256$) leaves room for competing explanations of the same phenomenon.
...
\subsubsection{RG Universality: Testable Predictions.}

Conjecture~\ref{conj:rg_universality} generates several falsifiable predictions, organized by computational requirements:

\paragraph{Workstation-scale experiments ($<50$ GPU-hours).}
\begin{enumerate}
\item \emph{Sample efficiency grows with $K$.} Train gauge VFE and standard transformers at matched parameter counts for $K \in \{8, 16, 32, 64\}$ on WikiText-103. The ratio $R(K) = D_{\mathrm{TF}}(K) / D_{\mathrm{VFE}}(K)$ of tokens needed to reach a target perplexity should grow as $R(K) \propto \sqrt{K}$. The null hypothesis is $R(K) = \mathrm{const}$.
\item \emph{Same scaling exponent.} Both architectures should exhibit the same Chinchilla-style exponent $\beta$ in $\mathrm{PPL}(D) \sim D^{-\beta}$, differing only in the prefactor. The null hypothesis is $\beta_{\mathrm{VFE}} \neq \beta_{\mathrm{TF}}$ at $p < 0.05$.
\item \emph{Coarse-graining exponents.} Extract attention matrices and beliefs from trained checkpoints, apply spectral coarse-graining, and measure how $g_1$, $g_2$, $g_3$ scale with cluster size $n$. The predicted exponents ($-\tfrac{1}{2}$, $-1$, $-2$) can be tested against the null of equal decay rates.
\item \emph{Emergent anisotropy.} Decompose meta-agent covariances into $\sigma^2 I + \mathrm{Var}_A(\mu)$ and verify that the within-cluster variance is anisotropic (condition number $> 1 + \epsilon$) even when the original token covariances are isotropic.
\item \emph{LayerNorm as isotropy projector.} Measure the anisotropy (condition number of the representation covariance matrix) before and after LayerNorm at each layer in a pre-trained transformer. The prediction is that LayerNorm systematically reduces anisotropy, with the effect size increasing at deeper layers.
\end{enumerate}

\paragraph{At-scale predictions ($>1000$ GPU-hours).}
\begin{enumerate}
\item \emph{Compute crossover.} At large scale, the transformer should match or exceed gauge VFE performance at $C > C^* = O(K^2 V)$ FLOPs, with the gauge VFE dominating for $C < C^*$.
\item \emph{Emergent gauge structure.} As model scale increases, the learned $W_Q W_K^\top$ in standard transformers should increasingly approximate SPD structure (all eigenvalues positive), converging toward the gauge VFE's manifold constraint.
...
\end{tabular}
\end{center}
The ratio of averaged to predicted norms at each $n$ remains within $1\%$ of unity across all couplings and all coarse-graining levels, confirming that the CLT basis of the scaling argument~\eqref{eq:rg_scaling} is exact in the large-$K$ regime.

\paragraph{Graph-based RG flow.}
We construct a synthetic gauge VFE system with initial couplings $g_1 = 0.3$, $g_2 \approx 0.20$, $g_3 \approx 0.03$ and apply spectral coarse-graining on the attention graph. The fit excludes level~$0$ (the microscopic initial condition with constructed exact coupling values) and uses $\zeta = \log(N_1 / N_\ell)$ as the RG scale, where $N_1 = 64$ is the first coarse-grained level. Table~\ref{tab:rg_flow} reports the full flow.

\begin{table}[t]
\centering
\caption{RG flow of gauge couplings under binary spectral coarse-graining ($K = 90$, $N = 128$). The original-channel anisotropy $g_1^{(\mathrm{orig})}$ tracks the inherited traceless perturbation; the total anisotropy $g_1^{(\mathrm{tot})}$ includes the emergent within-cluster variance $\mathrm{Var}_A(\mu)$. Level~$0$ is the microscopic initial condition and is excluded from exponent fits.}
\label{tab:rg_flow}
...
The graph-based exponent for the original-channel anisotropy is $y_1^{(\mathrm{orig})} = -0.499$ (predicted $-0.500$), confirming that the inherited traceless perturbation decays as $1/\sqrt{n}$ even through spectral clustering. The total anisotropy $g_1^{(\mathrm{tot})}$ does not decay ($y_1^{(\mathrm{tot})} \approx +0.01$), as predicted by Conjecture~\ref{conj:rg_universality}(iv): the emergent within-cluster variance $\mathrm{Var}_A(\mu)$ dominates at every coarse-grained level, producing $O(1)$ anisotropy even though the microscopic perturbation vanishes. This confirms that coarse-graining generates anisotropy as an irreducible feature of the RG flow, providing the geometric mechanism that LayerNorm must counteract.

The gauge variation and holonomy exponents on the attention graph ($y_2 = -0.66$, $y_3 = +0.17$) deviate from the CLT predictions, which we attribute to finite-size effects inherent in spectral clustering: at $N = 128$ with binary coarse-graining, the deepest levels contain only $4$--$8$ meta-agents, and the clustering assignments introduce correlations absent from the pure-averaging CLT analysis. The sign and magnitude of these deviations are consistent with the fact that spectral clustering on a finite attention graph does not produce independent, equal-size clusters. The CLT validation confirms that the mathematical content of the scaling predictions is exact; the graph-based deviations quantify the finite-size corrections that would vanish in the $N \to \infty$ limit.
...
\textbf{Note on appendix structure.} Standard differential geometry, covariance dynamics, gauge frame gradient derivations, and numerical method details are provided in the Supplementary Material (Appendices~A--D). The appendices below present results that are more central to the argument: Appendix~A establishes the conditional uniqueness of the forward KL divergence, and Appendix~B details the symmetry breaking simulations.
...
Assume the minimizer has the geometric mean form \eqref{eq:geometric_mean_target} for all priors $p_i$ and neighbor beliefs $\{q_j\}$.  
