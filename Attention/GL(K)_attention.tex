...
\node[below, black!75, font=\small\bfseries] at (4.3,-1.4) {Base manifold $\mathcal{C}$};

% =====================================================================
% FIBER ANCHOR POINTS — five points along the base surface
% =====================================================================
\coordinate (C1) at (0.6,-0.15);
\coordinate (C2) at (2.3,-0.25);
\coordinate (C3) at (4.2,-0.05);
\coordinate (C4) at (6.2,0.15);
\coordinate (C5) at (8.0,0.25);
...
\node[above=2pt, fiberblue!60!black, font=\footnotesize]
  at ($(C5)+(0,\fh)$) {$\mathcal{B}_q \cong \mathbb{R}^K$};

% =====================================================================
% SECTION CURVES — agents i and j as smooth sections of the bundle
%
% Each agent is a section: a map c ↦ q_i(c) assigning a Gaussian
% belief in the fiber over EACH point c. The section curve threads
% through all fibers, taking a value in each one.
% =====================================================================

% --- Section q_i (blue, solid) ---
\coordinate (Si1) at ($(C1)+(0,1.6)$);
\coordinate (Si2) at ($(C2)+(0,2.0)$);
\coordinate (Si3) at ($(C3)+(0,2.6)$);
\coordinate (Si4) at ($(C4)+(0,2.2)$);
\coordinate (Si5) at ($(C5)+(0,1.8)$);

\draw[sectionblue, line width=1.1pt]
  (Si1) .. controls ($(Si2)+(-0.4,0.15)$) and ($(Si2)+(0.4,0.1)$) ..
  (Si2) .. controls ($(Si3)+(-0.6,-0.1)$) and ($(Si3)+(-0.2,0.05)$) ..
  (Si3) .. controls ($(Si3)+(0.5,0)$) and ($(Si4)+(-0.4,0.15)$) ..
  (Si4) .. controls ($(Si4)+(0.5,-0.1)$) and ($(Si5)+(-0.4,0.1)$) ..
  (Si5);

% --- Section q_j (red, dashed) ---
\coordinate (Sj1) at ($(C1)+(0,3.4)$);
\coordinate (Sj2) at ($(C2)+(0,3.2)$);
\coordinate (Sj3) at ($(C3)+(0,3.7)$);
\coordinate (Sj4) at ($(C4)+(0,4.0)$);
\coordinate (Sj5) at ($(C5)+(0,3.7)$);

\draw[sectionred, densely dashed, line width=1.1pt]
  (Sj1) .. controls ($(Sj2)+(-0.3,-0.05)$) and ($(Sj2)+(0.3,-0.1)$) ..
  (Sj2) .. controls ($(Sj3)+(-0.6,0.0)$) and ($(Sj3)+(-0.2,0.1)$) ..
  (Sj3) .. controls ($(Sj3)+(0.5,0.05)$) and ($(Sj4)+(-0.5,-0.1)$) ..
  (Sj4) .. controls ($(Sj4)+(0.5,-0.05)$) and ($(Sj5)+(-0.4,0.1)$) ..
  (Sj5);

% Section labels (left side, well separated)
\node[sectionblue, font=\footnotesize, left=4pt] at (Si1) {$q_i(c)$};
\node[sectionred, font=\footnotesize, left=4pt] at (Sj1) {$q_j(c)$};

% =====================================================================
% HIGHLIGHT FIBER AT c* — the fourth fiber (C4)
%
% At this single point c*, BOTH sections q_i(c*) and q_j(c*) live
% in the SAME fiber. The transport Ω_ij(c*) acts WITHIN this fiber
% to align j's frame to i's frame.
% =====================================================================
...
\begin{scope}[shift={(Sj4)}, rotate=-15]
  ...
  \draw[-{Stealth[length=1.8mm]}, framecol, line width=0.7pt]
    (0,0) -- (0,0.55);
\end{scope}
\node[framecol!80!black, font=\scriptsize] at ($(Sj4)+(0.7,0.1)$) {$\phi_j$};
...
over sampled points.
Agents $i$ and $j$ define smooth sections
$c \mapsto q_i(c)$ and $c \mapsto q_j(c)$
(solid blue, dashed red),
each assigning a Gaussian belief in the fiber over every base point.
At the highlighted fiber over $c^*$,
both $q_i(c^*)$ and $q_j(c^*)$ reside in the \emph{same} fiber
...
is then the divergence between same-frame distributions
within a single fiber.
For transformers ($\dim\mathcal{C} = 0$),
all tokens share one base point and the entire
computation occurs within one fiber.
}
