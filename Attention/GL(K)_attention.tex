...
First, standard transformers utilize attention sub-layers with feedforward networks (FFNs). These are typically two-layer MLPs with a non-linear activation mechanism such as GeLU \citep{hendrycks2016gaussian}. Within the gauge-theoretic framework, the FFN corresponds to the iterative VFE dynamics that adjust beliefs between attention computations. We can show that the non-linear activation function is not an architectural choice but instead a consequence of the variational principle itself: the VFE gradient derives a Gated Linear Unit (GLU) \citep{dauphin2017language}.
...
\end{equation}
%
which is a Gated Linear Unit: a linear function of the input gated by a Boltzmann distribution over KL energies computed from the same input. This is therefore derived rather than designed.
...
\end{equation}
%
This has the SiLU/Swish \citep{ramachandran2018searching} structure $f(x) = x \cdot \sigma(g(x))$, with the gate argument determined by the relative KL energies.  The sigmoid is thus not the general case but simply the $N = 2$ limit of the Boltzmann gate. For general $N$, each message is gated by its Boltzmann weight whereby the sigmoid is recovered only when two neighbors compete.

\paragraph{Boltzmann gate, Gaussian family, and connection to GELU.}

The Boltzmann gate therefore has a specific functional form determined by the energy landscape. In the isotropic limit $\Sigma_j = \sigma^2 I$, the un-normalized gate for neighbor $j$ is $\exp(-E_{ij}/\tau) = \exp(-\|e_{ij}\|^2 / 2\sigma^2\tau)$. This is a Gaussian function of the prediction error. The Boltzmann distribution over KL energies is therefore a \emph{Gaussian mixture gate}: each neighbor's weight is a Gaussian in the prediction-error space, normalized by the partition function $Z_i = \sum_k \exp(-\|e_{ik}\|^2/2\sigma^2\tau)$. Compare this with the standard activation functions, all of which are Gated Linear Units $f(x) = x \cdot g(x)$:
%
\begin{align}
