...
$e \cdot \mathrm{softmax}(-\|e\|^2/\tau)$ (Eq.~\ref{eq:vfe_glu}) & Derived GLU gate & GELU/SiLU activation & D \\
...
\end{equation}
%
The message $e_{ij} \equiv \mu_i - \Omega_{ij}\mu_j$ is \emph{linear} in the beliefs. But the attention gate $\beta_{ij} = \operatorname{softmax}_j\!\bigl(-D_{\mathrm{KL}}(q_i \| \Omega_{ij}q_j)/\tau\bigr)$ is a \emph{sigmoid function} of the message magnitude, since for Gaussian beliefs:
%
\begin{equation}
...
%
\begin{equation}
\boxed{\Delta\mu_i\big|_j \;\propto\; \underbrace{e_{ij}}_{\text{linear}} \;\cdot\; \underbrace{\operatorname{softmax}_j\!\bigl(-\tfrac{1}{2\tau}\|e_{ij}\|^2_{\Sigma_j^{-1}} + \cdots\bigr)}_{\text{sigmoid gate}},}
\label{eq:vfe_glu}
\end{equation}
%
which is a \emph{Gated Linear Unit}: a linear function of the input gated by a sigmoid applied to a function of the same input. This is derived, not assumed.

\paragraph{Binary case: exact sigmoid gate.}
For $N = 2$ neighbors, the softmax reduces to the logistic sigmoid $\sigma$:
%
\begin{equation}
...
\end{equation}
%
This is precisely the SiLU/Swish \citep{ramachandran2018searching} structure $f(x) = x \cdot \sigma(g(x))$, with the gate argument determined by the relative KL divergences.  In the general $N$-way case, each message is gated by the corresponding softmax component --- the multi-class generalization of the sigmoid gate.

\paragraph{Gaussian family and connection to GELU.}
In the isotropic limit $\Sigma_j = \sigma^2 I$, the gate for the dominant-neighbor contribution takes the form $\beta_{ij} \propto \exp(-\|e_{ij}\|^2 / 2\sigma^2\tau)$. The gate function is therefore in the \emph{Gaussian family}: it depends on $\exp(-\text{quadratic})$. Compare this with the standard activations:
%
\begin{align}
\text{SiLU}(x) &= x \cdot \sigma(x) && \text{logistic sigmoid gate, linear argument}, \notag \\
\text{GELU}(x) &= x \cdot \Phi(x) && \text{Gaussian CDF gate, linear argument}, \label{eq:activation_comparison} \\
\text{VFE}(e) &= e \cdot \operatorname{softmax}\!\bigl(-\|e\|^2/\tau'\bigr) && \text{softmax gate, quadratic argument}. \notag
\end{align}
%
All three are instances of a \emph{Gated Linear Unit} $f(x) = x \cdot g(x)$ where $g$ belongs to the sigmoid/Gaussian family. The VFE derives this structure from first principles; the specific gate functions differ in whether the argument is linear (SiLU, GELU) or quadratic (VFE) in the gated quantity. However, the Gaussian CDF $\Phi(x) = \frac{1}{2}\operatorname{erfc}(-x/\sqrt{2})$ and the Gaussian PDF $\propto \exp(-x^2)$ appearing in the VFE gate are directly related ($\Phi' \propto \exp(-x^2/2)$), placing the VFE-derived activation in the same functional family as GELU. ReLU is the hard-gate ($\tau \to 0$) limit of any sigmoid gate: when the gate becomes a step function, $x \cdot \Theta(x) = \max(0, x)$.
