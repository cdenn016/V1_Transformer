...
This parallels the mean gradient: $\alpha_i\,\Sigma_{p,i}^{-1}$ is the prior precision scaled by the adaptive coupling (analogous to weight decay for $\mu$), $\sum_j \beta_{ij}(\Omega_{ij}\Sigma_j\Omega_{ij}^\top)^{-1}$ is attention-weighted precision aggregation that pulls agent $i$'s precision toward its gauge-transported neighbors, and the final term $\\$ $\sum_j (\partial \beta_{ij}/\partial \Sigma_i)\, D_{\mathrm{KL}}(q_i \,\|\, \Omega_{ij} q_j)$ is gradient flow through the attention weights (the covariance analogue of the non-linear activation term in the $\mu$ gradient). Note that no $(\partial\alpha_i/\partial\Sigma_i)\,D_{\mathrm{KL}}(q_i\,\|\,p_i)$ gate gradient term appears: in the deterministic limit (Eq.~\ref{eq:alpha_i_deterministic}), $\alpha_i$ depends only on $\mu_i$, so $\partial\alpha_i/\partial\Sigma_i = 0$. The coefficient $-(1+\alpha_i)$ on $\Sigma_i^{-1}$ arises because the entropy of $q_i$ enters both the prior KL (weighted by $\alpha_i$) and each neighbor KL (weighted by $\beta_{ij}$, summing to $1$).
