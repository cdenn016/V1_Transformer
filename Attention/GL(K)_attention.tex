...
We analyze the learned belief embeddings $\mu \in \mathbb{R}^{90}$ and gauge frames $\phi \in \mathfrak{gl}(10) \cong \mathbb{R}^{900}$ for emergent categorical structure, using five linguistic categories (content word, digit, function word, letter, punctuation) across 500 representative tokens. Clustering metrics are computed in the full-dimensional spaces at every 5,000 training steps.
...
\begin{figure*}[t]
  ...
  \caption{Belief magnitudes $\|\mu_Q^{\text{center}}\|$ without observations. All agents converge to identical norms (gauge-symmetric equilibrium).
  (\textbf{Left}) Population mean, standard deviation, and range.
...
\begin{figure*}[t]
  ...
  \caption{Belief magnitudes $\|\mu_Q^{\text{center}}\|$ with observations. Agents specialize to distinct norms under symmetry breaking.
  (\textbf{Left}) Population mean, standard deviation, and range.
  (\textbf{Right}) Top four and bottom two agents by $\|\mu_Q(t)\|$.}
