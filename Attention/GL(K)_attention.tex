...
\begin{align}
g_1 &= \|\Sigma_i - \sigma^2 I\| / \sigma^2 & &\text{(anisotropy)}, \label{eq:g1_def} \\
g_2 &= \|\Omega_{ij} - \Omega\| / \|\Omega\| & &\text{(gauge variation)}, \label{eq:g2_def} \\
g_3 &= \|H_{ijk} - I\| & &\text{(holonomy)}, \label{eq:g3_def}
\end{align}
where $H_{ijk} = \Omega_{ij}\,\Omega_{jk}\,\Omega_{ki}$.  The transformer limit corresponds to $g_1^* = g_2^* = g_3^* = 0$.
...
\label{eq:rg_scaling}
\end{equation}
with all scaling dimensions negative ($y_1 = -\tfrac{1}{2}$, $y_2 = -1$, $y_3 = -2$), classifying every operator as \emph{irrelevant}.

\begin{conjecture}[RG universality of the transformer limit]
\label{conj:rg_universality}
Let $\mathcal{T}(K, N, g_1, g_2, g_3)$ denote the family of gauge VFE models. Then:
\begin{enumerate}
...
\item[(iv)] \textbf{Emergent anisotropy.} $R_n$ generates anisotropy via $\mathrm{Var}_A(\mu)$ even at $g_1 = 0$. Transformers absorb this into $W_Q, W_K$; the gauge VFE tracks it in $\Sigma_i$.
...
The scaling dimensions in Eq.~\eqref{eq:rg_scaling} follow from CLT averaging of independent perturbations---a mathematical fact that does not itself require the gauge framework. The non-trivial content of the conjecture is the \emph{hypothesis} that this coarse-graining structure applies to the learned representations of trained transformers, where the independence assumption may not hold. We have verified that the CLT exponents are exact (to three significant figures) under direct averaging of synthetic i.i.d.\ perturbations, confirming the mathematical basis. However, when the same predictions are tested on attention-graph coarse-graining with spectral clustering (which introduces finite-size correlations), the measured exponents deviate: $y_2 \approx -0.6$ and $y_3 \approx +0.2$ versus predicted $-1$ and $-2$. These deviations arise from clustering artifacts (unequal cluster sizes, correlated assignments) rather than a failure of the underlying scaling argument, but they underscore that \emph{validation on trained models is needed before the conjecture can be considered empirically supported}. We defer detailed numerical analysis of the RG flow to a companion paper where it can be developed with trained-model evidence.

The ordering of the three limits in Section~\ref{sec:transformer_limit} is dictated by the RG hierarchy: holonomy vanishes fastest ($y_3 = -2$), then gauge variation ($y_2 = -1$), then anisotropy ($y_1 = -\tfrac{1}{2}$), matching Limit~3 $\to$ Limit~2 $\to$ Limit~1. The emergent anisotropy from part~(iv) provides a geometric interpretation of LayerNorm as a projection back toward the isotropic submanifold at each layer.
