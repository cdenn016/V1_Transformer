...
% =====================================================================
% BASE MANIFOLD — oblique parallelogram with gentle curvature
% =====================================================================
\path
  (0,0) coordinate (M1)
  (8.5,0.3) coordinate (M2)
  (8.0,-1.2) coordinate (M3)
  (-0.5,-1.0) coordinate (M4);

% Shadow
\filldraw[fill=black!8, draw=none]
  ($(M1)+(0.12,-0.12)$)
    .. controls (2.5,-0.35) and (5.5,0.45) .. ($(M2)+(0.12,-0.12)$)
    -- ($(M3)+(0.12,-0.12)$)
    .. controls (5.0,-1.35) and (1.5,-1.25) .. ($(M4)+(0.12,-0.12)$)
    -- cycle;

% Surface fill
\shade[top color=basetan!80, bottom color=basetan!30, draw=baseedge, thick]
  (M1)
    .. controls (2.5,-0.3) and (5.5,0.5) .. (M2)
    -- (M3)
    .. controls (5.0,-1.3) and (1.5,-1.2) .. (M4)
    -- cycle;

% Front edge highlight
\draw[baseedge!80, thick]
  (M1) .. controls (2.5,-0.3) and (5.5,0.5) .. (M2);

% Grid lines for 3D depth cue (subtle)
\foreach \t in {0.25, 0.5, 0.75} {
  \draw[baseedge!20, very thin]
    ($(M1)!\t!(M4)$) .. controls
    ($($(M1)!\t!(M4)$)!0.35!($(M2)!\t!(M3)$)+(0,0.05)$) and
    ($($(M1)!\t!(M4)$)!0.65!($(M2)!\t!(M3)$)+(0,-0.05)$)
    .. ($(M2)!\t!(M3)$);
}

\node[below, black!80, font=\small\bfseries] at (4.0,-1.65) {Base manifold $\mathcal{C}$};

% =====================================================================
% THREE AGENT POSITIONS — tokens i, j, k on the base
% =====================================================================
% Position agents along the base with slight vertical offsets for perspective
\coordinate (Ai) at (1.2,-0.35);   % Agent i (left)
\coordinate (Aj) at (4.2,-0.15);   % Agent j (center)
\coordinate (Ak) at (6.8,0.05);    % Agent k (right)

% Agent base markers — filled circles with halos
\foreach \agent/\lbl in {Ai/$i$, Aj/$j$, Ak/$k$} {
  \filldraw[black!25] (\agent) circle (2.5pt);
  \filldraw[black!80] (\agent) circle (1.8pt);
}
% Token labels on base
\node[below=4pt, font=\footnotesize\itshape] at (Ai) {token $i$};
\node[below=4pt, font=\footnotesize\itshape] at (Aj) {token $j$};
\node[below=4pt, font=\footnotesize\itshape] at (Ak) {token $k$};

% =====================================================================
% FIBERS — vertical lines from each agent position
% =====================================================================
\newcommand{\fiberheight}{4.2}

\foreach \agent in {Ai, Aj, Ak} {
  \draw[fiberblue!40, densely dashed, line width=0.5pt]
    (\agent) -- ++(0,\fiberheight);
}

% Fiber label
\node[above=2pt, fiberblue!70!black, font=\footnotesize]
  at ($(Ak)+(0,\fiberheight)$) {Fiber $\mathcal{B}_q \cong \mathbb{R}^K$};

% =====================================================================
% LOCAL GAUGE FRAMES — rotated coordinate axes at each fiber
% These show each agent's local reference frame φ_i
% =====================================================================
\newcommand{\framesize}{0.55}

% Frame for agent i (tilted ~25°)
\begin{scope}[shift={($(Ai)+(0,1.6)$)}, rotate=25]
  \draw[-{Stealth[length=2mm]}, framecol!80, line width=0.8pt] (0,0) -- (\framesize,0);
  \draw[-{Stealth[length=2mm]}, framecol!80, line width=0.8pt] (0,0) -- (0,\framesize);
  \node[framecol!80, font=\tiny, right] at (\framesize,0) {$e_1^{(i)}$};
  \node[framecol!80, font=\tiny, above] at (0,\framesize) {$e_2^{(i)}$};
\end{scope}

% Frame for agent j (tilted ~-10°)
\begin{scope}[shift={($(Aj)+(0,2.0)$)}, rotate=-10]
  \draw[-{Stealth[length=2mm]}, framecol!80, line width=0.8pt] (0,0) -- (\framesize,0);
  \draw[-{Stealth[length=2mm]}, framecol!80, line width=0.8pt] (0,0) -- (0,\framesize);
  \node[framecol!80, font=\tiny, right] at (\framesize,0) {$e_1^{(j)}$};
  \node[framecol!80, font=\tiny, above] at (0,\framesize) {$e_2^{(j)}$};
\end{scope}

% Frame for agent k (tilted ~40°)
\begin{scope}[shift={($(Ak)+(0,2.4)$)}, rotate=40]
  \draw[-{Stealth[length=2mm]}, framecol!80, line width=0.8pt] (0,0) -- (\framesize,0);
  \draw[-{Stealth[length=2mm]}, framecol!80, line width=0.8pt] (0,0) -- (0,\framesize);
  \node[framecol!80, font=\tiny, right] at (\framesize,0) {$e_1^{(k)}$};
  \node[framecol!80, font=\tiny, above] at (0,\framesize) {$e_2^{(k)}$};
\end{scope}

% Frame label
\node[framecol!70!black, font=\tiny, fill=white, fill opacity=0.85, text opacity=1,
      inner sep=1.5pt, rounded corners=1pt]
  at ($(Ai)+(-0.9,2.3)$) {$\phi_i$};
\node[framecol!70!black, font=\tiny, fill=white, fill opacity=0.85, text opacity=1,
      inner sep=1.5pt, rounded corners=1pt]
  at ($(Aj)+(-0.85,2.6)$) {$\phi_j$};
\node[framecol!70!black, font=\tiny, fill=white, fill opacity=0.85, text opacity=1,
      inner sep=1.5pt, rounded corners=1pt]
  at ($(Ak)+(-0.9,3.0)$) {$\phi_k$};

% =====================================================================
% GAUSSIAN BELIEF DISTRIBUTIONS — ellipses on fibers
% Each agent's belief q_i is a Gaussian shown as a tilted ellipse
% =====================================================================

% Agent i: belief q_i — blue ellipse, tilted to match frame
\begin{scope}[shift={($(Ai)+(0,2.7)$)}, rotate=25]
  \filldraw[gaussblue, opacity=0.15] (0,0) ellipse (0.35 and 0.18);
  \draw[sectionblue, line width=0.9pt] (0,0) ellipse (0.35 and 0.18);
  \filldraw[sectionblue] (0,0) circle (1.5pt);
\end{scope}
\node[sectionblue, font=\footnotesize, fill=white, fill opacity=0.85, text opacity=1,
      inner sep=2pt, rounded corners=1pt]
  at ($(Ai)+(-1.0,2.7)$) {$q_i$};

% Agent j: belief q_j — red ellipse, tilted to match frame
\begin{scope}[shift={($(Aj)+(0,3.1)$)}, rotate=-10]
  \filldraw[gaussred, opacity=0.15] (0,0) ellipse (0.4 and 0.15);
  \draw[sectionred, line width=0.9pt] (0,0) ellipse (0.4 and 0.15);
  \filldraw[sectionred] (0,0) circle (1.5pt);
\end{scope}
\node[sectionred, font=\footnotesize, fill=white, fill opacity=0.85, text opacity=1,
      inner sep=2pt, rounded corners=1pt]
  at ($(Aj)+(-1.0,3.1)$) {$q_j$};

% Agent k: belief q_k — teal ellipse
\begin{scope}[shift={($(Ak)+(0,3.4)$)}, rotate=40]
  \filldraw[teal, opacity=0.12] (0,0) ellipse (0.3 and 0.22);
  \draw[teal!80!black, line width=0.9pt] (0,0) ellipse (0.3 and 0.22);
  \filldraw[teal!80!black] (0,0) circle (1.5pt);
\end{scope}
\node[teal!80!black, font=\footnotesize, fill=white, fill opacity=0.85, text opacity=1,
      inner sep=2pt, rounded corners=1pt]
  at ($(Ak)+(1.05,3.4)$) {$q_k$};

% =====================================================================
% SECTION CURVES — connecting beliefs across fibers
% =====================================================================

% Section q_i (blue, solid)
\draw[sectionblue, line width=1.0pt]
  ($(Ai)+(0,2.7)$)
    .. controls ($(Aj)+(-0.8,2.85)$) and ($(Aj)+(0.8,3.2)$)
    .. ($(Ak)+(0,3.15)$);

% Section q_j (red, dashed)
\draw[sectionred, densely dashed, line width=1.0pt]
  ($(Ai)+(0,1.8)$)
    .. controls ($(Aj)+(-0.8,2.6)$) and ($(Aj)+(0.8,3.0)$)
    .. ($(Ak)+(0,3.55)$);

% Tiny section labels at far ends
\node[sectionblue, font=\tiny, right] at ($(Ak)+(0.15,3.15)$) {section $q_i$};
\node[sectionred, font=\tiny, right] at ($(Ak)+(0.15,3.55)$) {section $q_j$};

% =====================================================================
% GAUGE TRANSPORT Ω_ij — the key operation: transport j's belief to i's frame
% =====================================================================

% Ghost ellipse: q_j transported into agent i's frame (dashed red, at i's fiber)
\begin{scope}[shift={($(Ai)+(0,2.2)$)}, rotate=25]
  \filldraw[gaussred, opacity=0.08] (0,0) ellipse (0.4 and 0.15);
  \draw[sectionred, densely dotted, line width=0.8pt] (0,0) ellipse (0.4 and 0.15);
  \filldraw[sectionred, opacity=0.5] (0,0) circle (1.2pt);
\end{scope}
\node[sectionred!80, font=\tiny, fill=white, fill opacity=0.85, text opacity=1,
      inner sep=1.5pt, rounded corners=1pt]
  at ($(Ai)+(-1.25,2.2)$) {$\Omega_{ij}\,q_j$};

% Transport arrow from q_j at j to the transported ghost at i
\draw[-{Stealth[length=2.5mm, width=1.8mm]}, transportgreen!90!black,
      line width=1.2pt,
      decorate, decoration={snake, amplitude=0.5mm, segment length=3.5mm, post length=2mm}]
  ($(Aj)+(0,3.1)$)
    .. controls ($(Aj)+(-1.2,2.6)$) and ($(Ai)+(1.0,2.0)$)
    .. ($(Ai)+(0.05,2.2)$);

% Transport label
\node[transportgreen!85!black, font=\footnotesize\bfseries,
      fill=white, fill opacity=0.9, text opacity=1,
      inner sep=3pt, rounded corners=2pt, draw=transportgreen!40]
  at ($(Ai)!0.5!(Aj)+(0,2.15)$) {$\Omega_{ij} = e^{\phi_i}e^{-\phi_j}$};

% =====================================================================
% KL DIVERGENCE BRACE — shows the attention cost between q_i and Ω_ij q_j
% =====================================================================

% Vertical double-arrow between q_i and transported q_j at agent i's fiber
\draw[{Stealth[length=1.8mm]}-{Stealth[length=1.8mm]},
      black!60, line width=0.7pt]
  ($(Ai)+(0.65,2.25)$) -- ($(Ai)+(0.65,2.65)$);

\node[black!70, font=\tiny, fill=white, fill opacity=0.9, text opacity=1,
      inner sep=1.5pt, rounded corners=1pt, right]
  at ($(Ai)+(0.7,2.45)$) {$D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j)$};

% =====================================================================
% ATTENTION WEIGHT annotation — β_ij as softmax of KL costs
% =====================================================================
\node[black!80, font=\footnotesize,
      fill=yellow!8, draw=black!25,
      inner sep=4pt, rounded corners=3pt,
      text width=4.5cm, align=center]
  at (4.0,-2.4)
  {$\beta_{ij} = \softmax\!\big(-D_{\KL}(q_i \| \Omega_{ij} q_j)\,/\,\kappa\big)$};

\end{tikzpicture}
\caption{%
The gauge-theoretic attention mechanism on an associated bundle.
Three agents (tokens $i, j, k$) occupy positions on the base manifold $\mathcal{C}$,
each with a vertical fiber $\mathcal{B}_q \cong \mathbb{R}^K$.
Agent beliefs $q_i, q_j, q_k$ are Gaussian distributions (ellipses, with orientation
reflecting covariance $\Sigma$) living in local gauge frames $\phi_i, \phi_j, \phi_k$
(purple axes, each rotated differently).
The gauge transport operator $\Omega_{ij} = \exp(\phi_i)\exp(-\phi_j)$
(green arrow) carries $q_j$ from agent $j$'s frame into agent $i$'s frame,
producing the transported belief $\Omega_{ij} q_j$ (dotted red ellipse).
The attention cost is the KL divergence $D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j)$
between the agent's own belief and the frame-aligned neighbor signal.
Attention weights $\beta_{ij}$ arise as softmax-normalized costs over all neighbors.
In the flat-bundle limit ($\Omega_{ij} \to I$, all frames aligned),
this reduces to standard dot-product attention.
}
...
\FOR{$t = 1$ \TO $T_{\text{inner}}$}
    ...
    \FOR{each head $h$ (irrep block)}
        \STATE $\beta_{ij}^{(h)} \leftarrow \softmax\!\Big(-D_{\KL}(q_i^{(h)} \,\|\, \Omega_{ij}^{(h)} q_j^{(h)}) \,/\, \kappa_h\Big)$ \hfill $\triangleright$ KL attention
        \STATE $\nabla_{\mu} \mathcal{F} \leftarrow \alpha\,\Sigma_p^{-1}(\mu_i - \mu_p) + \lambda \sum_j \beta_{ij}\, \nabla_\mu D_{\KL}(q_i \,\|\, \Omega_{ij} q_j) + \nabla_\mu \mathcal{L}_{\text{obs}}$
    \ENDFOR
    ...
    \STATE (Optional) $\Sigma_i \leftarrow \mathrm{Retract}_{\mathrm{SPD}}(\Sigma_i,\, -\eta_\Sigma\, \tilde{\nabla}_\Sigma)$ \hfill $\triangleright$ SPD manifold retraction
    \STATE (Optional) $\phi_i \leftarrow \mathrm{Retract}_{\mathfrak{g}}(\phi_i,\, -\eta_\phi\, \nabla_\phi \mathcal{F})$ \hfill $\triangleright$ Lie algebra retraction
\ENDFOR
\STATE
\STATE \textit{--- M-step: Parameter learning (backpropagation) ---}
\STATE Compute loss: $\mathcal{L} = \mathcal{L}_{\text{CE}}(\mu_i^*, W_{\text{out}}) + \alpha\sum_i D_{\KL}(q_i^* \,\|\, p_i) + \frac{\alpha_\phi}{2}\|\phi\|^2$
