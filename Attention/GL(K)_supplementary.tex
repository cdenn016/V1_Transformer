...
Here we derive the gradient of the single-agent free energy $\mathcal{F}_i$ with respect to the covariance $\Sigma_i$ for agent $i$; a well known result in information geometry. For brevity we work with the \emph{entropy-suppressed surrogate} $\sum_j \beta_{ij} D_{\mathrm{KL}}(q_i \| \Omega_{ij}q_j)$ (i.e.\ holding the attention distribution $\beta$ fixed and treating the alignment energy at this $\beta$); the canonical free energy of the main text adds the $\tau\beta_{ij}\log(\beta_{ij}/\pi_{ij})$ entropy term to make the softmax form of $\beta$ a stationary point. The covariance gradient is identical under both forms because the attention entropy does not depend on $\Sigma_i$.
...
The alignment term $D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j)$ involves KL divergences between Gaussians related by gauge transport $\Omega_{ij} = e^{\phi_i}e^{-\phi_j} \in \mathrm{GL}^+(K)$. The standard Gaussian KL divergence \citep{bishop2006pattern,murphy2012machine,CoverThomas2006} is
...
The push-forward $\Omega_{ij} q_j = \mathcal{N}(\Omega_{ij}\mu_j, \Omega_{ij}\Sigma_j\Omega_{ij}^\top)$ used below is the standard linear-transformation rule for a Gaussian random variable under $\Omega \in \mathrm{GL}(K)$ \citep[\S2.3.3]{bishop2006pattern}. Applying the above derivative to each term in $\mathcal{F}_i$:
...
with equality iff $H = 0$. The Hessian depends only on $\Sigma_1$ (the $\mathrm{tr}(\Sigma_2^{-1}\Sigma_1)$ term is linear in $\Sigma_1$ and contributes no $\Sigma_2$-dependent second derivative), and is manifestly positive definite for $\Sigma_1 \succ 0$. The full Hessian of $\mathcal{F}_i$ extends to a $\beta$-weighted convex combination of single-KL Hessians at the joint $(\beta, \Sigma)$ stationary point; the envelope-theorem identity of Eq.~\eqref{eq:autograd_envelope_gap} (main paper) suppresses the $\partial\beta/\partial \Sigma$ contributions at the fixed point.

Hence the covariance alignment fixed-point is an attractor of the variational dynamics under the standing assumptions $\sum_j \beta_{ij}=1$, $\Omega_{ij}$ invertible, and the observation precision $R^{-1}$ negligible relative to the inter-agent coupling (cf.\ Eq.~\eqref{eq:Sigma_gradient_final}).
...
\paragraph{$\mathrm{GL}(K)$ updates.} For $\mathrm{GL}(K)$, a single matrix exponential $\exp: \mathfrak{gl}(K) \to \mathrm{GL}^+(K)$ is not surjective onto the identity component \citep{culver1966existence}, but the pairwise transport $\Omega_{ij} = \exp(\phi_i)\exp(-\phi_j)$ covers all of $\mathrm{GL}^+(K)$ via polar decomposition \citep[\S8]{Higham2008}: for any $A \in \mathrm{GL}^+(K)$ with polar decomposition $A = UP$ ($U \in \mathrm{SO}(K)$, $P \in \mathrm{Sym}_{++}(K)$), the exponential map restricts to a surjection $\mathfrak{so}(K) \twoheadrightarrow \mathrm{SO}(K)$ on the compact factor and a bijection $\mathrm{Sym}(K) \xrightarrow{\sim} \mathrm{Sym}_{++}(K)$ on the symmetric factor, so choosing $\phi_i$ with $\exp(\phi_i) = UP^{1/2}$ and $\phi_j$ with $\exp(-\phi_j) = P^{1/2}$ realises $\Omega_{ij} = A$. No retraction is required beyond standard gradient clipping for numerical stability. In our implementation, automatic differentiation through PyTorch's \texttt{matrix\_exp} handles the $\mathrm{GL}(K)$ differential implicitly.
...
The metric used in this paper is therefore not the Killing form itself but the Cartan-involution-modified bilinear form $g(X, Y) = -B(X, \theta(Y))$ with $\theta(X) = -X^\top$ \citep[Prop.~1.93]{Knapp2002}. Equivalently, in the generator basis:
...
= \operatorname{tr}\bigl(\Psi(\mathrm{ad}_\phi)(T_a)^\top \cdot \exp(\phi)\exp(\phi)^\top \cdot \Psi(\mathrm{ad}_\phi)(T_b)\bigr),
...
Our implementation uses natural gradient descent for the statistical parameters, which exploits the information-geometric structure of the Gaussian manifold. Rather than treating the covariance matrices $\Sigma_i$ as Euclidean variables, we recognize them as points on the manifold of symmetric positive-definite matrices, equipped with the Fisher-Rao metric \citep{Cencov1982,amari2016information}. The natural gradient at a point $\Sigma$ on this manifold is obtained by applying the inverse Fisher metric to the Euclidean gradient, yielding an intrinsic tangent vector that respects the manifold's curved geometry. Given Euclidean gradients $\nabla_\mu$ and $\nabla_\Sigma$, the natural gradient projections are
\begin{equation}
\delta\mu = -\Sigma \nabla_\mu, \qquad
...
The differential of the matrix exponential relates infinitesimal variations $\delta \phi$ to variations in the group element via the \emph{left-trivialised} form
\begin{equation}
\frac{\partial}{\partial \phi^a}\exp \Bigl(\textstyle\sum_b \phi^b T_b\Bigr) = \exp(X)\cdot Q_a^{L},
\end{equation}
where $X = \sum_b \phi^b T_b$ and $Q_a^{L} = d\exp^{L}_X(T_a)$ denotes the left-trivialised derivative of the exponential map applied to the generator $T_a$. This convention differs from the right-trivialised form $D_{\phi_i}(\exp)[T_a] = Q_a^{R,(i)} \exp(\phi_i)$ used in Eq.~\eqref{eq:dtilde_mu} of Appendix~C by an adjoint action: the two are related by $Q_a^{R} \exp(\phi) = \exp(\phi) \cdot Q_a^{L}$, equivalently $Q_a^{L} = \mathrm{Ad}_{\exp(-\phi)}(Q_a^{R})$~\citep{Hall2015}. The sign of the first-order ad-coefficient flips accordingly. For the $K = 3$ fundamental representation of $\mathfrak{so}(3)$, we use the exact closed-form expression
\begin{equation}
Q_a^{L} = T_a - c_1(\theta) \mathrm{ad}_X(T_a) + c_2(\theta) \mathrm{ad}_X^2(T_a),
\end{equation}
where $\theta = \|\phi\|$, $c_1(\theta) = (1-\cos\theta)/\theta^2$, and $c_2(\theta) = (\theta - \sin\theta)/\theta^3$, with Taylor-expanded small-angle limits for $\theta < 10^{-4}$. (The right-trivialised form used in Appendix~C carries $+c_1(\theta) \mathrm{ad}_X$ in place of $-c_1(\theta)\mathrm{ad}_X$; both formulas reproduce the numerical Fr\'{e}chet derivative to machine precision under their own conventions.) For higher-dimensional representations ($K > 3$), where the Cayley--Hamilton truncation requires $K$ terms, we instead compute $Q_a^{L}$ via the Fr\'{e}chet derivative of the matrix exponential using symmetric quadrature.

The resulting Euclidean gradient $\partial\mathcal{F}/\partial\phi^a$ can optionally be preconditioned using one of several Lie-algebraic metrics. In the transformer implementation, these include: (i)~the \emph{Cartan-involution-modified} natural gradient, which uses the bilinear form $\tilde{g}_{ab} = 2K \mathrm{tr}(T_a^\top T_b) - 2 \mathrm{tr}(T_a)\mathrm{tr}(T_b)$ (positive-definite on $\mathfrak{sl}(K)$ as derived in Appendix~\ref{sec:glk_preconditioning}) as a position-independent metric with no free parameters; (ii)~a \emph{Cartan decomposition} preconditioner that dampens the non-compact (symmetric) directions of $\mathfrak{gl}(K)$ relative to the compact $\mathfrak{so}(K)$ directions; and (iii)~the full \emph{pullback} natural gradient $\mathcal{G}_{ab}(\phi) = \operatorname{tr}\bigl(\Psi(\mathrm{ad}_\phi)(T_a)^\top \cdot \exp(\phi)\exp(\phi)^\top \cdot \Psi(\mathrm{ad}_\phi)(T_b)\bigr)$ (Eq.~\ref{eq:pullback_metric}), where $\Psi(z) = (e^z - 1)/z$ and the position-dependent factor $\exp(\phi)\exp(\phi)^\top$ captures the exponential amplification of $D\exp$ in the non-compact sector.
...
The posterior population mean for dot-product key-norm correlation is $\hat{\mu}_\alpha = -0.144$ (94\% HDI $[-0.195, -0.096]$); the head-level frequentist estimate cited in \S\ref{sec:keynorm_bias_supp} as $|\rho|_\beta = 0.256$ corresponds to a single-head summary statistic and is not directly comparable to the partially pooled population posterior, which shrinks toward the larger between-head average $|\hat\mu_\beta| = 0.475$ (HDI $[0.447, 0.507]$). For KL-distance attention, the posterior population mean is $\hat{\mu}_\beta = -0.475$. The standardized effect size is Cohen's $d = 1.43$ (94\% HDI $[1.08, 1.88]$), reported alongside the chain-mixing caveat noted above, and $P(|\mu_\beta| > |\mu_\alpha|) = 1.000$ across all posterior samples.
...
\end{equation}
where the exponents follow from the central limit theorem (intrinsic anisotropy), the law of large numbers applied to pairwise transport (gauge variation), and the linear-norm scaling of holonomy products around triangles whose sides each contract as $n^{-1/2}$. The linearised RG matrix at the intrinsic fixed point is diagonal:
\begin{equation}
...
Let $\mathcal{T}(K, N, g_1^{(\mathrm{orig})}, g_2, g_3)$ denote the family of gauge VFE models with belief dimension $K$, sequence length $N$, and intrinsic coupling constants $(g_1^{(\mathrm{orig})}, g_2, g_3)$. Let $R_n: \mathcal{T} \to \mathcal{T}$ be the coarse-graining map. Then:
\begin{enumerate}
\item[(i)] \textbf{Intrinsic fixed point.} $g_1^{(\mathrm{orig}),*} = g_2^* = g_3^* = 0$ is a fixed point of $R_n$ for all $n \geq 2$ and $K \geq 1$, in the intrinsic-channel coordinates.
\item[(ii)] \textbf{Stability.} All scaling dimensions are negative: $y_1 = -\tfrac{1}{2}$, $y_2 = -1$, $y_3 = -1$ (with $y_3^{(\mathrm{action})} = -2$ for the squared norm). The fixed point is infrared-stable.
\item[(iii)] \textbf{Universality.} All finite-coupling models flow to the transformer limit (in the intrinsic channel) under repeated coarse-graining.
\item[(iv)] \textbf{Emergent anisotropy in the total channel.} The coarse-graining map generates anisotropy from within-cluster mean variance: even at $g_1^{(\mathrm{orig})} = 0$, $\Sigma_A = \sigma^2 I + \mathrm{Var}_A(\mu)$ is generically anisotropic, so the total coupling $g_1^{(\mathrm{tot})} = g_1^{(\mathrm{orig})} + g_1^{(\mathrm{emer})}$ does not flow to zero. The intrinsic-channel fixed point of clause~(i) coexists with a non-zero total anisotropy floor $g_1^{(\mathrm{emer})}$ generated by the structure of $R_n$ itself.
...
for some constant $k$, and for all values of the ratio $t = q_i(c)/[\Omega_{ij}(c) q_j(c)]$ realised by varying the prior $p_i$ and the neighbour beliefs $\{q_j\}$ over their admissible families. The set of such ratios contains an open subinterval of $(0,\infty)$; by the real-analyticity hypothesis on $f$, the identity $f'(t) = \log t + k$ extends from this subinterval to all of $(0,\infty)$. Hence $f'(t)=\log t + k$ globally.
...
\end{equation}
where $H(q_i^{\star}) = \tfrac{1}{2}\log\bigl((2\pi e)^K |\Sigma_{q,i}|\bigr)$ is the differential entropy of $q_i^{\star}$ and $C = -\tfrac{K}{2}\log 2\pi - \tfrac{K}{2}$ is a $v$-invariant scalar constant. Both $H(q_i^{\star})$ and $C$ depend on the row index $i$ but not on the column index $v$. Under the softmax over $v$,
\begin{equation}
