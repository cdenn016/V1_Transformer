...
The uniqueness of the term is conditional and follows from two assumptions: (i) $\mathcal{D}$ is local in $c$, of $f$-divergence form $\displaystyle \int q_i(c) f \left(\frac{q_i(c)}{\Omega_{ij}(c) q_j(c)}\right) dc$; and (ii) the coupling is linear in $\beta_{ij}$. Exponential-family closure of the resulting $q_i^*$ is a \emph{consequence} of the theorem (Step 2 of the proof produces the log-linear form automatically once $f'(t)=\log t$), not an additional hypothesis.
