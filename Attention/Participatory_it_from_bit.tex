...
Due to these discoveries and realisations there exists an implicit, unspoken tension in our contemporary understanding of reality. Neuroscientists and philosophers, following Kant and Helmholtz through to Friston, argue that reality is a perceptual construction. The weaker version of this claim is the predictive-processing view of perception as a ``controlled hallucination'' that nevertheless approximately tracks action-relevant external structure~\cite{Clark2016,Seth2021}. The stronger version, Hoffman's Interface Theory of Perception~\cite{Hoffman2019}, argues that evolved perception is selected for fitness rather than veridicality and need bear no faithful relation to the noumenal substrate at all. The framework developed below is closer to the stronger reading: different agents pull back different geometries from the same noumenal substrate, with no agent-independent ``correct'' geometry. Despite this, predictable and repeatable patterns emerge about which agents form consensus.
...
When two agents maintain beliefs about the same underlying reality, they generally do so in different reference frames --- analogous to two physicists using different coordinate systems to describe the same spacetime event. To compare beliefs or exchange information, we must transform between frames. General relativity provides the physical analogy: two observers use different coordinate systems $(t,x,y,z)$ versus $(t',x',y',z')$ to describe the same event, with transformation laws relating measurements in one frame to the other. The framework here requires analogous structure for \emph{cognitive} frames, in the sense recently articulated by Lahav and Neemeh~\cite{LahavNeemeh2022,LahavNeemeh2025} in their relativistic theory of consciousness: a cognitive frame of reference is the perspective from which a particular cognitive system measures, and quantities such as phenomenal experience are observer-relative the way velocity is observer-relative in special relativity. Section~\ref{sec:lahav_convergence} develops the explicit correspondence in detail. Here the gauge frame field $\phi_i$ takes on this role: it specifies the cognitive frame of agent $i$, and the transport operators introduced below provide the explicit transformation law between cognitive frames that the relativistic-consciousness account requires but does not, in its current published form, supply.
...
Each smooth section induces a metric on the base manifold $\mathcal{C}$ via pullback. We define the belief-induced metric as $G_i^{(q)}(c) = (\sigma_i^{(q)})^* g_{\mathcal{B}}$, which in components becomes the expression below. Strictly, the construction is the pullback of the natural $L^2$ inner product on log-densities along the parameterised family $c \mapsto q_i(c)$, rather than a literal pullback of a fiber metric of the bundle: the Fisher-Rao metric $g_{\mathcal{B}}$ on the statistical manifold is pulled back via the composition of $\sigma_i^{(q)}$ with the natural map to log-density space. This is standard in information geometry and creates no category error, but the metric components arise from score functions, not from derivatives of the bundle structure itself.
...
This perspective aligns with Kant's phenomenal/noumenal distinction, where space is a form of intuition rather than a property of things-in-themselves. It resonates with Wheeler's observer-participatory universe, where reality cannot be separated from the act of observation. The stronger reading also resonates with Hoffman's Interface Theory of Perception~\cite{Hoffman2019}, in which perceived space is an evolved interface optimised for fitness rather than a faithful depiction of any agent-independent geometry. The weaker predictive-processing reading~\cite{Clark2016,Seth2021} is also compatible with the construction below, but the framework's pullback structure --- different agents inducing different metrics on the same noumenal substrate --- is naturally closer to the stronger Interface-Theory reading.
...
In the original formulation of this manuscript, this was identified as the most critical unsolved problem. We now argue that the source of the difficulty is not information geometry itself, but the restriction to the compact gauge group $\mathrm{SO}(3)$. As established in Section~\ref{sec:gauge_group_choice}, the natural gauge group for KL-based variational inference is $\mathrm{GL}(K)$---the full general linear group---not any compact subgroup. This observation opens a concrete pathway toward Lorentzian signature.
