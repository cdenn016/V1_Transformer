...
The natural gauge group for KL-based variational inference is $G = \mathrm{GL}(K)$, the general linear group of invertible $K \times K$ matrices. The KL divergence between multivariate Gaussians---and indeed all $f$-divergences---is invariant under simultaneous push-forward by any $\Omega \in \mathrm{GL}(K)$:
\begin{equation}
...
We derive the inter-agent coupling terms and their softmax attention weights from a single variational principle by formulating each agent's inference as source selection over gauge-transported neighbors. This derivation, in its 0-dimensional (single-point) limit recovering standard transformer attention (Section~\ref{sec:transformers}), here extends naturally to arbitrary base manifolds $\mathcal{C}$.
