...
This recovers relative position biases used in T5~\cite{raffel2020exploring} and other architectures that parameterize attention with learnable position-dependent offsets. In a straightforward manner, each positioning architecture emerges as a special case of \eqref{eq:softmax_attention_general} operating via the same mathematical object. Although the choice of attention priors may be ad-hoc, their fundamental source and structure is determined from first principles within this framework.
...
The Hessian of the free energy $M = \partial^2 \mathcal{F}/\partial\xi\partial\xi$ defines a configuration-space metric on belief space, which we refer to as the \emph{mass matrix} when (and only when) it admits a kinetic interpretation. The full state vector is $\xi = (\mu_1, \ldots, \mu_N, \Sigma_1, \ldots, \Sigma_N)$, yielding the block structure
\begin{equation}
