...
\textbf{Three philosophical traditions, one mathematical framework.} The introduction invokes Wheeler's ``it from bit'', Kant's phenomenal/noumenal distinction, and Lahav and Neemeh's relativistic theory of consciousness. These are not jointly consistent positions: Wheeler is a quantum-measurement participatory realist, Kant is a transcendental idealist whose noumenon is structurally inaccessible, and Lahav and Neemeh are relational physicalists whose underlying physical reality is unproblematic. The framework here gives mathematical content to a single position --- information-geometric structuralism with gauge-frame-dependent pullback (defined formally in Section~\ref{sec:consciousness_section}) --- which is consistent with elements of all three traditions but unifies none of them. Where the three positions disagree, the framework sides with Lahav and Neemeh while borrowing the noumenal/phenomenal vocabulary from Kant and the slogan ``it from bit'' from Wheeler. The borrowings should be read as borrowings of vocabulary and motivation, not of philosophical commitment.
...
\end{equation}
where $\Omega_{ij} = \exp(\phi_i)\exp(-\phi_j)$ is the gauge transport operator connecting their frames and $\tilde{\Omega}_{ij}$ is the corresponding transport on the model fiber. Agreement of the per-agent hyper-priors $r_i$ is not required for consensus, since $r_i$ is fixed and shared structurally.
\end{definition}
...
\end{equation}
All quantities inside the bracket live in agent $i$'s frame: $\tilde{\Lambda}_{q_k}$ is agent $k$'s precision pushed into $i$'s frame and $\Sigma_i$ is $i$'s native covariance, so the products $\tilde{\Lambda}_{q_k}\Sigma_i\tilde{\Lambda}_{q_k}$ are well-typed. The outer $\Omega_{ik} \otimes \Omega_{ik}$ factors implement the chain rule through $\tilde{\Lambda}_{q_k}(\Sigma_k)$. Under $\mathrm{O}(d)$, $\tilde{\Lambda}_{q_k} = \Omega_{ik}\Lambda_{q_k}\Omega_{ik}^T$ and the expression reduces to its earlier-draft form; under general $\mathrm{GL}(d)$, the GL-correct $\tilde{\Lambda}$ of Eq.~\eqref{eq:precision_transport} must be used.
...
\paragraph{Honest limitation: gauge invariance does not constrain the qualia assignment.}
The framework admits a severe limitation that must be stated plainly. By gauge invariance, the framework provides \emph{no constraint} on which gauge frame $\phi$ produces which quale. If $\phi_{\text{visual}}$ induces a metric whose geometric structure the framework identifies with ``redness,'' gauge equivalence means that any $\phi' = \phi + \xi$ (or, equivalently, any $G' = \Omega \cdot G \cdot \Omega^\top$ for $\Omega \in \mathrm{GL}(K)$) produces the same free energy and the same empirical predictions. The assignment $\phi_{\text{visual}} \mapsto \text{redness}$ rather than greenness, silence, or any other quale is entirely free: nothing in the formalism singles out which physical gauge frame produces which phenomenal experience. This is a feature in one philosophical reading --- it explains why the inverted spectrum is logically possible even under information-geometric structuralism, and why absent-qualia scenarios cannot be ruled out by theory. It is a bug in another reading --- it removes any predictive content from the qualia claim, reducing the proposed identification of geometry with phenomenal character to a relabelling of the problem rather than a solution. The framework supplies mathematical vocabulary but not mathematical constraint on the mapping from gauge structure to subjective quality, and we flag this prominently as a third honest acknowledgement alongside the two divergences from Lahav and Neemeh in Section~\ref{sec:lahav_convergence}.
...
The conclusions reached in the preceding subsection share a striking structural overlap with the Relativistic Theory of Consciousness developed independently by Lahav and Neemeh~\cite{LahavNeemeh2022,LahavNeemeh2025}. Their starting point is that both dualism and illusionism tacitly assume phenomenal consciousness is an absolute property, frame-independent in the way Newtonian time was once held to be. They argue that consciousness should instead be treated as relativistic: a system has phenomenal consciousness with respect to a \emph{cognitive frame of reference}, and the question whether some external system is conscious has no observer-independent answer, much as the question whether an object is at rest has no observer-independent answer in special relativity. In their Alice/Bob thought experiment, each observer measures the other as ``mere brain activity'' from their own cognitive frame; both measurements are correct, and the two perspectives are claimed to be related by an isomorphism between phenomenal structures (first-person side) and phenomenal-judgement representations (third-person side). The hard problem is taken to dissolve, on their account, once the absolutist assumption is dropped --- a stronger move than the relocation we adopt in Section~\ref{sec:consciousness_section}, and one of two honest divergences between the two frameworks discussed below.

The framework presented here arrives at the same conclusion by a different route. The cognitive frame of reference of agent $i$ is, in our construction, the gauge frame $\phi_i \in \mathfrak{gl}(K)$, and the section $\sigma_i: \mathcal{C} \to E$ that this frame determines. The first-person/third-person split is the difference between (a) the metric $G_i = \sigma_i^* g_{\mathcal{B}}$ that agent $i$ pulls back from the noumenal substrate, which constitutes its phenomenal geometry, and (b) the description of agent $i$ available to a different agent $j$, which is the same section read through $\Omega_{ji} = e^{\phi_j} e^{-\phi_i}$. The two readings are not equal; they are related by the gauge-covariant transport operator. This is precisely the kind of structure-preserving map between perspectives that Lahav and Neemeh assert exists but, from what is publicly available of their formalization, do not write down explicitly.

Three points of mathematical contact are worth stating directly. First, the transformation between cognitive frames in our framework is concrete: $\Omega_{ij}$ acts on belief means by left multiplication and on covariances by the sandwich product $\Omega_{ij} \Sigma_j \Omega_{ij}^\top$, and KL divergences are invariant under simultaneous push-forward by any element of $\mathrm{GL}(K)$. Second, the analogue of a frame-invariant quantity (the role played by the spacetime interval in special relativity) is the gauge-equivariant free energy itself: $F$ is a scalar under the diagonal action of $\mathrm{GL}(K)$, even though its constituent KL terms transform between frames. Third, the postulate that ``identical neural dynamics imply identical experience'' becomes, in our setting, the statement that two agents with identical $(\mu, \Sigma, \phi)$ at the relevant scale occupy the same section and therefore induce the same pullback metric; the postulate is recovered as a consequence of section identity rather than imposed.
...
Two honest divergences should be noted. Lahav and Neemeh's account is positioned as physicalist with relational/measurement-theoretic flavour, with the underlying reality of which the two perspectives are ``two sides'' left ontologically thin. The construction here commits to information-geometric structuralism (defined in the preceding subsection): the underlying reality is the gauge-theoretic information geometry of the framework, and both phenomenal experience and what we call physical structure arise as observer-dependent pullbacks from this substrate. This is a stronger ontological commitment than Lahav and Neemeh make, but it is in the same broad neighbourhood as their relational/measurement-theoretic physicalism --- both deny that physical reality is independent of cognitive frame --- rather than a sharp opposition. The resulting account of qualia in the preceding subsection follows from the structuralist commitment. Likewise, the relativistic analogy in Lahav and Neemeh proceeds without an analogue of the speed of light --- without the second postulate that uniquely fixes the Lorentz group in special relativity. The present framework does not supply such a second postulate either; what it offers instead is a mechanism, the Yang-Mills kinetic term acting on $\mathrm{GL}(K, \mathbb{C})$ gauge frames, by which Lorentz signature might emerge dynamically rather than be assumed. Whether either route reaches a fully Einsteinian rather than Galilean theory of cognitive frames remains open.
