...
\textbf{Conceptual reframing, not derivation.} The discussion of consciousness (Section~\ref{sec:consciousness_section}) and the connection to Lahav and Neemeh (Section~\ref{sec:lahav_convergence}) propose a dissolution of the hard problem rather than a solution. The framework provides mathematical vocabulary for treating phenomenal experience as gauge-frame-dependent, but it does not explain why gauge frames have phenomenal character at all. We have formalized the structure of experience without explaining felt quality.
...
\textbf{(A)} Proper time versus coordinate time. All trajectories lie above the light cone diagonal ($\tau = t$, dashed gray), indicating that oscillatory dynamics cause proper time to advance faster than coordinate time. heavier agents (red) accumulate substantially more proper time than lighter agents (blue): $\tau_{\text{heavy}}/\tau_{\text{light}} \approx 1.5$ after 30 time units, constituting a genuine relativistic time dilation effect from information geometry.
