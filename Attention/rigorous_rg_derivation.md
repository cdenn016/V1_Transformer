...
The term \(\mathcal F_s^{\rm higher}\) may include observation terms, self-terms, cross-scale priors, curvature penalties, entropy penalties, or implementation-specific regularizers.
...
\boxed{
 d\mathbb P_{s+1}(Y)
 =
 Z_{s+1}^{-1}e^{-\mathcal F_{s+1}^{\rm exact}(Y)/\tau}\,d\nu_{s+1}(Y).
}
\]
...
\[
\boxed{
 e^{-\mathcal F_{s+1}^{\rm exact}(Y)/\tau}
 =
 \int_{R_s(X)=Y}
...
up to an additive constant in \(\mathcal F_{s+1}^{\rm exact}\).
...
:=
\Pi_{\mathfrak M}(\mathcal F_{s+1}^{\rm exact}).
}
\]
...
\frac{
\|\mathcal F_{s+1}^{\rm exact}-\mathcal F_\theta-a\|_{\mathcal B}
}{
\tau+\|\mathcal F_{s+1}^{\rm exact}\|_{\mathcal B}
}.
}
\]
...
\[
\boxed{
A_{\rm eff}=A_{YY}-A_{Y\xi}A_{\xi\xi}^{-1}A_{\xi Y}.
}
\]
...
The microscopic free energy is quadratic in \((Y,\xi)\), and the constraint subspace removes only the zero mode corresponding to the retained parent coordinate. Integration over \(\xi\) is a finite-dimensional Gaussian integral. Gaussian integration produces another quadratic form in \(Y\), with Schur-complement matrix \(A_{\rm eff}\), plus the log-determinant of the internal Hessian. A quadratic form in Gaussian natural parameters is equivalent to a Gaussian KL/free-energy term with renormalized precision/couplings. Therefore the class closes. \(\square\)
...
4. edge-marginal mismatch \(\delta_I^{\rm marg}\) is small;
...
+
C_3\sum_I\delta_I^{\rm marg}
+
C_4\sum_I m_I^{-1}\|A_{Y\xi}^{(I)}\|^2
+
C_5\sum_I \mathcal E_{\rm anh}^{(I)}
+
C_6\sum_I\mathcal E_{\rm nonG}^{(I)}.
}
\]
...
- \(\delta_I^{\rm marg}\) measures incompatibility between barycentric weights and edge marginals.
- \(m_I^{-1}\|A_{Y\xi}^{(I)}\|^2\) is the Schur/slaving correction from slow-internal coupling.
- \(\mathcal E_{\rm anh}^{(I)}\) is the Laplace anharmonic correction.
- \(\mathcal E_{\rm nonG}^{(I)}\) measures failure of the Gaussian family to be closed under the exact pushforward.
...
\[
\mathcal L_{\rm micro}(I)=\mathcal F_{\rm micro}^*(I)+C_{\rm micro}(I).
\]

Define the optimized parent description length by

\[
\mathcal L_{\rm parent}(I)=\mathcal F_{\rm parent}^*(I)+C_{\rm parent}(I)+\varepsilon_I.
\]
...
\Delta_I
=
\mathcal L_{\rm micro}(I)-\mathcal L_{\rm parent}(I).
}
\]
...
\[
\mathcal F_{\rm parent}^*(I)
\le
\mathcal F_{\rm micro}^*(I)
-
A_I
+
L_qV_I^{(q)}
...
\[
C_I=C_{\rm parent}(I)-C_{\rm micro}(I).
\]
...
Subtracting from \(\mathcal L_{\rm micro}=\mathcal F_{\rm micro}^*+C_{\rm micro}\) gives
...
\[
\overline D_{\rm parent}
=
\frac{1}{|I|}
\sum_i\mathrm{KL}(\Omega_{Ii}q_i\|q_I).
\]

If microscopic alignment uses all ordered pairs with weight \(\lambda_{\rm pair}\), while the parent description uses child-parent edges with weight \(\lambda_{\rm parent}\), a crude saving estimate is

\[
\boxed{
A_I
\approx
\lambda_{\rm pair}|I|(|I|-1)\overline D_{\rm pair}
-
\lambda_{\rm parent}|I|\overline D_{\rm parent}.
}
\]
...
A_I
=
\mathcal F_{\rm micro}^*(I)-\mathcal F_{\rm parent,no\ overhead}^*(I).
}
\]
...
Let \(t_{\rm slow}\) be the timescale for parent variables. Adiabatic elimination requires

\[
\boxed{
t_{\rm int}\ll t_{\rm slow}.
}
\]
...
\[
\xi=\xi^*(Y)+O(t_{\rm int}/t_{\rm slow}).
\]
...
\dot Y
=
-\mathrm{grad}_{G_{\rm eff}}\mathcal F_{s+1}^{\rm exact}(Y)
+
...
\[
\boxed{
G_{\rm eff}=G_{YY}-G_{Y\xi}G_{\xi\xi}^{-1}G_{\xi Y}.
}
\]
...
3. For each cluster:
   ...
   7. Estimate closure residual terms: \(\mathcal H_{IJ}\), \(\delta_I^{\rm marg}\), anharmonicity, non-Gaussianity.
...
\[
\Delta_I=\mathcal L_{\rm micro}(I)-\mathcal L_{\rm parent}(I),
\]
