...
**When `skip_attention=True` requires care: `em_mode` selection.** The detaching EM modes `em_phi_p` and `em_phi_q` (rows with σ_p detached or φ detached or both — see the EM modes table below) detach σ_p and/or φ inside the FFN's E-step. In those modes the attention sublayer is the *sole* autograd path back to `sigma_embed` and `phi_embed`. Combining either of them with `skip_attention=True` will silently freeze σ_embed and φ_embed at initialization — Σ gets zero variance across tokens, φ/Ω cluster overlap completely in token visualizations. Use `em_mode='ift_phi'` (default) for a clean `skip_attention=True` configuration where the FFN's E-step itself provides the M-step gradients for σ and φ. `BlockConfig.__post_init__` warns when an incompatible combination is detected.
