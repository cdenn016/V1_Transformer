...
def soN_compose_bch_torch(
    ...
    if order >= 3:
        # Degree-4 BCH: -(1/24)[φ₂,[φ₁,[φ₁,φ₂]]]
        # Standard BCH: (1/24)[X,[Y,[Y,X]]] = -(1/24)[Y,[X,[X,Y]]] via Jacobi
        bracket_2_1_12 = soN_bracket_torch(phi2, bracket_1_12, generators)
        result = result - (1.0/24.0) * bracket_2_1_12

    if order >= 4:
        # Degree-5 BCH: all 6 Dynkin terms (Dynkin 1947; Varadarajan Theorem 2.15.4).
        #
        # The full degree-5 contribution is:
        #   Z₅ = -(1/720)([Y,[Y,[Y,[Y,X]]]] + [X,[X,[X,[X,Y]]]])
        #        +(1/360)([X,[Y,[Y,[Y,X]]]] + [Y,[X,[X,[X,Y]]]])
        #        +(1/120)([Y,[X,[Y,[X,Y]]]] + [X,[Y,[X,[Y,X]]]])
        #
        # All terms are rewritten with bracket_12 = [X,Y] using [Y,X] = -bracket_12.
        #
        # Intermediates available from order >= 2:
        #   bracket_1_12 = [X,[X,Y]],  bracket_2_12 = [Y,[X,Y]]
        # New depth-3 brackets:
        bracket_1_1_12 = soN_bracket_torch(phi1, bracket_1_12, generators)  # [X,[X,[X,Y]]]
        bracket_2_2_12 = soN_bracket_torch(phi2, bracket_2_12, generators)  # [Y,[Y,[X,Y]]]
        bracket_1_2_12 = soN_bracket_torch(phi1, bracket_2_12, generators)  # [X,[Y,[X,Y]]]
        # New depth-4 brackets:
        bracket_2_2_2_12 = soN_bracket_torch(phi2, bracket_2_2_12, generators)  # [Y,[Y,[Y,[X,Y]]]]
        bracket_1_1_1_12 = soN_bracket_torch(phi1, bracket_1_1_12, generators)  # [X,[X,[X,[X,Y]]]]
        bracket_1_2_2_12 = soN_bracket_torch(phi1, bracket_2_2_12, generators)  # [X,[Y,[Y,[X,Y]]]]
        bracket_2_1_1_12 = soN_bracket_torch(phi2, bracket_1_1_12, generators)  # [Y,[X,[X,[X,Y]]]]
        bracket_2_1_2_12 = soN_bracket_torch(phi2, bracket_1_2_12, generators)  # [Y,[X,[Y,[X,Y]]]]
        bracket_1_2_1_12 = soN_bracket_torch(phi1, bracket_2_1_12, generators)  # [X,[Y,[X,[X,Y]]]]
        #
        # Z₅ assembled (signs from [Y,X] = -[X,Y] rewriting, verified term-by-term):
        result = (result
                  + (1.0 / 720.0) * bracket_2_2_2_12    # -(1/720)[Y,[Y,[Y,[Y,X]]]] -> +(1/720)[Y,[Y,[Y,[X,Y]]]]
                  - (1.0 / 720.0) * bracket_1_1_1_12    # -(1/720)[X,[X,[X,[X,Y]]]]
                  - (1.0 / 360.0) * bracket_1_2_2_12    # +(1/360)[X,[Y,[Y,[Y,X]]]] -> -(1/360)[X,[Y,[Y,[X,Y]]]]
                  + (1.0 / 360.0) * bracket_2_1_1_12    # +(1/360)[Y,[X,[X,[X,Y]]]]
                  + (1.0 / 120.0) * bracket_2_1_2_12    # +(1/120)[Y,[X,[Y,[X,Y]]]]
                  - (1.0 / 120.0) * bracket_1_2_1_12    # +(1/120)[X,[Y,[X,[Y,X]]]] -> -(1/120)[X,[Y,[X,[X,Y]]]]
                  )
...
def retract_soN_torch(
    ...
    bch_order: int = 4,  # Degree-5 BCH (all 6 Dynkin terms; O(ε^6) error)
    eps: float = 1e-6,
) -> 'torch.Tensor':
    """
    ...
    Args:
        ...
        bch_order: Order of BCH expansion (0=add, 1-4=BCH corrections)
...
def lie_compose_bch_general_torch(
    ...
    if order >= 3:
        # Degree-4 BCH: -(1/24)[φ₂,[φ₁,[φ₁,φ₂]]]
        # Standard BCH: (1/24)[X,[Y,[Y,X]]] = -(1/24)[Y,[X,[X,Y]]] via Jacobi
        bracket_2_1_12 = lie_bracket_general_torch(phi2, bracket_1_12, generators)
        result = result - (1.0/24.0) * bracket_2_1_12
    ...
    if order >= 4:
        ...
        #      +(1/120)([Y,[X,[Y,[X,Y]]]] + [X,[Y,[X,[Y,X]]]])
        #
        # Rewritten with bracket_12 = [X,Y] using [Y,X] = -bracket_12.
        bracket_1_1_12 = lie_bracket_general_torch(phi1, bracket_1_12, generators)  # [X,[X,[X,Y]]]
        bracket_2_2_12 = lie_bracket_general_torch(phi2, bracket_2_12, generators)  # [Y,[Y,[X,Y]]]
        bracket_1_2_12 = lie_bracket_general_torch(phi1, bracket_2_12, generators)  # [X,[Y,[X,Y]]]
        bracket_2_2_2_12 = lie_bracket_general_torch(phi2, bracket_2_2_12, generators)  # [Y,[Y,[Y,[X,Y]]]]
        bracket_1_1_1_12 = lie_bracket_general_torch(phi1, bracket_1_1_12, generators)  # [X,[X,[X,[X,Y]]]]
        bracket_1_2_2_12 = lie_bracket_general_torch(phi1, bracket_2_2_12, generators)  # [X,[Y,[Y,[X,Y]]]]
        bracket_2_1_1_12 = lie_bracket_general_torch(phi2, bracket_1_1_12, generators)  # [Y,[X,[X,[X,Y]]]]
        bracket_2_1_2_12 = lie_bracket_general_torch(phi2, bracket_1_2_12, generators)  # [Y,[X,[Y,[X,Y]]]]
        bracket_1_2_1_12 = lie_bracket_general_torch(phi1, bracket_2_1_12, generators)  # [X,[Y,[X,[X,Y]]]]
        result = (result
                  + (1.0 / 720.0) * bracket_2_2_2_12
                  - (1.0 / 720.0) * bracket_1_1_1_12
                  - (1.0 / 360.0) * bracket_1_2_2_12
                  + (1.0 / 360.0) * bracket_2_1_1_12
                  + (1.0 / 120.0) * bracket_2_1_2_12
                  - (1.0 / 120.0) * bracket_1_2_1_12
                  )
...
def glK_compose_bch_torch(
    ...
    Args:
        ...
        generators: Transport generators (n_gen, K, K); only n_gen is used
        order: BCH expansion order (0=addition, 1=first, 2=second, 3=third, 4=fourth)
    ...
    if order >= 3:
        # Degree-4 BCH: -(1/24)[φ₂,[φ₁,[φ₁,φ₂]]]
        # Standard BCH: (1/24)[X,[Y,[Y,X]]] = -(1/24)[Y,[X,[X,Y]]] via Jacobi
        bracket_2_1_12 = glK_bracket_torch(phi2, bracket_1_12, generators)
        result = result - (1.0/24.0) * bracket_2_1_12
    ...
    if order >= 4:
        ...
        #
        # Rewritten with bracket_12 = [X,Y] using [Y,X] = -bracket_12.
        bracket_1_1_12 = glK_bracket_torch(phi1, bracket_1_12, generators)  # [X,[X,[X,Y]]]
        bracket_2_2_12 = glK_bracket_torch(phi2, bracket_2_12, generators)  # [Y,[Y,[X,Y]]]
        bracket_1_2_12 = glK_bracket_torch(phi1, bracket_2_12, generators)  # [X,[Y,[X,Y]]]
        bracket_2_2_2_12 = glK_bracket_torch(phi2, bracket_2_2_12, generators)  # [Y,[Y,[Y,[X,Y]]]]
        bracket_1_1_1_12 = glK_bracket_torch(phi1, bracket_1_1_12, generators)  # [X,[X,[X,[X,Y]]]]
        bracket_1_2_2_12 = glK_bracket_torch(phi1, bracket_2_2_12, generators)  # [X,[Y,[Y,[X,Y]]]]
        bracket_2_1_1_12 = glK_bracket_torch(phi2, bracket_1_1_12, generators)  # [Y,[X,[X,[X,Y]]]]
        bracket_2_1_2_12 = glK_bracket_torch(phi2, bracket_1_2_12, generators)  # [Y,[X,[Y,[X,Y]]]]
        bracket_1_2_1_12 = glK_bracket_torch(phi1, bracket_2_1_12, generators)  # [X,[Y,[X,[X,Y]]]]
        result = (result
                  + (1.0 / 720.0) * bracket_2_2_2_12
                  - (1.0 / 720.0) * bracket_1_1_1_12
                  - (1.0 / 360.0) * bracket_1_2_2_12
...
def retract_glK_torch(
    ...
    **kwargs,  # Accept but ignore legacy grad_clip parameter
    ...
    3. Compose via BCH formula (fourth-order by default for accurate Lie group composition)
    4. Clamp final norm
