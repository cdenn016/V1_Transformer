...
# ═══════════════════════════════════════════════════════════════════════
CONFIG = {
    'mode':          'basic',    # 'basic', 'pub', or 'paper'
    'file':          None,       # Path to metrics.csv (auto-detects if None)
    'ablation_dir':  None,       # Directory with FFN mode subdirs (for paper mode)
    'output':        None,       # Output file/directory path (auto if None)
    'start_step':    5,          # Skip initial steps to avoid transient spikes
}
...
def main():
    ...
    ablation_dir_str = CONFIG['ablation_dir']
    ...
    if mode == 'paper':
        if ablation_dir_str is None:
            print("\nPaper mode requires 'ablation_dir' in CONFIG")
            return
    ...
    if file_path:
        ...
        if input_path is None:
            ...
            print("   Run training first or set 'file' in CONFIG")
            return
    ...
    print(f"   Skipping first {start_step} steps (edit 'start_step' in CONFIG to change)")

    if mode == 'basic':
        output_path = Path(output_str) if output_str else input_path.parent / 'training_plots.png'
        print_basic_summary(metrics)
        create_basic_plots(metrics, output_path, start_step=start_step)

    elif mode == 'pub':
        output_dir = Path(output_str) if output_str else input_path.parent / 'figures'
