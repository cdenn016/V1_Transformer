    ...
    def forward(
        ...
        for iteration in range(self.n_iterations):
            ...
            if self.update_phi_per_iteration and torch.is_grad_enabled():
                ...
                # Compute ∂F/∂φ — guard against broken autograd graph
                if alignment_loss.grad_fn is None:
                    import warnings
                    warnings.warn(
                        f"[VariationalFFNDynamic] alignment_loss has no grad_fn — "
                        f"skipping phi update at iteration {iteration}. "
                        f"alignment_loss={alignment_loss.item():.4e}, "
                        f"phi_for_grad.requires_grad={phi_for_grad.requires_grad}, "
                        f"K={K}, n_gen={self.generators.shape[0]}, "
                        f"multihead_vfe={self.multihead_vfe}, "
                        f"diagonal_covariance={is_diagonal}"
                    )
                else:
        ...
        if self.update_phi and not self.update_phi_per_iteration and torch.is_grad_enabled():
            ...
            if alignment_loss.grad_fn is None:
                import warnings
                warnings.warn(
                    f"[VariationalFFNDynamic] alignment_loss has no grad_fn (post-loop) — "
                    f"skipping phi update. "
                    f"alignment_loss={alignment_loss.item():.4e}, "
                    f"phi_for_grad.requires_grad={phi_for_grad.requires_grad}, "
                    f"K={K}, n_gen={self.generators.shape[0]}, "
                    f"multihead_vfe={self.multihead_vfe}, "
