...
class PriorBank(nn.Module):
    ...
    def decode(
        ...
        # only_forward=True: decode never needs exp(-φ·G), saving one matrix_exp
        # per token for GL(K). With block-diagonal structure this is skipped
        # entirely (irrep_dims=None for decode to avoid caching V×d_h×d_h pairs).
        all_token_ids = torch.arange(V, device=device)
        if self.gauge_fixed_priors and self.training:
            # Lambda wrapper avoids passing only_forward through checkpoint's
            # **kwargs (future-proofs against PyTorch adding same kwarg name).
