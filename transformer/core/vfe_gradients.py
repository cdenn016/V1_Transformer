...
def _compute_vfe_gradients_block_diagonal(
    ...
    for i_start in range(0, N, C):
        ...
        for block_idx, d in enumerate(irrep_dims):
            ...
            kl_values[:, i_start:i_end, :] = kl_values[:, i_start:i_end, :] + kl_block.clamp(min=0.0, max=max(100.0, KL_CEIL_SCALE * d))
    ...
    if _vfe_utils_mod._VFE_GRAD_DEBUG is not None:
        ...
        _kl_ceil = max(100.0, KL_CEIL_SCALE * K)
...
def compute_vfe_gradients_gpu(
    ...
    if is_diagonal:
        ...
        kl_ceil = max(100.0, KL_CEIL_SCALE * K)
...
def _compute_rope_full_gauge_gradient_per_head(
    ...
    """
    from transformer.core.transport_ops import _apply_rope, _apply_rope_to_covariance
