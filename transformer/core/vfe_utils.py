    ...
    **Gauge invariance**: The Cholesky factor L is NOT gauge covariant
    ...
    with torch.amp.autocast('cuda', enabled=False):
        ...
        if not info.any():
            ...
            # Secondary path: escalating jitter.  The regularization is
            # broadcast over all batch elements (including those that
            # already succeeded); the extra jitter is tiny relative to the
            # matrix scale so the successful elements are not meaningfully
            # perturbed.
            ...
            for _ in range(3):
                ...
                L, info = torch.linalg.cholesky_ex(M_reg_more)
                if not info.any():
                    result = torch.cholesky_inverse(L)
                    success = True
                    break
            if not success:
                # Last resort: pseudoinverse.  On a regularized SPD matrix
                # this equals the inverse up to roundoff; kept as a
                # never-fails safety net for pathological inputs.
                try:
