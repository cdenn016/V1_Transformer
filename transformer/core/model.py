        ...
        for layer_idx, block in enumerate(self.transformer.blocks):
            ...
            if self._collect_layer_diagnostics:
                ...
                _ld = {
                    ...
                    'mu_attn_norm': _mu_attn.detach().norm().item() if _mu_attn is not None else 0.0,
                    'mu_ffn_norm': _mu_ffn.detach().norm().item() if _mu_ffn is not None else 0.0,
                    'residual_ratio': (
                        (_mu_ffn.detach().norm().item() if _mu_ffn is not None else 0.0)
