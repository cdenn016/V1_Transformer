    ...
    def forward(
        ...
        *,
        ...
        if not self.skip_attention:
            ...
            # Delta extraction (symmetric with the FFN residual at line 548):
            # mu_attn = Σ_j β_ij · Ω_ij · mu_normalized[j] is an aggregated
            # version of the normalized input, NOT a zero-centered correction.
            # When self-attention dominates (KL(q_i||q_i)=0 makes β_ii maximal
            # whenever mask_self_attention=False), mu_attn[i] ≈ mu_normalized[i],
            # so the plain residual mu_q + mu_attn would dump norm(mu) into the
            # residual stream each layer — the exact pathology the FFN delta
            # extraction was introduced to prevent.  Extract (mu_attn -
            # mu_normalized) so the residual stream accumulates corrections
            # rather than copies of the pre-normalization input.
