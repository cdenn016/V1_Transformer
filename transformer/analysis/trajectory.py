...
class LayerTrajectory:
    ...
    phi_in: np.ndarray                 # (B, N, phi_dim)

    # Output state
    mu_out: np.ndarray                 # (B, N, K)
    Sigma_diag_out: np.ndarray         # (B, N, K)
    phi_out: np.ndarray                # (B, N, phi_dim)

    # Attention
    beta: Optional[np.ndarray] = None  # (B, n_heads, N, N) or (B, N, N)
    kl_matrix: Optional[np.ndarray] = None  # (B, N, N) KL divergences

    # Intra-fiber trajectory (per-iteration within this layer's E-step)
    iterations: Optional[List[IterationSnapshot]] = None
    recorded_token_indices: Optional[np.ndarray] = None  # which tokens were recorded
...
class ForwardTrajectory:
    ...
    phi_embed: np.ndarray              # (B, N, phi_dim) initial gauge frames
...
class TrajectoryRecorder:
    """
    Records belief trajectories during forward passes.
    ...
    def record_layer_output(
        ...
        Sigma: torch.Tensor,
        phi: torch.Tensor,
        diagnostics: Optional[Dict] = None,  # Legacy parameter, ignored
    ) -> None:
...
def enable_trajectory_tracking(
    ...
    Example:
        ...
        >>> mu_trace = recorder.get_mu_trajectory(trajectory)
    """
