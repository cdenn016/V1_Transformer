...
"Drowning in work" maps physical drowning → overwhelmed via analogy.
The meaning IS derivable from components (unlike idioms), but the
mapping is non-linear (unlike literal composition).

Prediction from idiom study (confirmed double dissociation):
    - Idioms: holonomy DOWN, curvature UP
    - Metaphors: intermediate effect (partial compositionality)
    - Literal: baseline

Three measurement scales:
    1. Whole-sentence: holonomy and curvature over all tokens
    2. Phrase-localized: restrict to metaphor phrase tokens (length control)
    3. Cross-boundary: phrase + context tokens (interaction)
...
COLORS = {
    'metaphorical': '#ff7f0e',   # orange (between red idiom and green literal)
...
def find_phrase_positions(tokenizer, full_text, phrase):
    """
    Find the token positions of a metaphor phrase within a tokenized sentence.
...
def cross_boundary_positions(phrase_pos, n_tokens, context_window=3):
    """Phrase tokens + nearby context tokens for cross-boundary measurement."""
...
def plot_distributions(kappas_by_label, title, ylabel, output_path):
    ...
    labels_order = ['literal', 'metaphorical', 'control']
...
def plot_layer_profiles(profiles_by_label, title, output_path, n_layers=12):
    ...
    for label in ['literal', 'metaphorical', 'control']:
...
def plot_curvature_layer_profiles(profiles_by_label, output_path, n_layers=12):
    """Plot per-layer curvature as a function of depth."""
    fig, ax = plt.subplots(figsize=(10, 6))
    layers = list(range(n_layers))
...
def plot_paired_comparison(met_by_pair, lit_by_pair, shared, phrases_by_pair, ylabel, title, output_path):
    """Paired lines connecting metaphorical vs literal for same phrase."""
    fig, ax = plt.subplots(figsize=(8, 6))

    mval = [met_by_pair[p] for p in shared]
    lval = [lit_by_pair[p] for p in shared]

    for mv, lv in zip(mval, lval):
        color = COLORS['metaphorical'] if mv > lv else COLORS['literal']
        ax.plot([0, 1], [lv, mv], color=color, alpha=0.4, linewidth=1)

    ax.scatter([0]*len(lval), lval, c=COLORS['literal'], s=40, zorder=3, label='literal')
    ax.scatter([1]*len(mval), mval, c=COLORS['metaphorical'], s=40, zorder=3, label='metaphorical')

    n_higher = sum(1 for m, l in zip(mval, lval) if m > l)
    pct = 100 * n_higher / len(shared) if shared else 0

    ax.set_xticks([0, 1])
    ax.set_xticklabels(['Literal usage', 'Metaphorical usage'])
    ax.set_ylabel(ylabel)
    ax.set_title(f'{title}\nMetaphorical > Literal in {n_higher}/{len(shared)} pairs ({pct:.0f}%)')
    ax.legend()
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"  Saved: {output_path}")
...
def main():
    ...
    # ── 2. Load metaphor dataset ─────────────────────────────────────────
    hbar('Metaphor Dataset')
    all_pairs = load_metaphor_pairs()
    ...
    for la, lb in [('metaphorical','literal'), ('metaphorical','control'), ('literal','control')]:
    ...
    results = {'metaphorical': [], 'literal': [], 'control': []}
    layer_profiles = {'metaphorical': [], 'literal': [], 'control': []}
    ...
    for label, items in groups.items():
        for sp in items:
            ...
            plh = layerwise_jacobian_holonomy(model, ids, max_triples=MAX_TRI)
    ...
    curvature_results = {'metaphorical': [], 'literal': [], 'control': []}
    curvature_layer_profiles = {'metaphorical': [], 'literal': [], 'control': []}
    done = 0
    ...
    for label, items in groups.items():
        for sp in items:
            ...
            cr = discrete_curvature(model, ids, max_triples=MAX_PAIRS)
    ...
    for label in ['metaphorical', 'literal', 'control']:
        if label in kappas:
            k = kappas[label]
    ...
    met_by_pair = {hr.metadata['pair_id']: hr.kappa_mean for hr in results['metaphorical']}
    lit_by_pair = {hr.metadata['pair_id']: hr.kappa_mean for hr in results['literal']}
    phrases_by_pair = {}
    for hr in results['metaphorical']:
        phrases_by_pair[hr.metadata['pair_id']] = hr.metadata.get('metaphor', '')
    shared = sorted(set(met_by_pair) & set(lit_by_pair))

    n_higher = 0
    for pid in shared:
        mk, lk = met_by_pair[pid], lit_by_pair[pid]
        arrow = '>' if mk > lk else '<'
        tag = '*' if mk > lk else ' '
        phrase = phrases_by_pair.get(pid, '')
        print(f'  {tag} pair {pid:2d}: met={mk:.4f} {arrow} lit={lk:.4f}  "{phrase}"')
        if mk > lk:
            n_higher += 1
    ...
    if len(shared) >= 5:
        paired_met = [met_by_pair[p] for p in shared]
        paired_lit = [lit_by_pair[p] for p in shared]
        stat_w, p_w = stats.wilcoxon(paired_met, paired_lit, alternative='two-sided')
    ...
    curv_met_by_pair = {}
    curv_lit_by_pair = {}
    for idx, sp in enumerate(groups['metaphorical']):
        if idx < len(curvature_results['metaphorical']):
            curv_met_by_pair[sp.pair_id] = curvature_results['metaphorical'][idx]
    ...
    shared_curv = sorted(set(curv_met_by_pair) & set(curv_lit_by_pair))
    if len(shared_curv) >= 5:
        pc_met = [curv_met_by_pair[p] for p in shared_curv]
        pc_lit = [curv_lit_by_pair[p] for p in shared_curv]
        n_higher_c = sum(1 for m, l in zip(pc_met, pc_lit) if m > l)
        pct_c = 100 * n_higher_c / len(shared_curv)
        print(f'\n  Curvature: Metaphorical > Literal in {n_higher_c}/{len(shared_curv)} pairs ({pct_c:.0f}%)')
        stat_w_c, p_w_c = stats.wilcoxon(pc_met, pc_lit, alternative='two-sided')
    ...
    if len(shared) >= 5:
        paired_diffs = np.array(paired_met) - np.array(paired_lit)
    ...
    # ── 10. Bootstrap CIs ────────────────────────────────────────────────
    hbar('Bootstrap CIs (Paired Metaphorical - Literal)')
    N_BOOT = 10_000
    rng = np.random.RandomState(42)
    ...
    for l in range(n_layers):
        kl_met = [prof[l] for prof in layer_profiles['metaphorical'] if len(prof) > l]
        kl_lit = [prof[l] for prof in layer_profiles['literal'] if len(prof) > l]
        if len(kl_met) >= 2 and len(kl_lit) >= 2:
            U, p = stats.mannwhitneyu(kl_met, kl_lit, alternative='two-sided')
            d_mean = np.mean(kl_met) - np.mean(kl_lit)
            layer_pvals.append(p)
            layer_rows.append((l, np.mean(kl_met), np.mean(kl_lit), d_mean, p))
        else:
            layer_pvals.append(1.0)
            layer_rows.append((l, 0, 0, 0, 1.0))

    layer_qvals = benjamini_hochberg(layer_pvals)
    for (l, mm, ml, d_mean, p), q in zip(layer_rows, layer_qvals):
        sig = '*' if q < 0.05 else ' '
        print(f'  Layer {l:2d}: met={mm:.4f}  '
              f'lit={ml:.4f}  diff={d_mean:+.4f}  p={p:.4f}  q={q:.4f} {sig}')

    # ── 12. Phrase-localized holonomy ────────────────────────────────────
    hbar('Phrase-Localized Holonomy (Length Control)')
    print('  Measuring holonomy ONLY on metaphor-phrase tokens.')
    ...
    loc_met_by_pair = {}
    loc_lit_by_pair = {}
    loc_curv_met_by_pair = {}
    loc_curv_lit_by_pair = {}
    loc_layer_profiles = {'metaphorical': [], 'literal': []}
    loc_curv_layer_profiles = {'metaphorical': [], 'literal': []}

    pair_map = {}
    for sp in all_pairs:
        if sp.label in ('metaphorical', 'literal') and sp.metaphor:
            pair_map.setdefault(sp.pair_id, {})[sp.label] = sp

    n_found = 0
    n_skip = 0
    for pid in sorted(pair_map.keys()):
        if 'metaphorical' not in pair_map[pid] or 'literal' not in pair_map[pid]:
            continue
        sp_m = pair_map[pid]['metaphorical']
        sp_l = pair_map[pid]['literal']
        phrase = sp_m.metaphor

        ids_m = tokenize(tokenizer, sp_m.text)
        ids_l = tokenize(tokenizer, sp_l.text)

        pos_m = find_phrase_positions(tokenizer, sp_m.text, phrase)
        pos_l = find_phrase_positions(tokenizer, sp_l.text, phrase)

        if pos_m is None or pos_l is None or len(pos_m) < 3 or len(pos_l) < 3:
            n_skip += 1
            print(f'  SKIP pair {pid}: phrase "{phrase}" — '
                  f'pos_m={pos_m}, pos_l={pos_l}')
            continue

        n_found += 1
        t0 = time.time()

        plh_m = layerwise_jacobian_holonomy(model, ids_m, max_triples=MAX_TRI, positions=pos_m)
        plh_l = layerwise_jacobian_holonomy(model, ids_l, max_triples=MAX_TRI, positions=pos_l)

        loc_met_by_pair[pid] = plh_m['kappa_mean']
        loc_lit_by_pair[pid] = plh_l['kappa_mean']
        loc_layer_profiles['metaphorical'].append(plh_m['kappa_per_layer'])
        loc_layer_profiles['literal'].append(plh_l['kappa_per_layer'])

        cr_m = discrete_curvature(model, ids_m, max_triples=MAX_PAIRS, positions=pos_m)
        cr_l = discrete_curvature(model, ids_l, max_triples=MAX_PAIRS, positions=pos_l)

        loc_curv_met_by_pair[pid] = cr_m['curvature_mean']
        loc_curv_lit_by_pair[pid] = cr_l['curvature_mean']
        loc_curv_layer_profiles['metaphorical'].append(cr_m['curvature_per_layer'])
        loc_curv_layer_profiles['literal'].append(cr_l['curvature_per_layer'])

        dt = time.time() - t0
        arrow_h = '>' if plh_m['kappa_mean'] > plh_l['kappa_mean'] else '<'
        arrow_c = '>' if cr_m['curvature_mean'] > cr_l['curvature_mean'] else '<'
        print(f'  pair {pid:2d} "{phrase}": '
              f'hol {plh_m["kappa_mean"]:.4f} {arrow_h} {plh_l["kappa_mean"]:.4f}  '
              f'curv {cr_m["curvature_mean"]:.5f} {arrow_c} {cr_l["curvature_mean"]:.5f}  '
              f'({len(pos_m)}/{len(pos_l)} tok, {dt:.1f}s)')

    print(f'\n  Found phrase positions: {n_found} pairs  (skipped: {n_skip})')

    # ── 12a. Phrase-localized statistics ──────────────────────────────────
    loc_shared = sorted(set(loc_met_by_pair) & set(loc_lit_by_pair))
    if len(loc_shared) >= 5:
        hbar('Phrase-Localized Statistics')

        loc_pm = np.array([loc_met_by_pair[p] for p in loc_shared])
        loc_pl = np.array([loc_lit_by_pair[p] for p in loc_shared])
        loc_diffs = loc_pm - loc_pl

        n_higher_loc = int(np.sum(loc_pm > loc_pl))
        pct_loc = 100 * n_higher_loc / len(loc_shared)
        print(f'  Holonomy: Metaphorical > Literal in {n_higher_loc}/{len(loc_shared)} pairs ({pct_loc:.0f}%)')
        print(f'  Mean met: {np.mean(loc_pm):.4f}  literal: {np.mean(loc_pl):.4f}  '
              f'diff: {np.mean(loc_diffs):+.4f}')

        stat_w, p_w = stats.wilcoxon(loc_pm, loc_pl, alternative='two-sided')
        print(f'  Wilcoxon signed-rank: W={stat_w:.0f}, p={fmt_p(p_w)}')

        d_paired = np.mean(loc_diffs) / np.std(loc_diffs) if np.std(loc_diffs) > 0 else 0
        print(f'  Paired d: {d_paired:+.3f}')

        U_loc, p_loc = stats.mannwhitneyu(loc_pm, loc_pl, alternative='two-sided')
        pooled_std = np.sqrt((np.var(loc_pm) + np.var(loc_pl)) / 2)
        d_loc = (np.mean(loc_pm) - np.mean(loc_pl)) / pooled_std if pooled_std > 0 else 0
        ...
        tok_counts_m = [len(find_phrase_positions(tokenizer, pair_map[p]['metaphorical'].text,
                            pair_map[p]['metaphorical'].metaphor) or []) for p in loc_shared]
        tok_counts_l = [len(find_phrase_positions(tokenizer, pair_map[p]['literal'].text,
                            pair_map[p]['literal'].metaphor) or []) for p in loc_shared]
        n_exact_match = sum(1 for tm, tl in zip(tok_counts_m, tok_counts_l) if tm == tl)
        print(f'\n  Length verification: {n_exact_match}/{len(loc_shared)} pairs have identical phrase token count')
        print(f'  Mean phrase tokens: met={np.mean(tok_counts_m):.1f}  literal={np.mean(tok_counts_l):.1f}')
    ...
    loc_shared_c = sorted(set(loc_curv_met_by_pair) & set(loc_curv_lit_by_pair))
    if len(loc_shared_c) >= 5:
        loc_cm = np.array([loc_curv_met_by_pair[p] for p in loc_shared_c])
        loc_cl = np.array([loc_curv_lit_by_pair[p] for p in loc_shared_c])
        fm = np.isfinite(loc_cm) & np.isfinite(loc_cl)
        loc_cm_f, loc_cl_f = loc_cm[fm], loc_cl[fm]
        if len(loc_cm_f) >= 5:
            n_hc = int(np.sum(loc_cm_f > loc_cl_f))
            pct_hc = 100 * n_hc / len(loc_cm_f)
            print(f'\n  Phrase-localized curvature: Met > Lit in {n_hc}/{len(loc_cm_f)} ({pct_hc:.0f}%)')
            sw_c, pw_c = stats.wilcoxon(loc_cm_f, loc_cl_f, alternative='two-sided')
            d_c = np.mean(loc_cm_f - loc_cl_f) / np.std(loc_cm_f - loc_cl_f) if np.std(loc_cm_f - loc_cl_f) > 0 else 0
            print(f'  Wilcoxon: W={sw_c:.0f}, p={fmt_p(pw_c)}, paired d={d_c:+.3f}')
            stat_results['phrase_localized_curvature'] = {
                'n_pairs': int(len(loc_cm_f)),
    ...
    if loc_layer_profiles['metaphorical'] and loc_layer_profiles['literal']:
        hbar('Phrase-Localized Per-Layer Analysis')
        pl_pvals = []
        pl_rows = []
        for l in range(n_layers):
            kl_m = [prof[l] for prof in loc_layer_profiles['metaphorical'] if len(prof) > l]
            kl_l = [prof[l] for prof in loc_layer_profiles['literal'] if len(prof) > l]
            if len(kl_m) >= 2 and len(kl_l) >= 2:
                U_l, p_l = stats.mannwhitneyu(kl_m, kl_l, alternative='two-sided')
                d_mean_l = np.mean(kl_m) - np.mean(kl_l)
                pl_pvals.append(p_l)
                pl_rows.append((l, np.mean(kl_m), np.mean(kl_l), d_mean_l, p_l))
            else:
                pl_pvals.append(1.0)
                pl_rows.append((l, 0, 0, 0, 1.0))

        pl_qvals = benjamini_hochberg(pl_pvals)
        for (l, mm, ml, d_mean_l, p_l), q_l in zip(pl_rows, pl_qvals):
    ...
    print('  Measuring holonomy on metaphor phrase + nearby context tokens.')
    ...
    xb_curv_met_by_pair = {}
    xb_curv_lit_by_pair = {}
    xb_layer_profiles = {'metaphorical': [], 'literal': []}
    xb_pos_counts = {'metaphorical': [], 'literal': []}
    ...
    for pid in sorted(pair_map.keys()):
        ...
        if pos_m is None or pos_l is None or len(pos_m) < 2 or len(pos_l) < 2:
            continue

        xb_pos_m, _ = cross_boundary_positions(pos_m, N_m, CONTEXT_WINDOW)
        xb_pos_l, _ = cross_boundary_positions(pos_l, N_l, CONTEXT_WINDOW)

        if len(xb_pos_m) != len(xb_pos_l):
            target_len = min(len(xb_pos_m), len(xb_pos_l))
            if len(xb_pos_m) > target_len:
                center_m = (min(pos_m) + max(pos_m)) / 2
                xb_pos_m = sorted(xb_pos_m, key=lambda p: abs(p - center_m))[:target_len]
                xb_pos_m = sorted(xb_pos_m)
        ...
        if len(xb_pos_m) < 3 or len(xb_pos_l) < 3:
            continue

        n_xb_found += 1
        xb_pos_counts['metaphorical'].append(len(xb_pos_m))
        xb_pos_counts['literal'].append(len(xb_pos_l))
        t0 = time.time()

        plh_m = layerwise_jacobian_holonomy(model, ids_m, max_triples=MAX_TRI, positions=xb_pos_m)
        plh_l = layerwise_jacobian_holonomy(model, ids_l, max_triples=MAX_TRI, positions=xb_pos_l)

        xb_met_by_pair[pid] = plh_m['kappa_mean']
        xb_lit_by_pair[pid] = plh_l['kappa_mean']
        xb_layer_profiles['metaphorical'].append(plh_m['kappa_per_layer'])
        xb_layer_profiles['literal'].append(plh_l['kappa_per_layer'])

        cr_m = discrete_curvature(model, ids_m, max_triples=MAX_PAIRS, positions=xb_pos_m)
        cr_l = discrete_curvature(model, ids_l, max_triples=MAX_PAIRS, positions=xb_pos_l)

        xb_curv_met_by_pair[pid] = cr_m['curvature_mean']
        xb_curv_lit_by_pair[pid] = cr_l['curvature_mean']

        dt = time.time() - t0
        arrow_h = '>' if plh_m['kappa_mean'] > plh_l['kappa_mean'] else '<'
        arrow_c = '>' if cr_m['curvature_mean'] > cr_l['curvature_mean'] else '<'
        print(f'  pair {pid:2d}: hol {plh_m["kappa_mean"]:.4f} {arrow_h} {plh_l["kappa_mean"]:.4f}  '
              f'curv {cr_m["curvature_mean"]:.5f} {arrow_c} {cr_l["curvature_mean"]:.5f}  '
              f'({len(xb_pos_m)}/{len(xb_pos_l)} pos, {dt:.1f}s)  "{phrase}"')

    print(f'\n  Cross-boundary pairs: {n_xb_found}')

    # Cross-boundary statistics
    xb_shared = sorted(set(xb_met_by_pair) & set(xb_lit_by_pair))
    if len(xb_shared) >= 5:
        hbar('Cross-Boundary Statistics')

        xb_pm = np.array([xb_met_by_pair[p] for p in xb_shared])
        xb_pl = np.array([xb_lit_by_pair[p] for p in xb_shared])
        xb_diffs = xb_pm - xb_pl

        n_higher_xb = int(np.sum(xb_pm > xb_pl))
        pct_xb = 100 * n_higher_xb / len(xb_shared)
        print(f'  Holonomy: Met > Lit in {n_higher_xb}/{len(xb_shared)} pairs ({pct_xb:.0f}%)')
        print(f'  Mean met: {np.mean(xb_pm):.4f}  literal: {np.mean(xb_pl):.4f}  '
              f'diff: {np.mean(xb_diffs):+.4f}')

        stat_w_xb, p_w_xb = stats.wilcoxon(xb_pm, xb_pl, alternative='two-sided')
        ...
        n_exact_eq = sum(1 for a, b in zip(xb_pos_counts['metaphorical'], xb_pos_counts['literal']) if a == b)
        print(f'  Equalized positions: {n_exact_eq}/{len(xb_pos_counts["metaphorical"])} pairs identical  '
              f'mean met={np.mean(xb_pos_counts["metaphorical"]):.1f}  '
    ...
    xb_shared_c = sorted(set(xb_curv_met_by_pair) & set(xb_curv_lit_by_pair))
    if len(xb_shared_c) >= 5:
        xb_cm = np.array([xb_curv_met_by_pair[p] for p in xb_shared_c])
        xb_cl = np.array([xb_curv_lit_by_pair[p] for p in xb_shared_c])
        fm = np.isfinite(xb_cm) & np.isfinite(xb_cl)
        if np.sum(fm) >= 5:
            xb_cm_f, xb_cl_f = xb_cm[fm], xb_cl[fm]
            n_hc_xb = int(np.sum(xb_cm_f > xb_cl_f))
            pct_hc_xb = 100 * n_hc_xb / len(xb_cm_f)
            sw_xb, pw_xb = stats.wilcoxon(xb_cm_f, xb_cl_f, alternative='two-sided')
            d_c_xb = np.mean(xb_cm_f - xb_cl_f) / np.std(xb_cm_f - xb_cl_f) if np.std(xb_cm_f - xb_cl_f) > 0 else 0
            print(f'\n  Cross-boundary curvature: Met > Lit in {n_hc_xb}/{len(xb_cm_f)} ({pct_hc_xb:.0f}%)')
            print(f'  Wilcoxon: W={sw_xb:.0f}, p={fmt_p(pw_xb)}, paired d={d_c_xb:+.3f}')
            stat_results['cross_boundary_curvature'] = {
                'n_pairs': int(len(xb_cm_f)),
    ...
    xb_layer_rows = []
    xb_layer_qvals = None
    if xb_layer_profiles['metaphorical'] and xb_layer_profiles['literal']:
        hbar('Cross-Boundary Per-Layer Analysis')
        for l in range(n_layers):
            kl_m = [prof[l] for prof in xb_layer_profiles['metaphorical'] if len(prof) > l]
            kl_l = [prof[l] for prof in xb_layer_profiles['literal'] if len(prof) > l]
            if len(kl_m) >= 2 and len(kl_l) >= 2:
                U_l, p_l = stats.mannwhitneyu(kl_m, kl_l, alternative='two-sided')
                d_mean_l = np.mean(kl_m) - np.mean(kl_l)
                xb_layer_pvals.append(p_l)
                xb_layer_rows.append((l, np.mean(kl_m), np.mean(kl_l), d_mean_l, p_l))
            else:
                xb_layer_pvals.append(1.0)
                xb_layer_rows.append((l, 0, 0, 0, 1.0))

        xb_layer_qvals = benjamini_hochberg(xb_layer_pvals)
        for (l, mm, ml, d_mean_l, p_l), q_l in zip(xb_layer_rows, xb_layer_qvals):
            sig = '*' if q_l < 0.05 else ' '
            print(f'  Layer {l:2d}: met={mm:.4f}  '
                  f'lit={ml:.4f}  diff={d_mean_l:+.4f}  p={p_l:.4f}  q={q_l:.4f} {sig}')
    ...
    plot_layer_profiles(layer_profiles, 'Layer-wise Holonomy (VFE Steps)',
                        str(OUTPUT_DIR / 'layer_holonomy_profiles.png'), n_layers=n_layers)

    plot_curvature_layer_profiles(curvature_layer_profiles,
                                  str(OUTPUT_DIR / 'layer_curvature_profiles.png'), n_layers=n_layers)

    if shared:
        plot_paired_comparison(met_by_pair, lit_by_pair, shared, phrases_by_pair,
                               r'Mean holonomy $\kappa$',
                               'Paired Comparison: Same Phrase, Different Usage',
                               str(OUTPUT_DIR / 'paired_holonomy.png'))

    if shared_curv:
        plot_paired_comparison(curv_met_by_pair, curv_lit_by_pair, shared_curv, phrases_by_pair,
                               'Curvature (commutator norm)',
                               'Paired Curvature: Same Phrase, Different Usage',
                               str(OUTPUT_DIR / 'paired_curvature.png'))

    if loc_shared:
        plot_paired_comparison(loc_met_by_pair, loc_lit_by_pair, loc_shared, phrases_by_pair,
                               r'Phrase-localized $\kappa$',
                               'Phrase-Localized Holonomy (Length Controlled)',
                               str(OUTPUT_DIR / 'phrase_localized_holonomy.png'))

    if loc_shared_c:
        plot_paired_comparison(loc_curv_met_by_pair, loc_curv_lit_by_pair, loc_shared_c, phrases_by_pair,
                               'Phrase-localized curvature',
    ...
    if loc_layer_profiles['metaphorical'] and loc_layer_profiles['literal']:
        plot_layer_profiles(loc_layer_profiles,
                            ...
                            str(OUTPUT_DIR / 'phrase_localized_layer_profiles.png'), n_layers=n_layers)

    if loc_curv_layer_profiles['metaphorical'] and loc_curv_layer_profiles['literal']:
        plot_curvature_layer_profiles(loc_curv_layer_profiles,
                                      str(OUTPUT_DIR / 'phrase_localized_layer_curvature.png'), n_layers=n_layers)

    if xb_shared:
        plot_paired_comparison(xb_met_by_pair, xb_lit_by_pair, xb_shared, phrases_by_pair,
                               r'Cross-boundary $\kappa$',
                               'Cross-Boundary Holonomy (Phrase+Context)',
                               str(OUTPUT_DIR / 'cross_boundary_holonomy.png'))

    if xb_shared_c:
        plot_paired_comparison(xb_curv_met_by_pair, xb_curv_lit_by_pair, xb_shared_c, phrases_by_pair,
                               'Cross-boundary curvature',
    ...
    if xb_layer_profiles['metaphorical'] and xb_layer_profiles['literal']:
        plot_layer_profiles(xb_layer_profiles,
                            ...
                            str(OUTPUT_DIR / 'cross_boundary_layer_profiles.png'), n_layers=n_layers)
    ...
    if xb_shared and xb_shared_c:
        ...
        ax.set_ylabel("Paired Cohen's d (met - literal)")
        ax.set_title('Effect Size by Scale')
        ax.legend(loc='upper left', fontsize=9)

        ax = axes[1]
        xb_hol_diffs = np.array([xb_met_by_pair[p] - xb_lit_by_pair[p] for p in xb_shared])
        xb_curv_diffs_paired = []
        xb_shared_both = sorted(set(xb_shared) & set(xb_shared_c))
        for p in xb_shared_both:
            xb_curv_diffs_paired.append(xb_curv_met_by_pair[p] - xb_curv_lit_by_pair[p])
        ...
        ax.set_xlabel('Metaphorical - Literal')
        ax.set_ylabel('Density')
        ...
        if xb_layer_profiles['metaphorical'] and xb_layer_profiles['literal']:
            layers = list(range(n_layers))
            stack_m = np.array(xb_layer_profiles['metaphorical'])
            stack_l = np.array(xb_layer_profiles['literal'])
            diff_per_layer = np.mean(stack_m, axis=0)[:n_layers] - np.mean(stack_l, axis=0)[:n_layers]
            sem_diff = np.sqrt(
                (np.var(stack_m, axis=0)[:n_layers] / len(stack_m)) +
            ...
            ax.axhline(y=0, color='black', linewidth=0.5, linestyle='-')
            ax.set_xlabel('Layer')
            ax.set_ylabel(r'$\Delta\kappa$ (met - literal)')
    ...
    # ── 13c. Synthesis ───────────────────────────────────────────────────
    hbar('SYNTHESIS: Compositionality Gradient')
    print()
    print('  Metaphor gauge structure analysis:')
    print()
    print('  Metric               | Direction for metaphor | Interpretation')
    print('  ---------------------|----------------------|---------------------------')
    ...
    print()
    print('  Compositionality gradient prediction:')
    print('    literal  (full):    holonomy baseline, curvature baseline')
    print('    metaphor (partial): intermediate effect')
    print('    idiom    (none):    holonomy DOWN d=-0.34, curvature UP d=+0.41')
    print()
    print('  Gauge-theoretic reading:')
    print('    - Metaphors use PRODUCTIVE analogical mappings')
    print('    - The connection A should be partially smooth (some path')
    print('      dependence from the conceptual mapping)')
    print('    - The field strength F should be moderate (partial non-')
    print('      additivity from the cross-domain mapping)')
    print('    - If confirmed: curvature tracks compositionality gradient')
    ...
    summary = {
        ...
        'n_layers': n_layers,
        'd_model': d_model,
        'dataset': 'metaphor_pairs',
        'n_metaphorical': len(results['metaphorical']),
    ...
    if shared:
        summary['paired_holonomy'] = [
            {'pair_id': p, 'metaphor': phrases_by_pair.get(p, ''),
             'metaphorical': met_by_pair[p], 'literal': lit_by_pair[p]}
            for p in shared
        ]
    ...
    if loc_shared:
        summary['phrase_localized_holonomy'] = [
            {'pair_id': p, 'metaphor': phrases_by_pair.get(p, ''),
             'metaphorical': float(loc_met_by_pair[p]),
    ...
    if loc_shared_c:
        summary['phrase_localized_curvature'] = [
            {'pair_id': p, 'metaphor': phrases_by_pair.get(p, ''),
             'metaphorical': float(loc_curv_met_by_pair[p]),
    ...
    if xb_shared:
        summary['cross_boundary_holonomy_pairs'] = [
            {'pair_id': p, 'metaphor': phrases_by_pair.get(p, ''),
             'metaphorical': float(xb_met_by_pair[p]),
    ...
    if xb_shared_c:
        summary['cross_boundary_curvature_pairs'] = [
            {'pair_id': p, 'metaphor': phrases_by_pair.get(p, ''),
             'metaphorical': float(xb_curv_met_by_pair[p]),
             'literal': float(xb_curv_lit_by_pair[p])}
            for p in xb_shared_c
        ]

    with open(OUTPUT_DIR / 'metaphor_results.json', 'w') as f:
        json.dump(summary, f, indent=2, default=str)
    print(f'  Saved: {OUTPUT_DIR / "metaphor_results.json"}')
