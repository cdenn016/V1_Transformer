...
We implement and validate gauge-theoretic transformers using natural gradient descent on statistical manifolds with multi-head attention defined by Lie group structure. On token-level language modeling (WikiText-103, vocabulary 50,257), our gauge architectures achieve perplexity as low as [PPL\_BEST] using pure variational inference without MLPs, activation functions, or learned attention projections. We compare against standard transformers under parameter-matched ($\sim$[PARAMS\_MATCHED]M) and embedding-matched (dimension 100) conditions.

This proof-of-principle establishes that gauge-theoretic attention is not merely mathematically feasible but operationally viable at production scale. Our best architecture---an SO(3) model with multiple irreducible representations ($\ell=1,4,5,6,7$)---achieves [IMPROVEMENT\_BEST]$\times$ improvement over random chance through geometric structure alone, validating that transformer-like behavior emerges from gauge-theoretic principles. This SO(3) multi-irrep model with only [PARAMS\_SO3\_MULTI]M parameters \emph{outperforms} both the parameter-matched standard transformer (PPL [PPL\_BEST] vs [PPL\_STD\_PARAM]) and larger gauge models, demonstrating that irrep structure is more important than gauge group dimension. An SO(50) model with the fundamental representation achieves PPL [PPL\_SO50] with [PARAMS\_SO50]M parameters, while the compact SO(3) multi-irrep achieves PPL [PPL\_BEST] with [PARAMS\_RATIO\_SO50]$\times$ fewer parameters. Experiments with fully general (non-diagonal) covariance matrices confirm the framework operates without approximation. We conclude that neural architectures are, in this view, computational approximations to a deeper information gauge geometry that transcends the field of machine learning.
\end{abstract}
...
We validate on WikiText-103 token-level language modeling (vocabulary 50,257, context length 128--256) and compare against standard transformers under controlled conditions. Our best gauge VFE---an SO(3) model with multiple irreducible representations ($\ell=1,4,5,6,7$)---achieves perplexity [PPL\_BEST] (a [IMPROVEMENT\_BEST]$\times$ improvement over random chance), demonstrating that geometric inference alone produces meaningful language modeling that rivals standard neural approaches. With only [PARAMS\_SO3\_MULTI]M parameters, this compact architecture outperforms both a parameter-matched standard transformer (PPL [PPL\_STD\_PARAM]) and significantly larger gauge models, revealing that irrep structure dominates gauge group dimension for language modeling performance.
...
%
\begin{equation}
\beta_{ij} = \frac{\exp\!\left[-\kappa\_\beta^{-1} \, \mathrm{KL}(q\_i \| \Omega\_{ij}[q\_j])\right]}{\sum_k \exp\!\left[-\kappa\_\beta^{-1} \, \mathrm{KL}(q\_i \| \Omega\_{ik}[q\_k])\right]}
\end{equation}
%
...
Our models compute explicit beliefs $q_i(x)$, transport operators $\Omega_{ij}$, and belief disagreement $\mathrm{KL}(q_i \| \Omega_{ij}[q\_j])$, enabling direct inspection of communication dynamics.
...
&\quad - \underbrace{\mathbb{E}_q[\log p(o \mid \{k\_i\}, \{m\_i\})]}_{\text{(5) Observation likelihood}}
...
\nabla_{\Sigma_i} D_{\mathrm{KL}}(q_i \| p_i) &= \tfrac{1}{2}\left[(\Sigma\_p^i)^{-1} - (\Sigma\_q^i)^{-1}\right]. \label{eq:grad_sigma_self}
\end{align}

The coupling weights $\beta_{ij}$ have softmax form:

\begin{equation}
\beta_{ij} = \frac{\exp\!\left[-\kappa^{-1} K\_{ij}\right]}{\sum_k \exp\!\left[-\kappa^{-1} K\_{ik}\right]},
\quad K_{ij} := D_{\mathrm{KL}}(q_i \| \Omega_{ij}[q\_j]),
\label{eq:beta_def}
\end{equation}

where $K_{ij}$ denotes the KL divergence between agent $i$'s belief and the transported belief from agent $j$. 

For the alignment terms $\sum_{i,j} \beta_{ij} D_{\mathrm{KL}}(q_i \| \Omega_{ij}[q\_j])$, the product rule gives:

\begin{equation}
\nabla_{\mu_i} \left[\beta\_{ij} K\_{ij}\right] = 
...
\begin{equation}
\nabla_\theta \beta_{ij} = -\kappa_\beta^{-1} \beta_{ij} \left[\nabla\_\theta K\_{ij} - \sum\_k \beta\_{ik} \nabla\_\theta K\_{ik}\right],
\label{eq:grad_beta}
\end{equation}
...
&\quad + \sum_j \left[(\nabla\_{\mu\_i} \beta\_{ij}) K\_{ij} + \beta\_{ij} \nabla\_{\mu\_i} K\_{ij}\right] \label{eq:grad_mu_complete_b} \\
&\quad + \sum_k \beta_{ki} \nabla_{\mu_i} D_{\mathrm{KL}}(q_k \| \Omega_{ki}[q\_i]) \label{eq:grad_mu_complete_c} \\
...
&\quad + \sum_j \left[(\nabla\_{\Sigma\_i} \beta\_{ij}) K\_{ij} + \beta\_{ij} \nabla\_{\Sigma\_i} K\_{ij}\right] \label{eq:grad_sigma_complete_b} \\
&\quad + \sum_k \beta_{ki} \nabla_{\Sigma_i} D_{\mathrm{KL}}(q_k \| \Omega_{ki}[q\_i]) \label{eq:grad_sigma_complete_c} \\
...
\tfrac{1}{2}\left[\Sigma\_{p\_i}^{-1} - \Sigma\_i^{-1}\right],
\label{eq:grad_sigma_self_component}
\end{equation}
\begin{equation}
\nabla_{\Sigma_i} D_{\mathrm{KL}}(q_i \| \Omega_{ij}[q\_j]) = 
\tfrac{1}{2}\left[(\Omega\_{ij}[\Sigma\_j])^{-1} - \Sigma_i^{-1}\right],
\label{eq:grad_sigma_kl_forward}
\end{equation}
\begin{equation}
\nabla_{\Sigma_i} D_{\mathrm{KL}}(q_k \| \Omega_{ki}[q\_i]) =
\tfrac{1}{2} \Omega_{ki}^\top \, \Omega_{ki}[\Sigma\_i]^{-1} \, \Omega_{ki}.
\label{eq:grad_sigma_kl_backward}
\end{equation}
The coupling weight gradients follow from the chain rule:
\begin{equation}
\nabla_{\Sigma_i} \beta_{ij} = -\frac{\beta_{ij}}{\kappa_\beta} \left[\nabla\_{\Sigma\_i} D\_{\mathrm{KL}}(q\_i \| \Omega\_{ij}[q\_j]) - \langle \nabla_{\Sigma_i} D_{\mathrm{KL}}(q_i \| \Omega_{ik}[q\_k]) \rangle_{\beta_i} \right],
...
\begin{align}
\nabla_{\phi_i} \mathcal{F} &=
\sum_j \left[(\nabla\_{\phi\_i} \beta\_{ij}) K\_{ij} + \beta\_{ij} \nabla\_{\phi\_i} K\_{ij}\right] \label{eq:grad_phi_complete_a} \\
&\quad + \sum_k \beta_{ki} \nabla_{\phi_i} D_{\mathrm{KL}}(q_k \| \Omega_{ki}[q\_i]) \label{eq:grad_phi_complete_c} \\
\end{align}

For a transport operator $\Omega_{ij}[\cdot]$ acting on a Gaussian with parameters $(\mu, \Sigma)$, the gradients are:

\begin{equation}
\nabla_{\phi_i} \Omega_{ij}[\mu] = \frac{\mathrm{d}}{\mathrm{d}\phi_i}\left(e^{\phi_i} e^{-\phi_j} \mu\right) = \left[\frac{\mathrm{d} e^{\phi\_i}}{\mathrm{d}\phi\_i}\right] e^{-\phi_j} \mu,
\label{eq:grad_phi_transport_mu}
\end{equation}

\begin{equation}
\nabla_{\phi_i} \Omega_{ij}[\Sigma] = \frac{\mathrm{d}}{\mathrm{d}\phi_i}\left(e^{\phi_i} e^{-\phi_j} \Sigma e^{\phi_j} e^{-\phi_i}\right) = \left[\frac{\mathrm{d} e^{\phi\_i}}{\mathrm{d}\phi\_i}\right] e^{-\phi_j} \Sigma e^{\phi_j} + \text{transpose term}.
...
Our experiments demonstrate that the gauge VFE learns meaningful language statistics (up to [IMPROVEMENT\_BEST]$\times$ improvement over random) without positional encoding. This suggests that:
\begin{enumerate}
...
\begin{equation}
\theta \leftarrow \theta - \eta_M \, \nabla_{\theta} \mathcal{L}[\{q\_i^*\}, \theta],
\label{eq:m_step}
\end{equation}
...
\midrule
Embedding dim & [EMBED\_DIM\_VFE] & [EMBED\_DIM\_STD] & [EMBED\_DIM\_EMBED] \\
Layers & 1 & 6 & 6 \\
Attention heads & [HEADS\_VFE] & [HEADS\_STD] & [HEADS\_EMBED] \\
FFN hidden dim & --- & [FFN\_STD] & [FFN\_EMBED] \\
Parameters & [PARAMS\_SO20]M & [PARAMS\_STD\_PARAM]M & [PARAMS\_STD\_EMBED]M \\
\midrule
...
\midrule
Standard (param-match) & 6 & [PARAMS\_STD\_PARAM]M & [PPL\_STD\_PARAM] & [LOSS\_STD\_PARAM] & [IMPROVEMENT\_STD\_PARAM]$\times$ \\
Standard (embed-match) & 6 & [PARAMS\_STD\_EMBED]M & [PPL\_STD\_EMBED] & [LOSS\_STD\_EMBED] & [IMPROVEMENT\_STD\_EMBED]$\times$ \\
\midrule
\textbf{Gauge VFE SO(3) multi-irrep} & \textbf{1} & \textbf{[PARAMS\_SO3\_MULTI]M} & \textbf{[PPL\_BEST]} & \textbf{[LOSS\_BEST]} & \textbf{[IMPROVEMENT\_BEST]$\times$} \\
Gauge VFE SO(50) & 1 & [PARAMS\_SO50]M & [PPL\_SO50] & [LOSS\_SO50] & [IMPROVEMENT\_SO50]$\times$ \\
Gauge VFE SO(20) & 1 & [PARAMS\_SO20]M & [PPL\_SO20] & [LOSS\_SO20] & [IMPROVEMENT\_SO20]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The best gauge VFE---an SO(3) model with multiple irreducible representations ($\ell=1,4,5,6,7$, embedding dimension [EMBED\_DIM\_SO3])---achieves perplexity [PPL\_BEST], representing a [IMPROVEMENT\_BEST]$\times$ improvement over random chance (PPL 50,257). This demonstrates that the gauge-theoretic framework produces meaningful language modeling at production scale without standard neural network components: no MLPs, no activation functions, no learned attention projections.

The comparison reveals three key findings:

\begin{itemize}
  \item \textbf{Multi-irrep efficiency:} The SO(3) multi-irrep model with only [PARAMS\_SO3\_MULTI]M parameters achieves PPL [PPL\_BEST], \emph{outperforming} the parameter-matched standard transformer (PPL [PPL\_STD\_PARAM]) with [PARAMS\_STD\_PARAM]M parameters. This demonstrates that richer irrep structure provides more efficient representations than either larger gauge groups or more neural network parameters.

  \item \textbf{Gauge group scaling:} Larger gauge groups improve performance: SO(50) with the fundamental representation achieves PPL [PPL\_SO50] with [PARAMS\_SO50]M parameters. However, the compact SO(3) multi-irrep model achieves better performance with [PARAMS\_RATIO\_SO50]$\times$ fewer parameters, suggesting that irrep decomposition is more important than gauge group dimension.

  \item \textbf{Embedding-matched comparison:} With similar embedding dimension ([EMBED\_DIM\_SO3] vs [EMBED\_DIM\_VFE]), the gauge VFE \textbf{substantially outperforms} the embedding-matched standard transformer ([PPL\_BEST] vs [PPL\_STD\_EMBED] PPL) despite using only 1 layer compared to 6 layers. This confirms that geometric structure provides superior parameter efficiency when embedding dimensions are constrained.
\end{itemize}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{VFE_LLM_Manuscript/SO50_training_curves.png}
    ...
    \end{subfigure}
    \caption{Training and validation loss curves over 200,000 steps on WikiText-103. \textbf{(a)} Gauge VFE (SO(50)) shows steady descent from initial loss $\sim$11 to final validation loss [LOSS\_SO50] (PPL [PPL\_SO50]). Our best model, the SO(3) multi-irrep, achieves validation loss [LOSS\_BEST] (PPL [PPL\_BEST]). \textbf{(b)} Standard transformer (parameter-matched, [PARAMS\_MATCHED]M) shows similar descent pattern, reaching validation loss [LOSS\_STD\_PARAM] (PPL [PPL\_STD\_PARAM]).}
...
\begin{itemize}
  ...
  \item \textbf{Late phase (steps 100k--200k):} Gradual refinement with diminishing returns, reaching final PPL ranging from [PPL\_BEST] (SO(3) multi-irrep) to [PPL\_SO20] (SO(20)).
\end{itemize}
...
\midrule
Standard Transformer & $\sim$[TIME\_STD]s & $\sim$[TOKENS\_STD] & 1$\times$ & $\sim$[TOTAL\_TIME\_STD] \\
Gauge VFE (SO(20)) & $\sim$[TIME\_VFE]s & $\sim$[TOKENS\_VFE] & $\sim$[OVERHEAD\_VFE]$\times$ & $\sim$[TOTAL\_TIME\_VFE] \\
\bottomrule
\end{tabular}
\end{table}

The gauge VFE is approximately [OVERHEAD\_VFE]$\times$ slower than the standard baseline due to:
...
The current [OVERHEAD\_VFE]$\times$ overhead, while significant, is within the range where the approach becomes practical for research purposes.

Further optimization opportunities include sparse attention patterns (reducing $O(N^2)$ to $O(N)$), custom CUDA kernels for KL divergence and matrix exponentials, and mixed-precision training. We estimate that optimized implementations could reduce overhead to [OVERHEAD\_OPTIMIZED\_LOW]--[OVERHEAD\_OPTIMIZED\_HIGH]$\times$, making the approach competitive for certain applications where interpretability or geometric structure is valuable.  
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{VFE_LLM_Manuscript/SO50_train_val_gap.png}
...
At convergence, the gauge VFE architectures achieve validation losses ranging from [LOSS\_BEST] (PPL [PPL\_BEST] for SO(3) multi-irrep) to [LOSS\_SO20] (PPL [PPL\_SO20] for SO(20)), reducing uncertainty from the random baseline (PPL 50,257) by factors of [IMPROVEMENT\_SO20]--[IMPROVEMENT\_BEST]$\times$. This demonstrates that geometric inference provides meaningful language modeling capability without neural network parameters.
...
\centering
\small
\caption{SO(50) gauge VFE performance on WikiText-103. Fundamental SO(50) representation ($K=50$) with [PARAMS\_SO50]M parameters unless noted.}
...
\midrule
Diagonal & No & 200k & 256 & [PPL\_SO50\_DIAG\_VAL] & [PPL\_SO50\_DIAG\_TEST] & [IMPROVEMENT\_SO50\_DIAG]$\times$ \\
Diagonal & Yes & 150k & 256 & [PPL\_SO50\_PFLOW\_VAL] & --- & [IMPROVEMENT\_SO50\_PFLOW]$\times$ \\
Diagonal & No & 20k & 128 & [PPL\_SO50\_20K\_VAL] & [PPL\_SO50\_20K\_TEST] & [IMPROVEMENT\_SO50\_20K]$\times$ \\
Full (non-diagonal) & No & 25k & 128 & [PPL\_SO50\_FULL\_VAL] & [PPL\_SO50\_FULL\_TEST] & [IMPROVEMENT\_SO50\_FULL]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The fully-trained SO(50) diagonal model without P-flow (200k steps) achieves validation perplexity [PPL\_SO50\_DIAG\_VAL] (test PPL [PPL\_SO50\_DIAG\_TEST]), representing a [IMPROVEMENT\_SO50\_DIAG]$\times$ improvement over random chance. However, P-flow training with EMA prior updates (Section~\ref{sec:p_flow}) exhibited divergent behavior: validation perplexity increased from [PPL\_PFLOW\_START] to [PPL\_SO50\_PFLOW\_VAL] over training, suggesting that the current P-flow implementation may require further tuning or may be incompatible with large gauge groups. The SO(50) results still outperform our SO(20) baseline (PPL [PPL\_SO20]), but are surpassed by the compact SO(3) multi-irrep model (PPL [PPL\_BEST] with only [PARAMS\_SO3\_MULTI]M parameters).
...
At comparable training budgets (20k--25k steps, context length 128), the diagonal and non-diagonal models achieve nearly identical performance: the diagonal model reaches validation PPL [PPL\_SO50\_20K\_VAL] (test [PPL\_SO50\_20K\_TEST]) at 20k steps, while the non-diagonal model achieves validation PPL [PPL\_SO50\_FULL\_VAL] (test [PPL\_SO50\_FULL\_TEST]) at 25k steps. Remarkably, the non-diagonal model achieves slightly better test perplexity despite the diagonal model having a modest validation advantage. This demonstrates that the full covariance formulation incurs minimal performance penalty while enabling richer belief representations.
...
\begin{enumerate}
  ...
  \item \textbf{Irrep structure dominates gauge dimension:} The compact SO(3) multi-irrep model ([PARAMS\_SO3\_MULTI]M params) outperforms both larger gauge groups (SO(50) with [PARAMS\_SO50]M params) and standard transformers ([PARAMS\_STD\_PARAM]M params). Higher-dimensional SO(3) irreps capture richer geometric structure than larger groups restricted to fundamental representations.

  \item \textbf{Computational feasibility:} The implementation achieves $\sim$[TOKENS\_VFE] tokens/second ($\sim$[OVERHEAD\_VFE]$\times$ overhead vs standard), a dramatic improvement over earlier prototypes, demonstrating that gauge-theoretic inference is practically trainable.
\end{enumerate}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{VFE_LLM_Manuscript/SO50_belief_clustering_step200000.png}
    \caption{Semantic clustering in $\mu$ for SO(50) with $K=100$. Left: 3D PCA projection of the 190-dimensional gauge frame space $\mathfrak{so}(50)$. Right: 2D projection (PC1 vs PC2). Punctuation (green) forms a tight cluster near the origin, while content words (orange) and function words (purple) separate along PC2. The first three principal components capture $\sim$[PCA\_TOTAL\_VARIANCE]\% of variance ([PCA\_PC1]\% + [PCA\_PC2]\% + [PCA\_PC3]\%), yet reveal clear linguistic organization. This structure emerges without supervision, purely from variational free energy minimization.}
    \label{fig:gauge_clustering}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{VFE_LLM_Manuscript/SO50_gauge_frame_clustering_step200000.png}
...
A notable empirical observation is that gauge attention exhibits structured patterns early in training that progressively converge toward uniformity. At step 500, the attention weights $\beta_{ij}$ display differentiated patterns across heads, with visible variation in how each head distributes attention over context positions. However, by step 15,000, all heads converge to approximately uniform distributions over valid (causally masked) positions, with attention entropy reaching [ATTN\_ENTROPY] nats---close to the maximum of $\ln(128) \approx [ATTN\_ENTROPY\_MAX]$ for uniform attention over 128 positions.
...
\begin{enumerate}
    ...
    \item \textbf{KL equalization:} As embeddings train, the transported beliefs $\Omega_{ij}[q\_j]$ may become approximately equidistant from $q_i$ in KL divergence, eliminating the signal that would produce discriminative attention.
...
\begin{equation}
\frac{\partial \beta_{ij}}{\partial \mu_i} = \frac{\beta_{ij}}{\kappa} \left[ \frac{\partial D\_{\mathrm{KL},ij}}{\partial \mu\_i} - \sum\_k \beta\_{ik} \frac{\partial D\_{\mathrm{KL},ik}}{\partial \mu\_i} \right],
...
\begin{enumerate}
  ...
  \item \textbf{Practical implementation:} With $\sim$[OVERHEAD\_VFE]$\times$ computational overhead (improved from $\sim$[OVERHEAD\_EARLIER]$\times$ in earlier prototypes), the approach is feasible for research-scale experiments.
...
\midrule
SO(3) & 3 & [ABLATION\_PARAMS\_SO3]M & [ABLATION\_VAL\_SO3] & [ABLATION\_TEST\_SO3] & [ABLATION\_IMP\_SO3]$\times$ \\
SO(5) & 5 & [ABLATION\_PARAMS\_SO5]M & [ABLATION\_VAL\_SO5] & [ABLATION\_TEST\_SO5] & [ABLATION\_IMP\_SO5]$\times$ \\
SO(10) & 10 & [ABLATION\_PARAMS\_SO10]M & [ABLATION\_VAL\_SO10] & [ABLATION\_TEST\_SO10] & [ABLATION\_IMP\_SO10]$\times$ \\
SO(15) & 15 & [ABLATION\_PARAMS\_SO15]M & [ABLATION\_VAL\_SO15] & [ABLATION\_TEST\_SO15] & [ABLATION\_IMP\_SO15]$\times$ \\
SO(20) & 20 & [ABLATION\_PARAMS\_SO20]M & [ABLATION\_VAL\_SO20] & [ABLATION\_TEST\_SO20] & [ABLATION\_IMP\_SO20]$\times$ \\
SO(25) & 25 & [ABLATION\_PARAMS\_SO25]M & [ABLATION\_VAL\_SO25] & [ABLATION\_TEST\_SO25] & [ABLATION\_IMP\_SO25]$\times$ \\
SO(30) & 30 & [ABLATION\_PARAMS\_SO30]M & [ABLATION\_VAL\_SO30] & [ABLATION\_TEST\_SO30] & [ABLATION\_IMP\_SO30]$\times$ \\
SO(45) & 45 & [ABLATION\_PARAMS\_SO45]M & [ABLATION\_VAL\_SO45] & [ABLATION\_TEST\_SO45] & [ABLATION\_IMP\_SO45]$\times$ \\
\textbf{SO(50)} & \textbf{50} & \textbf{[ABLATION\_PARAMS\_SO50]M} & \textbf{[ABLATION\_VAL\_SO50]} & \textbf{[ABLATION\_TEST\_SO50]} & \textbf{[ABLATION\_IMP\_SO50]$\times$} \\
SO(55) & 55 & [ABLATION\_PARAMS\_SO55]M & [ABLATION\_VAL\_SO55] & [ABLATION\_TEST\_SO55] & [ABLATION\_IMP\_SO55]$\times$ \\
SO(60) & 60 & [ABLATION\_PARAMS\_SO60]M & [ABLATION\_VAL\_SO60] & [ABLATION\_TEST\_SO60] & [ABLATION\_IMP\_SO60]$\times$ \\
SO(70) & 70 & [ABLATION\_PARAMS\_SO70]M & [ABLATION\_VAL\_SO70] & [ABLATION\_TEST\_SO70] & [ABLATION\_IMP\_SO70]$\times$ \\
SO(100) & 100 & [ABLATION\_PARAMS\_SO100]M & [ABLATION\_VAL\_SO100] & [ABLATION\_TEST\_SO100] & [ABLATION\_IMP\_SO100]$\times$ \\
\midrule
SO(50) & 100 & [ABLATION\_PARAMS\_SO50\_K100]M & [ABLATION\_VAL\_SO50\_K100] & [ABLATION\_TEST\_SO50\_K100] & [ABLATION\_IMP\_SO50\_K100]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

Performance improves with gauge group dimension up to SO(50), achieving optimal test PPL of [ABLATION\_TEST\_SO50]. Beyond SO(50), performance degrades despite increased parameters, suggesting optimization difficulties at larger scales. Notably, SO(50) with $K=100$ (using 2 copies of the fundamental irrep) achieves test PPL [ABLATION\_TEST\_SO50\_K100], outperforming SO(100) with $K=100$.
...
\midrule
\multicolumn{6}{l}{\textit{SO(50), $K=50$, [PARAMS\_SO50]M params}} \\
\quad & 1 & 20k & [VFE\_ITER1\_VAL] & [VFE\_ITER1\_TEST] & [VFE\_ITER1\_IMP]$\times$ \\
\quad & \textbf{5} & \textbf{20k} & \textbf{[VFE\_ITER5\_VAL]} & \textbf{[VFE\_ITER5\_TEST]} & \textbf{[VFE\_ITER5\_IMP]$\times$} \\
\midrule
\multicolumn{6}{l}{\textit{SO(20), $K=20$, [VFE\_SO20\_PARAMS]M params}} \\
\quad & 1 & 2k & [VFE\_SO20\_1\_VAL] & [VFE\_SO20\_1\_TEST] & [VFE\_SO20\_1\_IMP]$\times$ \\
\quad & 2 & 2k & [VFE\_SO20\_2\_VAL] & [VFE\_SO20\_2\_TEST] & [VFE\_SO20\_2\_IMP]$\times$ \\
\quad & 10 & 2k & [VFE\_SO20\_10\_VAL] & [VFE\_SO20\_10\_TEST] & [VFE\_SO20\_10\_IMP]$\times$ \\
\quad & 50 & 2k & [VFE\_SO20\_50\_VAL] & [VFE\_SO20\_50\_TEST] & [VFE\_SO20\_50\_IMP]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

At the optimal scale (SO(50)), increasing VFE iterations from 1 to 5 yields substantial improvement (test PPL [VFE\_ITER1\_TEST] $\to$ [VFE\_ITER5\_TEST]). At smaller scale with limited training (SO(20), 2k steps), additional VFE iterations provide modest benefit, suggesting iteration count interacts with model capacity and training budget.
...
\midrule
0.25 & [TEMP\_025\_VAL] & [TEMP\_025\_TEST] & [TEMP\_025\_IMP]$\times$ \\
0.75 & [TEMP\_075\_VAL] & [TEMP\_075\_TEST] & [TEMP\_075\_IMP]$\times$ \\
\textbf{1.00} & \textbf{[TEMP\_100\_VAL]} & \textbf{[TEMP\_100\_TEST]} & \textbf{[TEMP\_100\_IMP]$\times$} \\
1.25 & [TEMP\_125\_VAL] & [TEMP\_125\_TEST] & [TEMP\_125\_IMP]$\times$ \\
1.75 & [TEMP\_175\_VAL] & [TEMP\_175\_TEST] & [TEMP\_175\_IMP]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The model is relatively robust to temperature, with $\kappa_\beta = 1.0$ achieving best performance. Both lower ($\kappa_\beta < 1$) and higher ($\kappa_\beta > 1$) temperatures degrade performance modestly ([TEMP\_PPL\_DIFF\_LOW]--[TEMP\_PPL\_DIFF\_HIGH] PPL).
...
\midrule
\textbf{SO(3)} & \textbf{$\ell=1,4,5,6,7$ (5 irreps)} & \textbf{[EMBED\_DIM\_SO3]} & \textbf{[PARAMS\_SO3\_MULTI]M} & \textbf{[PPL\_BEST]} & \textbf{---} \\
SO(3) & $\ell=1,2,4,6,7$ (5 irreps, 2-layer) & [IRREP\_2L\_K] & [IRREP\_2L\_PARAMS]M & [IRREP\_2L\_VAL] & --- \\
SO(3) & fundamental ($\ell=1$), $K=60$ & [IRREP\_FUND\_K] & [IRREP\_FUND\_PARAMS]M & --- & [IRREP\_FUND\_TEST] \\
SO(50) & fundamental & 50 & [PARAMS\_SO50]M & [PPL\_SO50] & [PPL\_SO50\_TEST] \\
SO(50) & 2$\times$ fundamental & 100 & [ABLATION\_PARAMS\_SO50\_K100]M & --- & [ABLATION\_TEST\_SO50\_K100] \\
\bottomrule
\end{tabular}
\end{table}

The SO(3) model with multiple irreps ($\ell=1,4,5,6,7$) achieves validation PPL [PPL\_BEST] with only [PARAMS\_SO3\_MULTI]M parameters---outperforming both the [PARAMS\_SO50]M parameter SO(50) model (PPL [PPL\_SO50]) and the parameter-matched standard transformer (PPL [PPL\_STD\_PARAM]). This demonstrates that richer irrep decompositions are dramatically more parameter-efficient than larger gauge groups with single irreps, achieving better results with [PARAMS\_RATIO\_SO50]$\times$ fewer parameters. The critical insight is that higher-dimensional SO(3) irreps ($\ell=4,5,6,7$) capture richer geometric structure than larger gauge groups restricted to fundamental representations.
...
\centering
\small
\caption{Training ablation. SO(20), $K=20$, [TRAIN\_ABL\_PARAMS]M params, context 64.}
\label{tab:ablation_training}
\begin{tabular}{lrrrr}
\toprule
\textbf{Setting} & \textbf{Steps} & \textbf{Val PPL} & \textbf{Test PPL} & \textbf{vs Random} \\
\midrule
$\eta = 0.01$ & 2k & [TRAIN\_2K\_VAL] & [TRAIN\_2K\_TEST] & [TRAIN\_2K\_IMP]$\times$ \\
$\eta = 0.01$ & 4k & [TRAIN\_4K\_VAL] & [TRAIN\_4K\_TEST] & [TRAIN\_4K\_IMP]$\times$ \\
$\eta = 0.01$ & 20k & [TRAIN\_20K\_VAL] & [TRAIN\_20K\_TEST] & [TRAIN\_20K\_IMP]$\times$ \\
$\eta = 0.001$ & 40k & [TRAIN\_40K\_VAL] & [TRAIN\_40K\_TEST] & [TRAIN\_40K\_IMP]$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
\item \textbf{Irrep structure dominates:} SO(3) with multiple irreps ($\ell=1,4,5,6,7$) achieves our best result---validation PPL [PPL\_BEST] with only [PARAMS\_SO3\_MULTI]M params---dramatically outperforming both [PARAMS\_SO50]M param SO(50) (PPL [PPL\_SO50]) and [PARAMS\_STD\_PARAM]M param standard transformers (PPL [PPL\_STD\_PARAM]). This is the central finding: richer irrep decompositions are far more efficient than larger gauge groups.

\item \textbf{Optimal single-irrep capacity:} For single-irrep models, SO(50) with $K=50$ provides the best performance; larger groups show diminishing returns. SO(50) with $K=100$ (2$\times$ fundamental) achieves PPL [ABLATION\_TEST\_SO50\_K100].

\item \textbf{Multiple VFE iterations:} Increasing from 1 to 5 iterations improves test PPL from [VFE\_ITER1\_TEST] to [VFE\_ITER5\_TEST] at SO(50) scale.

\item \textbf{P-flow limitations:} The P-flow mechanism (EMA prior updates) showed divergent behavior with SO(50), with validation PPL increasing from [PPL\_PFLOW\_START] to [PPL\_SO50\_PFLOW\_VAL] over training. Further investigation is needed.

\item \textbf{Temperature robustness:} $\kappa_\beta = 1.0$ is optimal; deviations in either direction degrade performance by [TEMP\_PPL\_DIFF\_LOW]--[TEMP\_PPL\_DIFF\_HIGH] PPL.
...
\begin{equation}
\mathcal{F}_{\text{vacuum}}[\{q\_i\}] = \sum_i D_{\mathrm{KL}}(q_i \| p_i) + \sum_{i,j} \beta_{ij} D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j)
\label{eq:vacuum_fe}
\end{equation}
...
\begin{equation}
D_{\mathrm{KL}}(q_i \| p_i) = \frac{1}{2}\left[\frac{\|\mu\_i\|^2}{\sigma\_p^2} + \frac{\mathrm{tr}(\Sigma\_i)}{\sigma\_p^2} - K - \log\frac{|\Sigma\_i|}{\sigma\_p^{2K}}\right]
\end{equation}
...
\end{equation}
where the transported covariance $\Omega_{ij}\Sigma_j\Omega_{ij}^\top$ appears because KL divergence is evaluated against the parallel-transported belief $\Omega_{ij}[q\_j]$. For isotropic beliefs $\Sigma_j = \sigma^2 I$, this simplifies to $\sigma^{-2}(\mu_i - \Omega_{ij}\mu_j)$ since orthogonal transport preserves isotropy.
...
\begin{equation}
\mathcal{F}[\{q\_i\}] = \mathcal{F}_{\text{vacuum}}[\{q\_i\}] - \sum_i \mathbb{E}_{q_i}[\log p(o\_i \mid \mu\_i)]
\end{equation}

For categorical observations with cross-entropy loss:
\begin{equation}
-\mathbb{E}_{q_i}[\log p(o\_i \mid \mu\_i)] \approx -\log \mathrm{softmax}(\mu_i)_{o_i} = -\mu_i^{(o_i)} + \log \sum_k e^{\mu_i^{(k)}}
\end{equation}
...
\begin{equation}
\frac{\partial}{\partial \mu_i}\left[-\log p(o\_i \mid \mu\_i)\right] = \mathrm{softmax}(\mu_i) - e_{o_i}
\end{equation}
...
Gaussian distributions are a special case of exponential families, and the framework generalizes accordingly. An exponential family distribution takes the form $q_i(\xi) = h(\xi) \exp[\eta\_i^\top T(\xi) - A(\eta\_i)]$, where $\eta_i$ are natural parameters, $T(\xi)$ are sufficient statistics, and $A(\eta)$ is the log-partition function. In this parameterization, the KL divergence reduces to the Bregman divergence $D_{\mathrm{KL}}(q_i \| q_j) = D_A(\eta_j \| \eta_i) = A(\eta_j) - A(\eta_i) - \nabla A(\eta_i)^\top(\eta_j - \eta_i)$, and the Fisher-Rao metric becomes the Hessian $g_{ab} = \partial_a \partial_b A(\eta)$.
