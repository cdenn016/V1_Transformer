...
We implement and validate gauge-theoretic transformers using natural gradient descent on statistical manifolds with multi-head attention defined by Lie group structure. On token-level language modeling (WikiText-103, vocabulary 50,257), our gauge architectures achieve perplexity as low as [PPL_BEST] using pure variational inference without MLPs, activation functions, or learned attention projections. We compare against standard transformers under parameter-matched ($\sim$[PARAMS_MATCHED]M) and embedding-matched (dimension 100) conditions.

This proof-of-principle establishes that gauge-theoretic attention is not merely mathematically feasible but operationally viable at production scale. Our best architecture---an SO(3) model with multiple irreducible representations ($\ell=1,4,5,6,7$)---achieves [IMPROVEMENT_BEST]$\times$ improvement over random chance through geometric structure alone, validating that transformer-like behavior emerges from gauge-theoretic principles. This SO(3) multi-irrep model with only [PARAMS_SO3_MULTI]M parameters \emph{outperforms} both the parameter-matched standard transformer (PPL [PPL_BEST] vs [PPL_STD_PARAM]) and larger gauge models, demonstrating that irrep structure is more important than gauge group dimension. An SO(50) model with the fundamental representation achieves PPL [PPL_SO50] with [PARAMS_SO50]M parameters, while the compact SO(3) multi-irrep achieves PPL [PPL_BEST] with [PARAMS_RATIO_SO50]$\times$ fewer parameters. Experiments with fully general (non-diagonal) covariance matrices confirm the framework operates without approximation. We conclude that neural architectures are, in this view, computational approximations to a deeper information gauge geometry that transcends the field of machine learning.
\end{abstract}
...
We validate on WikiText-103 token-level language modeling (vocabulary 50,257, context length 128--256) and compare against standard transformers under controlled conditions. Our best gauge VFE---an SO(3) model with multiple irreducible representations ($\ell=1,4,5,6,7$)---achieves perplexity [PPL_BEST] (a [IMPROVEMENT_BEST]$\times$ improvement over random chance), demonstrating that geometric inference alone produces meaningful language modeling that rivals standard neural approaches. With only [PARAMS_SO3_MULTI]M parameters, this compact architecture outperforms both a parameter-matched standard transformer (PPL [PPL_STD_PARAM]) and significantly larger gauge models, revealing that irrep structure dominates gauge group dimension for language modeling performance.
...
Our experiments demonstrate that the gauge VFE learns meaningful language statistics (up to [IMPROVEMENT_BEST]$\times$ improvement over random) without positional encoding. This suggests that:
\begin{enumerate}
...
\midrule
Embedding dim & [EMBED_DIM_VFE] & [EMBED_DIM_STD] & [EMBED_DIM_EMBED] \\
Layers & 1 & 6 & 6 \\
Attention heads & [HEADS_VFE] & [HEADS_STD] & [HEADS_EMBED] \\
FFN hidden dim & --- & [FFN_STD] & [FFN_EMBED] \\
Parameters & [PARAMS_SO20]M & [PARAMS_STD_PARAM]M & [PARAMS_STD_EMBED]M \\
\midrule
...
\midrule
Standard (param-match) & 6 & [PARAMS_STD_PARAM]M & [PPL_STD_PARAM] & [LOSS_STD_PARAM] & [IMPROVEMENT_STD_PARAM]$\times$ \\
Standard (embed-match) & 6 & [PARAMS_STD_EMBED]M & [PPL_STD_EMBED] & [LOSS_STD_EMBED] & [IMPROVEMENT_STD_EMBED]$\times$ \\
\midrule
\textbf{Gauge VFE SO(3) multi-irrep} & \textbf{1} & \textbf{[PARAMS_SO3_MULTI]M} & \textbf{[PPL_BEST]} & \textbf{[LOSS_BEST]} & \textbf{[IMPROVEMENT_BEST]$\times$} \\
Gauge VFE SO(50) & 1 & [PARAMS_SO50]M & [PPL_SO50] & [LOSS_SO50] & [IMPROVEMENT_SO50]$\times$ \\
Gauge VFE SO(20) & 1 & [PARAMS_SO20]M & [PPL_SO20] & [LOSS_SO20] & [IMPROVEMENT_SO20]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The best gauge VFE---an SO(3) model with multiple irreducible representations ($\ell=1,4,5,6,7$, embedding dimension [EMBED_DIM_SO3])---achieves perplexity [PPL_BEST], representing a [IMPROVEMENT_BEST]$\times$ improvement over random chance (PPL 50,257). This demonstrates that the gauge-theoretic framework produces meaningful language modeling at production scale without standard neural network components: no MLPs, no activation functions, no learned attention projections.

The comparison reveals three key findings:

\begin{itemize}
  \item \textbf{Multi-irrep efficiency:} The SO(3) multi-irrep model with only [PARAMS_SO3_MULTI]M parameters achieves PPL [PPL_BEST], \emph{outperforming} the parameter-matched standard transformer (PPL [PPL_STD_PARAM]) with [PARAMS_STD_PARAM]M parameters. This demonstrates that richer irrep structure provides more efficient representations than either larger gauge groups or more neural network parameters.

  \item \textbf{Gauge group scaling:} Larger gauge groups improve performance: SO(50) with the fundamental representation achieves PPL [PPL_SO50] with [PARAMS_SO50]M parameters. However, the compact SO(3) multi-irrep model achieves better performance with [PARAMS_RATIO_SO50]$\times$ fewer parameters, suggesting that irrep decomposition is more important than gauge group dimension.

  \item \textbf{Embedding-matched comparison:} With similar embedding dimension ([EMBED_DIM_SO3] vs [EMBED_DIM_VFE]), the gauge VFE \textbf{substantially outperforms} the embedding-matched standard transformer ([PPL_BEST] vs [PPL_STD_EMBED] PPL) despite using only 1 layer compared to 6 layers. This confirms that geometric structure provides superior parameter efficiency when embedding dimensions are constrained.
\end{itemize}
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{Training and validation loss curves over 200,000 steps on WikiText-103. \textbf{(a)} Gauge VFE (SO(50)) shows steady descent from initial loss $\sim$11 to final validation loss [LOSS_SO50] (PPL [PPL_SO50]). Our best model, the SO(3) multi-irrep, achieves validation loss [LOSS_BEST] (PPL [PPL_BEST]). \textbf{(b)} Standard transformer (parameter-matched, [PARAMS_MATCHED]M) shows similar descent pattern, reaching validation loss [LOSS_STD_PARAM] (PPL [PPL_STD_PARAM]).}
...
\begin{itemize}
  ...
  \item \textbf{Late phase (steps 100k--200k):} Gradual refinement with diminishing returns, reaching final PPL ranging from [PPL_BEST] (SO(3) multi-irrep) to [PPL_SO20] (SO(20)).
\end{itemize}
...
\midrule
Standard Transformer & $\sim$[TIME_STD]s & $\sim$[TOKENS_STD] & 1$\times$ & $\sim$[TOTAL_TIME_STD] \\
Gauge VFE (SO(20)) & $\sim$[TIME_VFE]s & $\sim$[TOKENS_VFE] & $\sim$[OVERHEAD_VFE]$\times$ & $\sim$[TOTAL_TIME_VFE] \\
\bottomrule
\end{tabular}
\end{table}

The gauge VFE is approximately [OVERHEAD_VFE]$\times$ slower than the standard baseline due to:
...
The current [OVERHEAD_VFE]$\times$ overhead, while significant, is within the range where the approach becomes practical for research purposes.

Further optimization opportunities include sparse attention patterns (reducing $O(N^2)$ to $O(N)$), custom CUDA kernels for KL divergence and matrix exponentials, and mixed-precision training. We estimate that optimized implementations could reduce overhead to [OVERHEAD_OPTIMIZED_LOW]--[OVERHEAD_OPTIMIZED_HIGH]$\times$, making the approach competitive for certain applications where interpretability or geometric structure is valuable.  
...
At convergence, the gauge VFE architectures achieve validation losses ranging from [LOSS_BEST] (PPL [PPL_BEST] for SO(3) multi-irrep) to [LOSS_SO20] (PPL [PPL_SO20] for SO(20)), reducing uncertainty from the random baseline (PPL 50,257) by factors of [IMPROVEMENT_SO20]--[IMPROVEMENT_BEST]$\times$. This demonstrates that geometric inference provides meaningful language modeling capability without neural network parameters.
...
\centering
\small
\caption{SO(50) gauge VFE performance on WikiText-103. Fundamental SO(50) representation ($K=50$) with [PARAMS_SO50]M parameters unless noted.}
...
\midrule
Diagonal & No & 200k & 256 & [PPL_SO50_DIAG_VAL] & [PPL_SO50_DIAG_TEST] & [IMPROVEMENT_SO50_DIAG]$\times$ \\
Diagonal & Yes & 150k & 256 & [PPL_SO50_PFLOW_VAL] & --- & [IMPROVEMENT_SO50_PFLOW]$\times$ \\
Diagonal & No & 20k & 128 & [PPL_SO50_20K_VAL] & [PPL_SO50_20K_TEST] & [IMPROVEMENT_SO50_20K]$\times$ \\
Full (non-diagonal) & No & 25k & 128 & [PPL_SO50_FULL_VAL] & [PPL_SO50_FULL_TEST] & [IMPROVEMENT_SO50_FULL]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The fully-trained SO(50) diagonal model without P-flow (200k steps) achieves validation perplexity [PPL_SO50_DIAG_VAL] (test PPL [PPL_SO50_DIAG_TEST]), representing a [IMPROVEMENT_SO50_DIAG]$\times$ improvement over random chance. However, P-flow training with EMA prior updates (Section~\ref{sec:p_flow}) exhibited divergent behavior: validation perplexity increased from [PPL_PFLOW_START] to [PPL_SO50_PFLOW_VAL] over training, suggesting that the current P-flow implementation may require further tuning or may be incompatible with large gauge groups. The SO(50) results still outperform our SO(20) baseline (PPL [PPL_SO20]), but are surpassed by the compact SO(3) multi-irrep model (PPL [PPL_BEST] with only [PARAMS_SO3_MULTI]M parameters).
...
At comparable training budgets (20k--25k steps, context length 128), the diagonal and non-diagonal models achieve nearly identical performance: the diagonal model reaches validation PPL [PPL_SO50_20K_VAL] (test [PPL_SO50_20K_TEST]) at 20k steps, while the non-diagonal model achieves validation PPL [PPL_SO50_FULL_VAL] (test [PPL_SO50_FULL_TEST]) at 25k steps. Remarkably, the non-diagonal model achieves slightly better test perplexity despite the diagonal model having a modest validation advantage. This demonstrates that the full covariance formulation incurs minimal performance penalty while enabling richer belief representations.
...
\begin{enumerate}
  ...
  \item \textbf{Irrep structure dominates gauge dimension:} The compact SO(3) multi-irrep model ([PARAMS_SO3_MULTI]M params) outperforms both larger gauge groups (SO(50) with [PARAMS_SO50]M params) and standard transformers ([PARAMS_STD_PARAM]M params). Higher-dimensional SO(3) irreps capture richer geometric structure than larger groups restricted to fundamental representations.

  \item \textbf{Computational feasibility:} The implementation achieves $\sim$[TOKENS_VFE] tokens/second ($\sim$[OVERHEAD_VFE]$\times$ overhead vs standard), a dramatic improvement over earlier prototypes, demonstrating that gauge-theoretic inference is practically trainable.
\end{enumerate}
...
A notable empirical observation is that gauge attention exhibits structured patterns early in training that progressively converge toward uniformity. At step 500, the attention weights $\beta_{ij}$ display differentiated patterns across heads, with visible variation in how each head distributes attention over context positions. However, by step 15,000, all heads converge to approximately uniform distributions over valid (causally masked) positions, with attention entropy reaching [ATTN_ENTROPY] nats---close to the maximum of $\ln(128) \approx [ATTN_ENTROPY_MAX]$ for uniform attention over 128 positions.
...
\begin{enumerate}
  ...
  \item \textbf{Practical implementation:} With $\sim$[OVERHEAD_VFE]$\times$ computational overhead (improved from $\sim$[OVERHEAD_EARLIER]$\times$ in earlier prototypes), the approach is feasible for research-scale experiments.
...
\midrule
SO(3) & 3 & [ABLATION_PARAMS_SO3]M & [ABLATION_VAL_SO3] & [ABLATION_TEST_SO3] & [ABLATION_IMP_SO3]$\times$ \\
SO(5) & 5 & [ABLATION_PARAMS_SO5]M & [ABLATION_VAL_SO5] & [ABLATION_TEST_SO5] & [ABLATION_IMP_SO5]$\times$ \\
SO(10) & 10 & [ABLATION_PARAMS_SO10]M & [ABLATION_VAL_SO10] & [ABLATION_TEST_SO10] & [ABLATION_IMP_SO10]$\times$ \\
SO(15) & 15 & [ABLATION_PARAMS_SO15]M & [ABLATION_VAL_SO15] & [ABLATION_TEST_SO15] & [ABLATION_IMP_SO15]$\times$ \\
SO(20) & 20 & [ABLATION_PARAMS_SO20]M & [ABLATION_VAL_SO20] & [ABLATION_TEST_SO20] & [ABLATION_IMP_SO20]$\times$ \\
SO(25) & 25 & [ABLATION_PARAMS_SO25]M & [ABLATION_VAL_SO25] & [ABLATION_TEST_SO25] & [ABLATION_IMP_SO25]$\times$ \\
SO(30) & 30 & [ABLATION_PARAMS_SO30]M & [ABLATION_VAL_SO30] & [ABLATION_TEST_SO30] & [ABLATION_IMP_SO30]$\times$ \\
SO(45) & 45 & [ABLATION_PARAMS_SO45]M & [ABLATION_VAL_SO45] & [ABLATION_TEST_SO45] & [ABLATION_IMP_SO45]$\times$ \\
\textbf{SO(50)} & \textbf{50} & \textbf{[ABLATION_PARAMS_SO50]M} & \textbf{[ABLATION_VAL_SO50]} & \textbf{[ABLATION_TEST_SO50]} & \textbf{[ABLATION_IMP_SO50]$\times$} \\
SO(55) & 55 & [ABLATION_PARAMS_SO55]M & [ABLATION_VAL_SO55] & [ABLATION_TEST_SO55] & [ABLATION_IMP_SO55]$\times$ \\
SO(60) & 60 & [ABLATION_PARAMS_SO60]M & [ABLATION_VAL_SO60] & [ABLATION_TEST_SO60] & [ABLATION_IMP_SO60]$\times$ \\
SO(70) & 70 & [ABLATION_PARAMS_SO70]M & [ABLATION_VAL_SO70] & [ABLATION_TEST_SO70] & [ABLATION_IMP_SO70]$\times$ \\
SO(100) & 100 & [ABLATION_PARAMS_SO100]M & [ABLATION_VAL_SO100] & [ABLATION_TEST_SO100] & [ABLATION_IMP_SO100]$\times$ \\
\midrule
SO(50) & 100 & [ABLATION_PARAMS_SO50_K100]M & [ABLATION_VAL_SO50_K100] & [ABLATION_TEST_SO50_K100] & [ABLATION_IMP_SO50_K100]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

Performance improves with gauge group dimension up to SO(50), achieving optimal test PPL of [ABLATION_TEST_SO50]. Beyond SO(50), performance degrades despite increased parameters, suggesting optimization difficulties at larger scales. Notably, SO(50) with $K=100$ (using 2 copies of the fundamental irrep) achieves test PPL [ABLATION_TEST_SO50_K100], outperforming SO(100) with $K=100$.
...
\midrule
\multicolumn{6}{l}{\textit{SO(50), $K=50$, [PARAMS_SO50]M params}} \\
\quad & 1 & 20k & [VFE_ITER1_VAL] & [VFE_ITER1_TEST] & [VFE_ITER1_IMP]$\times$ \\
\quad & \textbf{5} & \textbf{20k} & \textbf{[VFE_ITER5_VAL]} & \textbf{[VFE_ITER5_TEST]} & \textbf{[VFE_ITER5_IMP]$\times$} \\
\midrule
\multicolumn{6}{l}{\textit{SO(20), $K=20$, [VFE_SO20_PARAMS]M params}} \\
\quad & 1 & 2k & [VFE_SO20_1_VAL] & [VFE_SO20_1_TEST] & [VFE_SO20_1_IMP]$\times$ \\
\quad & 2 & 2k & [VFE_SO20_2_VAL] & [VFE_SO20_2_TEST] & [VFE_SO20_2_IMP]$\times$ \\
\quad & 10 & 2k & [VFE_SO20_10_VAL] & [VFE_SO20_10_TEST] & [VFE_SO20_10_IMP]$\times$ \\
\quad & 50 & 2k & [VFE_SO20_50_VAL] & [VFE_SO20_50_TEST] & [VFE_SO20_50_IMP]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

At the optimal scale (SO(50)), increasing VFE iterations from 1 to 5 yields substantial improvement (test PPL [VFE_ITER1_TEST] $\to$ [VFE_ITER5_TEST]). At smaller scale with limited training (SO(20), 2k steps), additional VFE iterations provide modest benefit, suggesting iteration count interacts with model capacity and training budget.
...
\midrule
0.25 & [TEMP_025_VAL] & [TEMP_025_TEST] & [TEMP_025_IMP]$\times$ \\
0.75 & [TEMP_075_VAL] & [TEMP_075_TEST] & [TEMP_075_IMP]$\times$ \\
\textbf{1.00} & \textbf{[TEMP_100_VAL]} & \textbf{[TEMP_100_TEST]} & \textbf{[TEMP_100_IMP]$\times$} \\
1.25 & [TEMP_125_VAL] & [TEMP_125_TEST] & [TEMP_125_IMP]$\times$ \\
1.75 & [TEMP_175_VAL] & [TEMP_175_TEST] & [TEMP_175_IMP]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The model is relatively robust to temperature, with $\kappa_\beta = 1.0$ achieving best performance. Both lower ($\kappa_\beta < 1$) and higher ($\kappa_\beta > 1$) temperatures degrade performance modestly ([TEMP_PPL_DIFF_LOW]--[TEMP_PPL_DIFF_HIGH] PPL).
...
\midrule
\textbf{SO(3)} & \textbf{$\ell=1,4,5,6,7$ (5 irreps)} & \textbf{[EMBED_DIM_SO3]} & \textbf{[PARAMS_SO3_MULTI]M} & \textbf{[PPL_BEST]} & \textbf{---} \\
SO(3) & $\ell=1,2,4,6,7$ (5 irreps, 2-layer) & [IRREP_2L_K] & [IRREP_2L_PARAMS]M & [IRREP_2L_VAL] & --- \\
SO(3) & fundamental ($\ell=1$), $K=60$ & [IRREP_FUND_K] & [IRREP_FUND_PARAMS]M & --- & [IRREP_FUND_TEST] \\
SO(50) & fundamental & 50 & [PARAMS_SO50]M & [PPL_SO50] & [PPL_SO50_TEST] \\
SO(50) & 2$\times$ fundamental & 100 & [ABLATION_PARAMS_SO50_K100]M & --- & [ABLATION_TEST_SO50_K100] \\
\bottomrule
\end{tabular}
\end{table}

The SO(3) model with multiple irreps ($\ell=1,4,5,6,7$) achieves validation PPL [PPL_BEST] with only [PARAMS_SO3_MULTI]M parameters---outperforming both the [PARAMS_SO50]M parameter SO(50) model (PPL [PPL_SO50]) and the parameter-matched standard transformer (PPL [PPL_STD_PARAM]). This demonstrates that richer irrep decompositions are dramatically more parameter-efficient than larger gauge groups with single irreps, achieving better results with [PARAMS_RATIO_SO50]$\times$ fewer parameters. The critical insight is that higher-dimensional SO(3) irreps ($\ell=4,5,6,7$) capture richer geometric structure than larger gauge groups restricted to fundamental representations.
...
\centering
\small
\caption{Training ablation. SO(20), $K=20$, [TRAIN_ABL_PARAMS]M params, context 64.}
\label{tab:ablation_training}
\begin{tabular}{lrrrr}
\toprule
\textbf{Setting} & \textbf{Steps} & \textbf{Val PPL} & \textbf{Test PPL} & \textbf{vs Random} \\
\midrule
$\eta = 0.01$ & 2k & [TRAIN_2K_VAL] & [TRAIN_2K_TEST] & [TRAIN_2K_IMP]$\times$ \\
$\eta = 0.01$ & 4k & [TRAIN_4K_VAL] & [TRAIN_4K_TEST] & [TRAIN_4K_IMP]$\times$ \\
$\eta = 0.01$ & 20k & [TRAIN_20K_VAL] & [TRAIN_20K_TEST] & [TRAIN_20K_IMP]$\times$ \\
$\eta = 0.001$ & 40k & [TRAIN_40K_VAL] & [TRAIN_40K_TEST] & [TRAIN_40K_IMP]$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
\item \textbf{Irrep structure dominates:} SO(3) with multiple irreps ($\ell=1,4,5,6,7$) achieves our best result---validation PPL [PPL_BEST] with only [PARAMS_SO3_MULTI]M params---dramatically outperforming both [PARAMS_SO50]M param SO(50) (PPL [PPL_SO50]) and [PARAMS_STD_PARAM]M param standard transformers (PPL [PPL_STD_PARAM]). This is the central finding: richer irrep decompositions are far more efficient than larger gauge groups.

\item \textbf{Optimal single-irrep capacity:} For single-irrep models, SO(50) with $K=50$ provides the best performance; larger groups show diminishing returns. SO(50) with $K=100$ (2$\times$ fundamental) achieves PPL [ABLATION_TEST_SO50_K100].

\item \textbf{Multiple VFE iterations:} Increasing from 1 to 5 iterations improves test PPL from [VFE_ITER1_TEST] to [VFE_ITER5_TEST] at SO(50) scale.

\item \textbf{P-flow limitations:} The P-flow mechanism (EMA prior updates) showed divergent behavior with SO(50), with validation PPL increasing from [PPL_PFLOW_START] to [PPL_SO50_PFLOW_VAL] over training. Further investigation is needed.

\item \textbf{Temperature robustness:} $\kappa_\beta = 1.0$ is optimal; deviations in either direction degrade performance by [TEMP_PPL_DIFF_LOW]--[TEMP_PPL_DIFF_HIGH] PPL.
