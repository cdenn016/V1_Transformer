...
\textbf{Empirical observation:} Our experiments demonstrate that the gauge VFE learns meaningful language statistics ([IMPROVEMENT\_VFE]$\times$ improvement over random) without positional encoding. This suggests that:
\begin{enumerate}
...
\begin{itemize}
  ...
  \item Total parameters: $\sim$[PARAMS\_VFE]M (95\% in variational embeddings)
\end{itemize}

\subsubsection{Standard Transformer (Parameter-Matched):}
\begin{itemize}
  \item Learned token embeddings: $V \times d$ matrix, $d=$[EMBED\_DIM\_STD\_PARAM]
  \item Dot-product attention: $\text{softmax}(QK^\top/\sqrt{d})V$ with learned projections
  \item Feed-forward MLP: 2-layer network with GELU, hidden dimension [FFN\_DIM\_STD\_PARAM]
  \item Optimization: Adam~\citep{kingma2014adam} with learning rate $\eta = 0.01$
  \item Layers: 6
  \item Total parameters: $\sim$[PARAMS\_STD\_PARAM]M (matched to gauge VFE)
\end{itemize}
...
\begin{itemize}
  ...
  \item Dot-product attention: $\text{softmax}(QK^\top/\sqrt{d})V$ with learned projections
  \item Feed-forward MLP: 2-layer network with GELU, hidden dimension [FFN\_DIM\_STD\_EMBED]
  \item Optimization: Adam with learning rate $\eta = 0.01$
  \item Layers: 6
  \item Total parameters: $\sim$[PARAMS\_STD\_EMBED]M
\end{itemize}
...
Table~\ref{tab:main_results} presents our primary results on WikiText-103 token-level language modeling. We compare the gauge VFE architecture against standard transformers under two conditions: parameter-matched ($\sim$[PARAMS\_VFE]M parameters) and embedding-matched (dimension 100).
...
\midrule
Standard (param-matched) & 6 & [PARAMS\_STD\_PARAM]M & [PPL\_STD\_PARAM] & [LOSS\_STD\_PARAM] & [IMPROVEMENT\_STD\_PARAM]$\times$ \\
Standard (embed-matched) & 6 & [PARAMS\_STD\_EMBED]M & [PPL\_STD\_EMBED] & [LOSS\_STD\_EMBED] & [IMPROVEMENT\_STD\_EMBED]$\times$ \\
\midrule
Gauge VFE (SO(20)) & 1 & [PARAMS\_VFE]M & [PPL\_VFE] & [LOSS\_VFE] & [IMPROVEMENT\_VFE]$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Standard (param-matched) & [EMBED\_DIM\_STD\_PARAM] & 6 & [HEADS\_STD\_PARAM] & Learned MLP ([FFN\_DIM\_STD\_PARAM]) & [PARAMS\_STD\_PARAM]M \\
Standard (embed-matched) & 100 & 6 & [HEADS\_STD\_EMBED] & Learned MLP ([FFN\_DIM\_STD\_EMBED]) & [PARAMS\_STD\_EMBED]M \\
Gauge VFE (SO(20)) & 100 & 1 & 5 & VFE\_dynamic & [PARAMS\_VFE]M \\
\bottomrule
\end{tabular}
\end{table}

The gauge VFE achieves perplexity [PPL\_VFE], representing a [IMPROVEMENT\_VFE]$\times$ improvement over random chance (PPL 50,257). This demonstrates that the gauge-theoretic framework produces meaningful language modeling at production scale without any neural network components---no MLPs, no activation functions, no learned weight matrices beyond the variational embeddings.

The comparison reveals two key findings:

\begin{itemize}
  \item \textbf{Parameter-matched comparison:} With comparable parameter counts ($\sim$[PARAMS\_VFE]M), the standard transformer achieves better perplexity ([PPL\_STD\_PARAM] vs [PPL\_VFE]). The gauge VFE achieves \textbf{[PERCENT\_OF\_STD]\% of the standard's performance} using only geometric structure---a single layer of KL-divergence attention and natural gradient descent, with no learned projections or feed-forward networks.

  \item \textbf{Embedding-matched comparison:} With identical embedding dimension (100), the gauge VFE \textbf{outperforms} the standard transformer ([PPL\_VFE] vs [PPL\_STD\_EMBED] PPL) despite using only 1 layer compared to 6 layers. This suggests that geometric structure can be more parameter-efficient than learned transformations when embedding dimensions are constrained.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Late phase (steps 100k--200k):} Gradual refinement with diminishing returns, reaching final PPL $\approx$ [PPL\_VFE].
\end{itemize}
...
\midrule
Standard Transformer & $\sim$[TIME\_PER\_STEP\_STD]s & $\sim$[TOKENS\_SEC\_STD] & 1$\times$ & $\sim$[TOTAL\_TIME\_STD] \\
Gauge VFE (SO(20)) & $\sim$[TIME\_PER\_STEP\_VFE]s & $\sim$[TOKENS\_SEC\_VFE] & $\sim$[OVERHEAD\_VFE]$\times$ & $\sim$[TOTAL\_TIME\_VFE] \\
\bottomrule
\end{tabular}
\end{table}
...
Notably, this overhead has improved dramatically from our earlier character-level experiments (which showed $\sim$[OVERHEAD\_EARLIER]$\times$ overhead) due to GPU-optimized implementations and diagonal covariance approximations. The current [OVERHEAD\_VFE]$\times$ overhead, while significant, is within the range where the approach becomes practical for research purposes.
...
At convergence, the gauge VFE achieves validation loss [LOSS\_VFE] (PPL [PPL\_VFE]), reducing uncertainty from the random baseline (PPL 50,257) by a factor of [IMPROVEMENT\_VFE]$\times$. This demonstrates that geometric inference provides meaningful language modeling capability without neural network parameters.


\section{Discussion}
\label{sec:discussion}

This work demonstrates that gauge-theoretic transformers without neural networks achieve meaningful language modeling at production scale. Our single-layer SO(20) gauge VFE achieves perplexity [PPL\_VFE] on WikiText-103 ([IMPROVEMENT\_VFE]$\times$ improvement over random), validating that transformer-like behavior emerges from geometric principles alone.
...
\begin{enumerate}
  ...
  \item \textbf{Computational feasibility:} The implementation achieves $\sim$[TOKENS\_SEC\_VFE] tokens/second ($\sim$[OVERHEAD\_VFE]$\times$ overhead vs standard), a dramatic improvement over earlier prototypes, demonstrating that gauge-theoretic inference is practically trainable.
\end{enumerate}
