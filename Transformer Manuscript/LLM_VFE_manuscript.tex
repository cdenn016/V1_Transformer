...
\toprule
\textbf{Covariance} & \textbf{P-flow} & \textbf{Steps} & \textbf{Context} & \textbf{Val PPL} $\downarrow$ & \textbf{Test PPL} $\downarrow$ & \textbf{vs Random} \\
\midrule
Diagonal & No & 200k & 256 & [PPL-SO50-DIAG-VAL] & [PPL-SO50-DIAG-TEST] & [IMPROVEMENT-SO50-DIAG]$\times$ \\
Diagonal & Yes & 150k & 256 & [PPL-SO50-PFLOW-VAL] & --- & [IMPROVEMENT-SO50-PFLOW]$\times$ \\
Diagonal & No & 20k & 128 & [PPL-SO50-20K-VAL] & [PPL-SO50-20K-TEST] & [IMPROVEMENT-SO50-20K]$\times$ \\
Full (non-diagonal) & No & 25k & 128 & [PPL-SO50-FULL-VAL] & [PPL-SO50-FULL-TEST] & [IMPROVEMENT-SO50-FULL]$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The fully-trained SO(50) diagonal model without P-flow (200k steps) achieves validation perplexity [PPL-SO50-DIAG-VAL] (test PPL [PPL-SO50-DIAG-TEST]), representing a [IMPROVEMENT-SO50-DIAG]$\times$ improvement over random chance. However, P-flow training with EMA prior updates (Section~\ref{sec:p_flow}) exhibited divergent behavior: validation perplexity increased from [PPL-PFLOW-START] to [PPL-SO50-PFLOW-VAL] over training, suggesting that the current P-flow implementation may require further tuning or may be incompatible with large gauge groups. The SO(50) results still outperform our SO(20) baseline (PPL [PPL-SO20]), but are surpassed by the compact SO(3) multi-irrep model (PPL [PPL-BEST] with only [PARAMS-SO3-MULTI]M parameters).
...
\begin{figure}[t]
    ...
    \caption{Semantic clustering in $\mu$ for SO(50) with $K=100$. Left: 3D PCA projection of the 190-dimensional gauge frame space $\mathfrak{so}(50)$. Right: 2D projection (PC1 vs PC2). Punctuation (green) forms a tight cluster near the origin, while content words (orange) and function words (purple) separate along PC2. The first three principal components capture $\sim$[PCA-TOTAL-VARIANCE]\% of variance ([PCA-PC1]\% + [PCA-PC2]\% + [PCA-PC3]\%), yet reveal clear linguistic organization. This structure emerges without supervision, purely from variational free energy minimization.}
...
\begin{figure}[t]
    ...
    \caption{Semantic clustering in learned gauge frames for SO(50) with $K=100$. Left: 3D PCA projection of the 190-dimensional gauge frame space $\mathfrak{so}(50)$. Right: 2D projection (PC1 vs PC2). Punctuation (green) forms a tight cluster near the origin, while content words (orange) and function words (purple) separate along PC2. The first three principal components capture $\sim$[PCA-TOTAL-VARIANCE]\% of variance ([PCA-PC1]\% + [PCA-PC2]\% + [PCA-PC3]\%), yet reveal clear linguistic organization. This structure emerges without supervision, purely from variational free energy minimization.}
    \label{fig:gauge_clustering}
\end{figure}
...
\item \textbf{P-flow limitations:} The P-flow mechanism (EMA prior updates) showed divergent behavior with SO(50), with validation PPL increasing from [PPL-PFLOW-START] to [PPL-SO50-PFLOW-VAL] over training. Further investigation is needed.
