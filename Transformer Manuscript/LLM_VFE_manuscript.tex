...
A central question for any novel architecture is whether it exhibits the power-law scaling behavior characteristic of standard transformers~\citep{kaplan2020scaling, hoffmann2022training}. We conducted systematic scaling experiments across SO($d$) gauge groups with $d \in \{2, 5, 10, 20, 30, 40\}$, training each model on identical compute budgets of 102.4M tokens (approximately 88\% of WikiText-103). Table~\ref{tab:scaling_results} summarizes the results.

\begin{table}[h]
\centering
\small
\caption{Scaling behavior across gauge group dimensions. All models trained on 102.4M tokens from WikiText-103 with identical optimization protocols.}
...
Fitting the standard neural scaling law $\text{PPL} \propto N^{\alpha}$ to our results yields an exponent of $\alpha = -0.42 \pm 0.06$ ($R^2 = 0.91$), which falls within the range reported for standard transformers ($\alpha \approx -0.3$ to $-0.5$ for perplexity at fixed compute)~\citep{hoffmann2022training}. This demonstrates that gauge-theoretic transformers exhibit qualitatively similar scaling behavior to conventional architectures, with performance improving as a power law of model size.
...
\begin{itemize}
    \item \textbf{Data-rich} (SO(2)--SO(10)): Tokens/param $> 20$, scaling continues
...
This analysis reveals that WikiText-103 (117M tokens) is insufficient to properly evaluate models larger than approximately SO(10) (3.8M parameters). The plateau is not an architectural limitation but a data bottleneck: larger models require proportionally more training data to realize their capacity.
...
\begin{enumerate}
    ...
    \item \textbf{Scaling exponent parity:} Despite the absolute performance gap, the scaling \emph{exponent} ($\alpha \approx -0.42$) matches standard transformers, indicating that gauge-theoretic architectures will benefit proportionally from increased scale.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Dimensional efficiency:} Parameters scale as $d^{1.63}$ while perplexity improves as $d^{-0.69}$, suggesting diminishing returns at high dimensions. Multi-irrep architectures (Section~\ref{sec:multi_irrep}) may provide more parameter-efficient scaling than increasing $d$ alone.
\end{enumerate}

Figure~\ref{fig:scaling_plots} visualizes the complete scaling analysis, including training dynamics across dimensions and the parameter-perplexity relationship with fitted power laws.

\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{data/seed=6/scaling_figures/scaling_combined.pdf}
    \caption{Neural scaling analysis for gauge-theoretic transformers. \textbf{(a)} Test loss vs.\ SO dimension. \textbf{(b)} Test perplexity vs.\ SO dimension, showing power-law decrease until data saturation. \textbf{(c)} Test perplexity vs.\ parameters with fitted scaling law $N^{-0.42}$. \textbf{(d)} Training curves for all dimensions plotted against tokens seen, demonstrating convergence within the 102.4M token budget. \textbf{(e)} Parameter count by dimension, illustrating the approximately quadratic growth $N \propto d^{1.63}$.}
