...
\textbf{SO(3)} & \textbf{$\ell$=1,4,5,6,7 (5 irreps)} & \textbf{7.8M} & \textbf{183} & \textbf{293} & \textbf{275$\times$} \\
...
The gauge VFE framework achieves perplexity 183--274 across configurations, representing 180--275$\times$ improvement over random chance (PPL 50,257). This demonstrates that gauge-theoretic transformers produce meaningful language modeling at production scale \emph{without any neural network components}---no MLPs, no activation functions, no learned weight matrices beyond the variational embeddings.

The results reveal three key findings:

\begin{itemize}
  \item \textbf{Competitive performance:} Our best gauge VFE configuration---SO(3) with mixed irreducible representations ($\ell=1,4,5,6,7$)---achieves PPL 183, approaching the parameter-matched standard transformer (PPL 178) while using only 7.8M parameters compared to 23.5M.

  \item \textbf{Irrep structure matters:} SO(3) with mixed higher-spin irreps (7.8M params, PPL 183) substantially outperforms SO(50) with a single fundamental irrep (69.1M params, PPL 196). The richness of the representation structure, not the gauge group dimension, primarily determines performance.
...
This work demonstrates that gauge-theoretic transformers without neural networks achieve meaningful language modeling at production scale. Our gauge VFE architectures achieve perplexity 183--274 on WikiText-103 (180--275$\times$ improvement over random), with the best configuration---SO(3) with mixed irreps---approaching the performance of parameter-matched standard transformers (PPL 183 vs 178) while using only 7.8M parameters.
...
The comparison with standard transformers reveals that the geometric approach achieves competitive performance (PPL 183 vs 178) with substantially different parameter allocation---all capacity resides in variational embeddings rather than learned projections and MLPs. The key contribution is demonstrating that attention mechanisms can be derived from and implemented via gauge theory at production scale.
