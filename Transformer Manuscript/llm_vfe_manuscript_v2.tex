...
We implement and validate gauge-theoretic transformers using natural gradient descent on statistical manifolds with multi-head attention defined by Lie group structure. On token-level language modeling (WikiText-103, vocabulary 50,257), our gauge architectures achieve perplexity 183--274 using pure variational inference---without MLPs, activation functions, or learned weight matrices. We systematically study gauge groups from SO(3) to SO(100) and compare against standard transformers under parameter-matched ($\sim$24M) and embedding-matched conditions.

This proof-of-principle establishes that gauge-theoretic attention is not merely mathematically feasible but operationally viable at production scale. Our best configuration---SO(3) with mixed irreducible representations ($\ell=1,4,5,6,7$)---achieves perplexity 183 with only 7.8M parameters, approaching the parameter-matched standard transformer (PPL 178, 23.5M parameters). This represents a 275$\times$ improvement over random chance through geometric structure alone, validating that transformer-like behavior emerges from gauge-theoretic principles. Ablation studies reveal that irreducible representation structure, not gauge group size, primarily determines performance. We conclude that neural architectures are, in this view, computational approximations to a deeper information gauge geometry that transcends the field of machine learning.
\end{abstract}
...
In this report we implement and compare:
    \begin{itemize}
        \item[(i)] \emph{Standard transformer baselines:} Dot-product attention with learned feed-forward networks under parameter-matched and embedding-matched conditions
        \item[(ii)] \emph{Full gauge VFE:} Complete geometric inference with KL-divergence attention and variational free energy minimization via natural gradient descent, systematically studying gauge groups from SO(3) to SO(100) and various irreducible representation structures
    \end{itemize}
...
where $\nabla_\mu \mathcal{F}$ and $\nabla_\Sigma \mathcal{F}$ are standard Euclidean gradients of the free energy functional~\eqref{eq:free_energy_decomposed}. The Fisher metric $\mathscr{G} = \Sigma^{-1}$ for the mean parameters and the symmetric product for covariance parameters ensure that updates remain on the manifold of positive-definite matrices.
...
\end{equation}
for $\xi \in \mathfrak{g}$, or alternatively via the adjoint representation:
\begin{equation}
...
Random baseline (uniform distribution over 50,257 tokens) achieves PPL = 50,257.

\paragraph{Checkpoint selection.} We save checkpoints based on best validation cross-entropy loss. Final test set evaluation uses the checkpoint with lowest validation loss. This standard practice means validation PPL reflects the selected checkpoint's performance, while test PPL provides an unbiased estimate of generalization.
...
Table~\ref{tab:main_results} presents our primary results on WikiText-103 token-level language modeling. We compare gauge VFE architectures against standard transformers, systematically varying gauge group structure from SO(3) to SO(100).

\begin{table}[t]
\centering
\caption{Main results on WikiText-103 token-level modeling. Gauge VFE models use context length 256; standard transformers use context length 128. All models trained for 200,000 steps.}
\label{tab:main_results}
\begin{tabular}{llrrrr}
\toprule
\textbf{Architecture} & \textbf{Configuration} & \textbf{Params} & \textbf{Val PPL} & \textbf{Test PPL} & \textbf{vs Random} \\
\midrule
Random baseline & --- & --- & 50,257 & 50,257 & 1$\times$ \\
\midrule
\multicolumn{6}{l}{\textit{Standard Transformers (with neural networks)}} \\
Standard & param-matched, 6L, d=320 & 23.5M & 178 & --- & 282$\times$ \\
Standard & embed-matched, 6L, d=100 & 5.76M & 260 & --- & 193$\times$ \\
\midrule
\multicolumn{6}{l}{\textit{Gauge VFE (no neural networks)}} \\
\rowcolor{yellow!15}
\textbf{GL(10)} & \textbf{$K{=}90$, 1L, 1 epoch} & \textbf{59M} & \textbf{---} & \textbf{90} & \textbf{558$\times$} \\
SO(3) & $\ell$=1,4,5,6,7 (5 irreps) & 7.8M & 183 & 293 & 275$\times$ \\
SO(3) & $\ell$=1,2,4,6,7 (2 layers) & 6.8M & 247 & --- & 203$\times$ \\
SO(50) & fundamental & 69.1M & 196 & 250 & 256$\times$ \\
SO(50) & 2$\times$ fundamental & 76.6M & --- & 282 & 178$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Architectural comparison. Gauge VFE allocates parameters to variational embeddings rather than neural network weights.}
\label{tab:arch_comparison}
\begin{tabular}{llcccc}
\toprule
\textbf{Architecture} & \textbf{Config} & \textbf{K} & \textbf{Layers} & \textbf{FFN} & \textbf{Params} \\
\midrule
Standard (param-matched) & d=320 & 320 & 6 & Learned MLP & 23.5M \\
Standard (embed-matched) & d=100 & 100 & 6 & Learned MLP & 5.76M \\
\midrule
SO(3) + 5 irreps & $\ell$=1,4,5,6,7 & 51 & 1 & VFE dynamics & 7.8M \\
SO(50) fundamental & fund $\times 1$ & 50 & 1 & VFE dynamics & 69.1M \\
SO(50) 2-head & fund $\times 2$ & 100 & 1 & VFE dynamics & 76.6M \\
\bottomrule
\end{tabular}
\end{table}

The gauge VFE framework achieves perplexity 90--274 across configurations, representing 180--558$\times$ improvement over random chance (PPL 50,257). This demonstrates that gauge-theoretic transformers produce meaningful language modeling at production scale \emph{without any neural network components}---no MLPs, no activation functions, no learned weight matrices beyond the variational embeddings.

The results reveal three key findings:

\begin{itemize}
  \item \textbf{Surpassing standard transformers:} Our best gauge VFE configuration---GL(10) with $K{=}90$ belief dimensions---achieves test PPL 90 with a single layer and 59M parameters, \emph{surpassing} the parameter-matched standard transformer (PPL 178) by a factor of 2$\times$. This result demonstrates that gauge-theoretic attention is not merely competitive but can exceed conventional neural architectures.

  \item \textbf{GL($K$) gauge structure:} Moving from orthogonal SO($N$) to the full general linear group GL($K$) with a lower-dimensional gauge subgroup ($K{=}90$ beliefs with GL(10) transport) yields dramatically better performance than SO(50) (PPL 90 vs 196--293), confirming that the GL($K$) gauge invariance of KL divergence is essential.

  \item \textbf{Single geometric layer:} All gauge VFE results use a single layer compared to 6 layers in standard transformers. The geometric structure of KL-divergence attention and natural gradient descent partially compensates for architectural depth.
\end{itemize}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Standard Transformer (param-matched)}
        \label{fig:train_baseline}
    \end{subfigure}
    \caption{Training and validation loss curves over 200,000 steps on WikiText-103. \textbf{(a)} Gauge VFE (SO(20)) shows steady descent from initial loss $\sim$11 to final validation loss, demonstrating that geometric inference learns meaningful language statistics. \textbf{(b)} Standard transformer (parameter-matched) for comparison.}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{VFE_LLM_Manuscript/train_val_gap_std_24M.png}
        \caption{Standard Transformer (param-matched)}
        \label{fig:gap_baseline}
    \end{subfigure}
    \caption{Train-validation gap over 200,000 training steps on WikiText-103. \textbf{(a)} Gauge VFE (SO(50)) shows the characteristic gap between training VFE loss and validation cross-entropy. \textbf{(b)} Standard transformer (param-matched) for comparison.}
...
\subsection{Ablation: Gauge Group Scaling}

To understand how gauge group structure affects performance, we systematically vary $N$ in SO($N$) while using a single fundamental irreducible representation. Table~\ref{tab:ablation_capacity} shows results for 20,000 training steps with context length 128.

\begin{table}[h]
\centering
\small
\caption{Gauge group scaling ablation. All models use 1 layer, $\kappa_\beta=1$, 20k training steps, context length 128, single fundamental irrep ($K=N$).}
...
\midrule
SO(50), 2 heads & 100 & 76.6M & 334 & 282 & 178$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The results reveal a characteristic scaling pattern: performance improves from SO(3) to SO(50), then degrades for larger groups. The optimal single-irrep configuration is SO(50) with $K=50$, achieving Val PPL 274. Notably, adding a second attention head (SO(50) with $K=100$) improves test performance to PPL 282 despite slightly worse validation PPL, suggesting better generalization with richer structure.

This scaling behavior likely reflects a trade-off between representational capacity (favoring larger $N$) and optimization difficulty (which increases with the dimension of the gauge frame space $\mathfrak{so}(N)$, growing as $N(N-1)/2$). The observation that SO(3) with mixed irreps (Table~\ref{tab:main_results}) outperforms SO(50) with single irreps demonstrates that representational richness---achieved through higher-spin irreducible representations---is more important than gauge group dimension.

\section{Discussion}
\label{sec:discussion}

This work demonstrates that gauge-theoretic transformers without neural networks achieve meaningful language modeling at production scale. Our gauge VFE architectures achieve perplexity 90--274 on WikiText-103 (180--558$\times$ improvement over random), with the best configuration---GL(10) with $K{=}90$ belief dimensions---\emph{surpassing} the parameter-matched standard transformer (test PPL 90 vs 178) using a single layer and 59M parameters trained for one epoch.
...
\begin{enumerate}
  ...
  \item \textbf{Irrep structure determines performance:} SO(3) with mixed higher-spin irreducible representations (7.8M params, PPL 183) substantially outperforms SO(50) with a single fundamental irrep (69.1M params, PPL 196). The richness of the representation structure matters more than the gauge group dimension.

  \item \textbf{Single geometric layer:} Unlike standard transformers requiring multiple layers for effective modeling, the gauge architecture achieves competitive performance with a single layer, suggesting that geometric structure partially compensates for depth.
...
The comparison with standard transformers reveals that the geometric approach \emph{exceeds} standard transformer performance (test PPL 90 vs 178) with substantially different parameter allocation---all capacity resides in variational embeddings rather than learned projections and MLPs. The key contribution is demonstrating that attention mechanisms can be derived from and implemented via gauge theory at production scale, and that the GL($K$) gauge structure of KL divergence provides a decisive advantage over the approximate SO($N$) subgroups explored in earlier experiments.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{VFE_LLM_Manuscript/Symmetric Vacuum.png}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{VFE_LLM_Manuscript/Symmetry Breaking.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{SO50_gauge_frame_clustering_step200000.png}
    \caption{Semantic clustering in learned gauge frames for SO(50) with $K=50$. Left: 3D PCA projection of the 1225-dimensional gauge frame space $\mathfrak{so}(50)$. Right: 2D projection (PC1 vs PC2). Punctuation (green) forms a tight cluster near the origin, while content words (orange) and function words (purple) separate along PC2. The first three principal components capture $\sim$15\% of variance (6.8\% + 4.8\% + 3.3\%), yet reveal clear linguistic organization. This structure emerges without supervision, purely from variational free energy minimization.}
