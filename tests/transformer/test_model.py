...
class TestGaugeTransformerLMCreation:
    ...
    def test_model_config_stored(self, minimal_config, cpu_device):
        """Test model stores config correctly (may add derived keys like gauge_param)."""
        from transformer.core.model import GaugeTransformerLM

        model = GaugeTransformerLM(minimal_config)

        # Model deep-copies config and may add derived fields (e.g. gauge_param).
        # Verify that all original keys are preserved with their values.
        for k, v in minimal_config.items():
            assert k in model.config, f"Key {k!r} missing from model.config"
            assert model.config[k] == v, f"Key {k!r}: expected {v!r}, got {model.config[k]!r}"
