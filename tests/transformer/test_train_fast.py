...
def _make_trainer(minimal_config, cpu_device, config_overrides=None):
    """Create a FastTrainer with minimal model and synthetic data."""
    from transformer.core.model import GaugeTransformerLM
    import tempfile

    config = TrainingConfig()
    # Set checkpoint_dir to a temp directory to avoid NoneType.mkdir error
...
class TestFastTrainerCreation:
    ...
    def test_instantiation(self, minimal_config, cpu_device):
        """FastTrainer instantiates without error."""
        trainer = _make_trainer(minimal_config, cpu_device)
        assert trainer is not None

    def test_optimizer_has_param_groups(self, minimal_config, cpu_device):
        """Optimizer has multiple parameter groups."""
        trainer = _make_trainer(minimal_config, cpu_device)
        groups = trainer.optimizer.param_groups
        assert len(groups) > 1, f"Expected >1 param groups, got {len(groups)}"

    def test_scheduler_created(self, minimal_config, cpu_device):
...
class TestFastTrainerTrainStep:
    ...
    def test_single_step_returns_metrics(self, minimal_config, cpu_device):
    ...
    def test_loss_is_finite(self, minimal_config, cpu_device):
    ...
    def test_model_in_train_mode(self, minimal_config, cpu_device):
...
class TestFastTrainerCheckpoint:
    ...
    def test_save_checkpoint(self, minimal_config, cpu_device):
    ...
    def test_load_checkpoint(self, minimal_config, cpu_device):
