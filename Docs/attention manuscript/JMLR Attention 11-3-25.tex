...
The gauge VFE model achieves test perplexity 121.1 (a 415$\times$ improvement over random chance), demonstrating that the unreduced gauge-theoretic framework---with KL-divergence attention, gauge transport, and natural gradient dynamics on the statistical manifold---is sufficient for non-trivial language modeling. The parameter-matched standard transformer baseline achieves test PPL 65.0, approximately $1.9\times$ lower, reflecting the computational advantages of dot-product attention with learned projections and MLP sublayers. For broader reference, published WikiText-103 results range from PPL $\sim$49 for basic LSTMs \citep{grave2017improving} to PPL $\sim$18 for large optimized transformers \citep{dai2019transformerxl}, though these comparisons should be interpreted cautiously as those systems employ extensive architectural optimizations (adaptive softmax, segment-level recurrence, neural caches) not present in either of our models.

The gauge VFE result is notable given the model's architectural constraints: it contains \emph{no neural network components}---no MLPs, no activation functions, no learned attention projections, and no positional encoding. All computation derives from KL divergences, matrix exponentials for gauge transport, and natural gradient descent on the Gaussian manifold. Closing the gap to optimized baselines is not the goal of this work; rather, these results establish that gauge-covariant variational inference is a \emph{viable} computational substrate for language modeling, supporting the explanatory thesis that transformers succeed because they approximate this underlying mathematical structure.
