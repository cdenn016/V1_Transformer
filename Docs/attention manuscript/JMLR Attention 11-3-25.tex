...
Given agents with Gaussian beliefs connected by gauge transport, the variational free energy for multi-agent inference takes the form:
\begin{equation}
\boxed{
\begin{aligned}
\mathcal{F}[\{q_i\},\{s_i\}]
...
Each term has a clear information-theoretic role: the belief prior $D_{\mathrm{KL}}(q_i \| p_i)$ regularizes toward local expectations; the model prior $D_{\mathrm{KL}}(s_i \| r_i)$ anchors model beliefs to hyperpriors; the alignment terms penalize disagreement between gauge-transported neighbors; and the observation likelihood couples beliefs to external data. The alignment coupling uses the forward KL divergence, which is the unique $f$-divergence that preserves exponential-family closure under linear coupling and yields a consistent dual interpretation for the attention weights (Appendix~C). The alignment weights $\beta_{ij}$ and $\gamma_{ij}$ are non-negative with $\sum_j \beta_{ij} = 1$; their optimal values take softmax form, as derived in Section~\ref{sec:mixture_derivation}.

An alternative derivation of this free energy via auxiliary agreement variables---which provides additional insight into how the pairwise KL terms emerge from a normalized generative model with Gaussian consistency constraints---is given in Appendix~\ref{app:agreement_variables}.
...
The cross-agent alignment term is the forward KL divergence with frame rotation:
\begin{equation}
