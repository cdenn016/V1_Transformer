...
The attention weights derived from this framework take the form (derived in Section~\ref{sec:mixture_derivation} and Appendix~D via two complementary arguments):
\begin{equation}
...
Softmax arises & Directly (Lagrange opt.) & Via max-entropy (Appendix~D) \\
...
The mixture-model derivation in Section~\ref{sec:mixture_derivation} obtains softmax attention weights by direct variational minimization over a categorical latent variable. Here we present a complementary derivation via Jaynes' maximum entropy principle, which arrives at the same result from a different starting point: rather than specifying a generative model, we seek the least-committed attention distribution consistent with a given expected alignment cost.
...
\paragraph{Relationship to the mixture-model derivation.}
The maximum entropy derivation above and the mixture-model derivation in Section~\ref{sec:mixture_derivation} arrive at the same softmax formula via different principles. The mixture model specifies a generative process (source selection via categorical latent $z$) and optimizes the variational posterior directly; the MaxEnt approach instead seeks the least-committed distribution consistent with an expected cost constraint. Their agreement confirms the robustness of the softmax form: it is simultaneously the variational optimum for source-selection inference and the maximum-entropy distribution for a given expected alignment cost. The mixture model additionally yields the non-uniform prior extension (Eq.~\ref{eq:mixture_softmax_general}), connecting attention masks and positional biases to prior beliefs about source availability.
