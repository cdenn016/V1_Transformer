...
We validate the full theory by training $\mathrm{GL}(K)$ gauge transformers on WikiText-103 using only KL divergences, geometry, and gauge transport without any learned attention projections, MLPs, or activation functions. Across configurations varying gauge dimension, context length, and depth, these models achieve test perplexities of 121--152 with emergent semantic structure in the learned gauge frames and beliefs.
\end{abstract}
...
We trained a systematic set of $\mathrm{GL}(K)$ gauge VFE configurations, varying gauge dimension, context length, number of layers, and prior coupling. All use VFE-dynamic mode with evolving covariance ($\Sigma$) and gauge frame ($\phi$), diagonal covariance structure, learnable per-component learning rates, and 50{,}000--75{,}000 training steps. Key configurations include: (1) $\mathrm{GL}(30)$ with context length $N=128$ (1 layer, 49.8M parameters); (2) $\mathrm{GL}(30)$ with $N=64$ (3 layers, trivial gauge $\Omega = I$, 49.8M parameters); (3) $\mathrm{GL}(30)$ with $N=64$ (1 layer, learned gauge, 49.8M parameters); (4) $\mathrm{GL}(20)$ with $K=80$ embedding dimension (1 layer, 92.5M parameters); and (5) $\mathrm{GL}(16)$ with multi-head structure (2 layers, 5 heads, 45.8M parameters). A standard transformer with comparable embedding dimension and an MLP-based FFN (17.6M parameters) serves as a baseline. Full configurations are documented in the code repository.
...
An important caveat is in order. The high correlation between KL-based attention $\beta_{ij} \propto \exp(-\|Q_i - K_j\|^2/\tau)$ and dot-product attention $\alpha_{ij} \propto \exp(Q_i K_j^\top / \sqrt{d_k})$ follows in part from an algebraic identity: when key norms $\|K_j\|$ are approximately constant (as enforced by layer normalization in BERT), the squared Euclidean distance $\|Q_i - K_j\|^2 = \|Q_i\|^2 + \|K_j\|^2 - 2Q_i K_j^\top$ differs from $-2Q_i K_j^\top$ only by terms that are approximately constant under softmax. The non-trivial content of this validation is therefore not the correlation per se---that much is guaranteed by the algebra---but rather three quantitative predictions that the identity alone does not supply: (a) the optimal temperature $\tau \approx 2\sqrt{d_k}$, confirmed empirically to within 19\%, whose value is fixed by the dimensional scaling of dot-product variance in the gauge framework; (b) the existence, sign, and magnitude of a residual key-norm bias $\rho \approx -0.35$, a prediction specific to the gauge-theoretic decomposition that has no counterpart in the algebraic identity; and (c) the per-layer variation in correlation strength, which reflects representational structure learned during training and is not predicted by the mathematical identity alone.
...
Figure~\ref{fig:gl30_training} shows representative training dynamics. All models achieve stable convergence with perplexity dropping from $>$1{,}000 at initialization to 109--148 at convergence (Table~\ref{tab:gl30_results}). For context, a vocabulary of 50{,}257 tokens gives random-chance perplexity $\sim$50{,}000, so these models capture substantial linguistic structure (a 340--462$\times$ improvement over chance). However, a standard transformer baseline of comparable embedding dimension achieves test perplexity 65.0 (Table~\ref{tab:gl30_results}); our models are intended as a proof of concept for the gauge-theoretic framework, not as competitive architectures. Panel (d) shows gradient norms for $\mu$ (belief means), $\Sigma$ (covariances), and $\phi$ (gauge frames) evolving with distinct dynamics; the gauge frame gradients ($\phi$, red) exhibit characteristic oscillations as the model learns optimal frame alignments. Training and validation curves track closely until late training, indicating the gauge-theoretic inductive bias provides reasonable regularization.
...
\begin{figure*}[t]
  ...
  \end{subfigure}
  \caption{Training dynamics for $\mathrm{GL}(30)$ gauge transformers on WikiText-103. (a,b) Total loss and perplexity vs.\ training step. (c) Bits per token. (d) Gradient norms decomposed by component: $\mu$ (beliefs), $\Sigma$ (covariances), $\phi$ (gauge frames). The 3-layer trivial-gauge model converges faster (plateaus by step 25k) while the 1-layer learned-gauge model achieves lower validation perplexity (113.9 vs.\ 125.1) but requires longer training.}
  \label{fig:gl30_training}
\end{figure*}

\subsubsection{Comparison of Gauge Configurations}

Table~\ref{tab:gl30_results} presents the full set of $\mathrm{GL}(K)$ configurations alongside a standard transformer baseline, revealing systematic trade-offs between gauge dimension, architectural depth, context length, and parameter efficiency:
...
\hline
\textbf{Configuration} & \textbf{Group} & \textbf{Layers} & \textbf{$N$} & \textbf{Params} & \textbf{Val PPL} & \textbf{Test PPL} \\
\hline
\multicolumn{7}{l}{\emph{Gauge VFE models (no MLPs, no learned $W_Q$/$W_K$/$W_V$)}} \\
$K\!=\!80$, 4 fund.\ irreps & $\mathrm{GL}(20)$ & 1 & 64 & 92.5M & 108.9 & 121.1 \\
$K\!=\!48$, 3 fund.\ irreps, 5 heads & $\mathrm{GL}(16)$ & 2 & 64 & 45.8M & 124.3 & 124.8 \\
$K\!=\!30$, $N\!=\!128$ & $\mathrm{GL}(30)$ & 1 & 128 & 49.8M & 139.1 & 125.1 \\
$K\!=\!30$, trivial gauge & $\mathrm{GL}(30)$ & 3 & 64 & 49.8M & 125.1 & 135.3 \\
$K\!=\!30$, learned gauge & $\mathrm{GL}(30)$ & 1 & 64 & 49.8M & 113.9 & 151.8 \\
$K\!=\!30$, learnable $\alpha_i$ & $\mathrm{GL}(30)$ & 1 & 64 & 19.6M & 147.7 & 142.9 \\
$K\!=\!30$, $N\!=\!256$ & $\mathrm{GL}(30)$ & 1 & 256 & 19.6M & 117.7 & 147.2 \\
\hline
\multicolumn{7}{l}{\emph{Standard transformer baseline}} \\
MLP + attention & --- & 1 & 64 & 17.6M & 55.4 & 65.0 \\
\hline
\end{tabular}
\caption{$\mathrm{GL}(K)$ gauge VFE language modeling results on WikiText-103 (vocab 50{,}257; random-chance PPL $\approx$ 50{,}000). $N$ denotes context length. All gauge VFE models use KL-divergence attention with no MLPs, activation functions, or learned attention projections. The standard baseline uses a conventional transformer architecture with comparable embedding dimension.}
\label{tab:gl30_results}
\end{table}

Several patterns emerge from this expanded comparison. The best test perplexity among $\mathrm{GL}(K)$ models is 121.1, achieved by the $\mathrm{GL}(20)$ configuration with $K=80$ embedding dimension and 4 fundamental irreps (92.5M parameters). Among the $\mathrm{GL}(30)$ variants, the $N=128$ context-length model achieves the best test perplexity (125.1) despite a higher validation perplexity, indicating that longer context improves generalization. The 3-layer trivial-gauge model generalizes better than the 1-layer learned-gauge model (test PPL 135.3 vs.\ 151.8) despite higher train PPL, suggesting that trivial gauge acts as an implicit regularizer while additional layers compensate for the absent transport. The learnable prior-coupling $\alpha_i$ model (19.6M parameters) achieves test PPL 142.9, demonstrating that the state-dependent precision mechanism from the theory (Section~\ref{sec:residual_connections}) provides a modest but consistent improvement over the fixed $\alpha = 1$ baseline. All gauge VFE models remain $\sim$2$\times$ the perplexity of the standard transformer baseline (test PPL 65.0), consistent with the proof-of-concept intent of these experiments.
...
\paragraph{Standard attention as a degenerate limit.} Our BERT validation (Section~5.2) demonstrates quantitative agreement between gauge-aligned KL attention and standard dot-product attention across 144 heads (mean $r = 0.821$, median $r = 0.889$ at $\tau = 19.0$). While the overall correlation partly reflects the mathematical relationship between squared Euclidean distance and dot product under approximately constant norms, the framework generates non-trivial predictions that are confirmed: the optimal temperature $\tau = 19.0$ matches the dimensional prediction $\tau \approx 2\sqrt{d_k} = 16$ to within 19\%, and the key-norm bias ($\rho = -0.352$, significant in 92.4\% of heads) has both the predicted sign and magnitude. The per-layer correlation structure further reveals that deeper layers (8--11) exhibit higher correlations with KL attention, consistent with observations that semantic representations become more structured in deeper transformer layers.

\paragraph{The unreduced framework as a language model.} As detailed in Section~5.3, our $\mathrm{GL}(K)$ gauge VFE models achieve test perplexities of 121--152 on WikiText-103 (Table~\ref{tab:gl30_results}) using zero MLPs, activation functions, learned attention projections, or positional encodings---retaining only a linear output projection to vocabulary logits. These experiments implement the full framework with non-isotropic covariances, non-trivial gauge transport, and KL-divergence attention rather than the degenerate limits that reduce to standard attention. That this unreduced gauge-theoretic structure alone achieves non-trivial language modeling supports the explanatory claim: if language is a dynamic informational system, and gauge-covariant VFE is the natural mathematics for such systems, then a model built directly from that mathematics should function as a language model.

\paragraph{Architectural depth and gauge expressiveness as substitutes.} The depth--gauge trade-off (Table~\ref{tab:gl30_results}) reveals that additional transformer layers and richer gauge structure are partially interchangeable: the 3-layer trivial-gauge model (test PPL 135.3) compensates for its lack of transport by depth, while the 1-layer $\mathrm{GL}(30)$ with $N=128$ achieves comparable performance (test PPL 125.1) through richer gauge dynamics. The best overall result (test PPL 121.1, $\mathrm{GL}(20)$ with $K=80$) suggests that gauge dimension and embedding capacity jointly determine performance, with these mechanisms serving as complementary routes to the same underlying information processing.

\paragraph{Transport geometry without selective attention.} The uniform-attention experiment (Section~\ref{sec:uniform_attention}) demonstrates that a model with \emph{exactly} uniform attention weights ($\beta_{ij} = 1/N$) throughout training achieves PPL 377 on WikiText-103, with the learning curve still descending. Since the attention entropy remains precisely frozen at $H(\beta) = \log N$ from initialization to convergence, all performance gains are attributable to gauge transport operators $\Omega_{ij}$, covariance dynamics $\Sigma_i$, and belief mean evolution $\mu_i$. This establishes a lower bound on the contribution of transport geometry to language modeling and provides direct empirical evidence for the ``transport geometry over attention weighting'' hypothesis: the \emph{content} of inter-token communication (how beliefs are geometrically transformed) matters more than its \emph{selectivity} (which tokens communicate).

\subsection{Transport Geometry as the Computational Substrate}

A striking observation from the language modeling experiments is that attention weights can converge to approximately uniform distributions while performance continues to improve. This suggests that contextual information is encoded primarily in the transport operators $\Omega_{ij}$ rather than in selective attention weights, and reveals the information flow architecture of the gauge VFE. The uniform-attention experiment (Section~\ref{sec:uniform_attention}) provides the strongest evidence for this claim: with attention entropy frozen at $H(\beta) = \log N$ throughout training (i.e., $\beta_{ij} = 1/N$ \emph{exactly}), the model reduces perplexity from $>$1{,}000 to 377 using transport geometry alone. This is not a model with \emph{weak} selective attention---it is a model with \emph{zero} selective attention that nonetheless captures substantial linguistic structure.

The gauge frames $\phi_i \in \mathbb{R}^{K^2}$ develop rich semantic structure (Section~5.3.2), yet the observation model $p(o \mid \mu_i)$ couples only to the $K$-dimensional belief means $\mu_i$. Gauge-encoded information reaches predictions through an indirect route: the VFE gradient $\partial \mathcal{F}/\partial \mu_i$ contains the transport-mediated alignment term $\sum_j \beta_{ij}(\Omega_{ij}\Omega_{ij}^\top)^{-1}(\mu_i - \Omega_{ij}\mu_j)/\sigma^2$, which drives $\phi$-dependent structure into $\mu$ at each iteration. When attention is exactly uniform ($\beta_{ij} = 1/N$), this transfer depends \emph{entirely} on the transport geometry---the $\Omega_{ij}$ operators---with selective weighting contributing nothing. The model thus implements a pure form of geometric message-passing where the \emph{content} of communication (how beliefs are transformed) carries all information, and the \emph{selectivity} of communication (which agents communicate) is absent. That this suffices for PPL 377 establishes a lower bound on how much of language modeling can be accomplished by transport geometry without any attention-based selection. Increasing the embedding dimension $K$, the number of VFE iterations, or the number of layers all serve to widen or deepen this channel.

This is a structural consequence of gauge symmetry. The observation likelihood $p(o_i \mid \mu_i)$ depends only on $\mu_i$ and not on $\phi_i$ or $\Sigma_i$ because the internal agent dynamics (attention, alignment, transport) are gauge-covariant---they transform consistently under $\mathrm{GL}(K)$ changes of frame---while observations couple agents to external data with a fixed reference frame (the vocabulary). The observation likelihood therefore acts as an explicit symmetry-breaking field (Section~\ref{sec:symmetry_breaking}) that pins beliefs to a definite orientation. Including $\phi_i$ or $\Sigma_i$ in the observation model would introduce gauge-dependent coupling to external data, undermining the separation between internal gauge-covariant dynamics and external symmetry-breaking observations.

Consequently, the output capacity of the model is $K$-dimensional even though the full representational capacity encompasses $K + K^2$ dimensions (beliefs plus gauge frames). Increasing $K$ thus widens the bottleneck through which gauge-encoded information reaches predictions.
...
Our BERT validation tested the flat bundle limit where all frames are globally aligned and the connection is trivial. This is the appropriate limit for comparison with standard transformers, which lack explicit gauge structure, but it does not probe the regime where the framework is most distinctive.
...
Our uniform-attention experiment (Section~\ref{sec:uniform_attention}) provides a partial realization of this vision. In the VFE-dynamic model, the belief means $\mu_i$, covariances $\Sigma_i$, and gauge frames $\phi_i$ are all updated via VFE gradient descent---a local, biologically plausible learning rule that does not require error backpropagation through the computational graph. When combined with a delta rule update for the output projection $W_{\text{out}}$ (a Widrow-Hoff rule $\Delta W = \eta(\text{target} - \text{prediction}) \otimes \mu^\top$), the entire learning pipeline becomes backpropagation-free: beliefs are updated by VFE minimization (fast timescale), gauge frames evolve via $\partial \mathcal{F}/\partial \phi$ (intermediate timescale), and the output mapping adapts via local error-driven learning (slow timescale). The PPL 377 result demonstrates that this multi-timescale VFE dynamics, without any backpropagation, captures substantial linguistic structure---suggesting that the variational free energy principle provides not only an explanatory framework for existing architectures but a potentially viable alternative learning algorithm grounded in information geometry rather than gradient chain rules.
...
The $\mathrm{GL}(K)$ invariance theorem (Theorem~\ref{thm:glk_invariance}) reveals that standard transformers already implement this gauge structure: the learned projections $W_Q, W_K$ are gauge transformations, and $\Omega = W_Q W_K^\top$ is a learned gauge transport. The $\mathrm{GL}(K)$ invariance of KL divergence guarantees that any invertible projection preserves the variational structure, explaining why transformers can learn arbitrary attention patterns. Standard transformer attention is thus recovered as a degenerate limit in which gauge degrees of freedom are absorbed into learned parameters, covariances reduce to scaled identity, and all frames are globally aligned.

Our language modeling experiments validate this perspective by implementing the full unreduced framework---non-isotropic covariances, non-trivial gauge transport, and KL-divergence attention---without taking any of these simplifying limits. That this gauge-theoretic structure alone achieves non-trivial language modeling (best test perplexity $\sim$121 on WikiText-103), using zero MLPs, activation functions, or learned attention projections, supports the view that transformers succeed because they approximate gauge-covariant variational inference over an informational system. The uniform-attention experiment (Section~\ref{sec:uniform_attention}) sharpens this claim: a model with \emph{exactly} uniform attention ($\beta_{ij} = 1/N$) throughout training achieves PPL 377 using only gauge transport operators and VFE belief dynamics, establishing that transport geometry alone---without any selective attention---captures substantial linguistic structure.

The framework further reveals a geometric interpretation of learning itself: the untrained network corresponds to a gauge-symmetric vacuum state whose degeneracy is broken by observations, with training acting as explicit symmetry breaking that selects a preferred orientation in representation space. We anticipate this perspective---that attention is gauge-covariant inference, and learning is symmetry breaking---will find application not only in machine learning but in understanding biological cognition, collective intelligence, and other domains where distributed agents must communicate under geometric constraints.
