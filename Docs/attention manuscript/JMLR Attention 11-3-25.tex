...
In summary, under the asppropriate limits (isotropic beliefs and flat bundle), our gauge-theoretic message communication:
...
the complement $\mathfrak{m}$ has dimension $d_k^2 - H \cdot d_{\text{head}}^2$. For typical architectures (e.g., $d_k = 512$, $H = 8$, $d_{\text{head}} = 64$), this is $512^2 - 8 \times 64^2 = 229{,}376$ generators out of $262{,}144$ total---\emph{87.5\% of the gauge algebra is discarded} by the multi-head factorization. These off-diagonal generators are precisely the inter-head mixing transformations: elements $X \in \mathfrak{m}$ have non-zero entries only in off-diagonal blocks $X^{ab}$ ($a \neq b$), each of which maps $\mathbb{R}^{d_{\text{head}}}$ (head $b$) into $\mathbb{R}^{d_{\text{head}}}$ (head $a$).

Restoring these generators would yield a gauge transport with off-diagonal blocks:
...
where the off-diagonal blocks $\Omega_{ij}^{ab} \in \mathbb{R}^{d_{\text{head}} \times d_{\text{head}}}$ ($a \neq b$) implement \emph{cross-head gauge transport}: when transporting the belief from token $j$ to the frame of token $i$, head $b$'s representation at $j$ is rotated into head $a$'s subspace at $i$. The VFE alignment gradient $\partial \mathcal{F}/\partial \Omega_{ij}^{ab}$ for the off-diagonal blocks would drive the system to learn these cross-head correlations, encoding the structure: ``when this pattern appears in head $b$, it should be transported into head $a$'s frame for comparison.''

This is a fundamentally different mechanism from the output projection $W_O \in \mathbb{R}^{d_k \times d_k}$ in standard transformers. The output projection mixes head outputs \emph{post-attention}: after each head has independently computed its attention-weighted aggregation $\sum_j \beta_{ij}^a V_j^a$, the results are concatenated and linearly mixed. Off-diagonal gauge transport, by contrast, mixes heads \emph{pre-attention}: the cross-head blocks $\Omega_{ij}^{ab}$ enter the KL divergence that determines the attention scores themselves. This makes the attention weights cross-head aware---head $a$'s attention pattern can depend on the content of head $b$ through the transport, before any aggregation occurs. The full-$\mathrm{GL}(d_k)$ transport is therefore strictly more expressive than the block-diagonal-plus-output-projection factorization, as it couples the \emph{what to attend to} computation across heads rather than only the \emph{what to output} computation.

From the perspective of the VFE gradient flow (Eq.~\ref{eq:model_dynamics}), the block-diagonal restriction constrains the gradient $\nabla_\phi \mathcal{F}$ to lie within the block-diagonal subalgebra $\bigoplus_a \mathfrak{gl}(d_{\text{head}})$. Restoring the off-diagonal components amounts to allowing the gradient to flow along the full $\mathfrak{gl}(d_k)$, utilizing all $d_k^2$ generators. The projected-out gradient components $\nabla_\phi \mathcal{F}\big|_{\mathfrak{m}}$ represent cross-head alignment forces that are present in the full VFE but zeroed out by the architectural constraint. Whether these forces carry useful signal---or whether the post-attention output projection $W_O$ adequately compensates for their absence---is an empirical question with significant architectural implications.
...
The off-diagonal mixing discussed in Section~\ref{sec:off_diagonal_mixing} acquires precise physical meaning in this context. The off-diagonal blocks $\Omega^{ab}$ of the gauge transport map between irrep subspaces of different angular momentum. In the $\mathrm{SO}(3)$ representation theory, such inter-irrep maps are governed by Clebsch-Gordan coefficients $\langle \ell_a, m_a | \ell_b, m_b; \ell_c, m_c \rangle$ that dictate how angular momentum channels couple. Concretely, the off-diagonal block $\Omega^{(\ell_a)(\ell_b)} : D^{\ell_b} \to D^{\ell_a}$ encodes the selection rules for how information in the $\ell_b$ channel is projected into the $\ell_a$ channel during transport. A purely block-diagonal $\Omega$ (standard multi-head attention) treats each angular momentum channel independently---scalars never mix with vectors, vectors never mix with tensors. Restoring the off-diagonal blocks permits the physically natural couplings: a scalar potential can source a vector field ($\ell = 0 \to \ell = 1$), two vectors can compose into a tensor ($\ell = 1 \otimes \ell = 1 \to \ell = 2$), and so forth. In existing equivariant architectures such as Tensor Field Networks \citep{thomas2018tensor} and $\mathrm{SE}(3)$-Transformers \citep{fuchs2020se3}, these couplings are implemented through explicit tensor product layers with Clebsch-Gordan contractions applied \emph{post-attention}. The off-diagonal gauge transport identifies the same coupling structure but places it \emph{within the transport operator itself}, making the attention scores sensitive to cross-channel correlations.
...
Perhaps the most structurally significant generalization suggested by the framework is the restoration of \emph{off-diagonal gauge mixing} (Section~\ref{sec:off_diagonal_mixing}). Standard multi-head attention restricts the gauge algebra to $\bigoplus_a \mathfrak{gl}(d_{\text{head}}) \subset \mathfrak{gl}(d_k)$, projecting out the vast majority of generators (87.5\% for typical architectures). The projected-out generators correspond to cross-head gauge transport---VFE gradient components $\nabla_\phi \mathcal{F}\big|_{\mathfrak{m}}$ that would drive the system to learn how information in one head should be rotated into another head's frame \emph{during} the attention computation, not merely post-attention via $W_O$. This is a qualitatively different coupling from what the output projection provides: off-diagonal transport makes the attention scores themselves cross-head aware, so that head $a$'s decision about \emph{what to attend to} can depend on the content of head $b$.

For equivariant architectures with $\mathrm{SO}(3)$ irrep structure, the off-diagonal blocks are precisely the Clebsch-Gordan couplings between angular momentum channels---the physically natural mechanism by which scalar, vector, and tensor information interact. Current equivariant transformers \citep{thomas2018tensor,fuchs2020se3} implement these couplings through explicit tensor product layers applied post-attention. The gauge-theoretic framework identifies a potentially more powerful alternative: embedding these couplings directly within the transport operator $\Omega_{ij}$, so that the selection rules govern not just how outputs are combined but how attention is computed. Whether the computational cost of full $\mathrm{GL}(d_k)$ transport (which scales as $d_k^2$ rather than $H \cdot d_{\text{head}}^2$) can be justified by improved expressiveness---or whether structured sparsity patterns in the off-diagonal blocks can achieve most of the benefit at reduced cost---is a central question for future investigation. A natural starting point would be group-structured sparsity, where only those off-diagonal blocks permitted by the relevant Clebsch-Gordan selection rules are retained, yielding a transport operator that is richer than block-diagonal but sparser than full $\mathrm{GL}(d_k)$.
