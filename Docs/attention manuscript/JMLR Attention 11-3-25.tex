...
\begin{abstract}
We present a unified gauge-theoretic formulation of attention and message communication in multi-agent Bayesian systems performing variational inference. Each agent is modeled as a smooth local section of an associated bundle with statistical manifold fiber over a noumenal base manifold $\mathcal{C}$. A central theoretical contribution is the proof that KL divergence---and all $f$-divergences---are invariant under the full general linear group $\mathrm{GL}(K)$, not merely orthogonal subgroups. This $\mathrm{GL}(K)$ gauge invariance has profound implications: the learned projection matrices $W_Q, W_K$ in standard transformers are themselves gauge transformations, with $\Omega = W_Q W_K^\top \in \mathrm{GL}(d_k)$ serving as a learned, constant gauge transport. We demonstrate that in the deterministic, constant-gauge limit the generalized attention weights reduce exactly to the canonical transformer rule $\beta_{ij} \propto \softmax(Q_i K_j^\top)$, and backpropagation emerges as gradient descent on the free energy---discarding the natural gradient structure that provides faster convergence. Starting from a generative model of inter-agent message exchange, we derive a generalized variational free-energy functional whose stationary points govern agent dynamics. In the absence of observations, this functional defines a gauge-symmetric vacuum theory in which all agents converge to identical beliefs modulo gauge orbit; introducing observations breaks this symmetry, leading to agent specialization. We validate our framework through simulations of $N=8$ agents with 9-dimensional Gaussian beliefs under $\mathrm{SO}(3)$ gauge transformations, demonstrating convergence to symmetric vacuum states and symmetry breaking under observation/training. The $\mathrm{GL}(K)$ invariance theorem explains why transformers can learn arbitrary attention patterns and suggests that standard deep learning architectures already implement gauge-theoretic inference with gauge structure absorbed into learned parameters.
\end{abstract}
...
Finally, we mathematically show that modern machine learning backpropagation and attention transformers are identically the deterministic, constant-gauge limit of a more general gauge-equivariant theory under natural gradient descent of the variational free energy functional. A key theoretical contribution is the proof that KL divergence is invariant under the full general linear group $\mathrm{GL}(K)$, not merely orthogonal subgroups. This has profound implications: the learned projection matrices $W_Q, W_K$ in standard transformers \emph{are themselves gauge transformations}, with $\Omega = W_Q W_K^\top$ serving as a learned gauge transport. Standard transformers thus already implement $\mathrm{GL}(K)$ gauge-theoretic attention, with the gauge structure absorbed into learned parameters rather than specified \emph{a priori}. Natural gradient descent on the Fisher metric offers theoretical exponential convergence guarantees compared to Euclidean gradient descent; whether this translates to practical speedup in deep learning requires controlled empirical comparison, which we leave to future work. 
...
\hline
\end{tabularx}
\caption{Comparison between full gauge-theoretic formulation and standard transformers. The key insight is that transformers implement $\mathrm{GL}(K)$ gauge structure through learned projections, not that they lack gauge structure.}
...
\subsubsection{Fundamental Theorem: $\mathrm{GL}(K)$ Gauge Invariance of KL Divergence}
\label{sec:glk_invariance}

A crucial observation underlying our entire framework is that the Kullback-Leibler divergence possesses a maximal gauge symmetry: it is invariant under the full general linear group $\mathrm{GL}(K)$.
...
This theorem has profound implications for our framework:
...
This exact correspondence, strengthened by the $\mathrm{GL}(K)$ invariance theorem, reveals several profound insights:
...
\begin{enumerate}
  ...
  \item \textbf{Transformers as $\mathrm{GL}(K)$ Gauge Theory}: Standard transformers are \emph{not} the ``gauge-free'' limit of our framework. Rather, they implement $\mathrm{GL}(K)$ gauge-theoretic attention with the gauge structure absorbed into learned projection matrices $W_Q, W_K$. The composite $\Omega = W_Q W_K^\top \in \mathrm{GL}(d_k)$ is a learned, constant gauge transformation. This explains the expressiveness of attention: transformers learn optimal gauge frames from data.

  \item \textbf{Training as Symmetry Breaking}: Without observations, the free energy is gauge-symmetric: $\mathcal{F}[\{q_i\}] = \mathcal{F}[\{g \cdot q_i\}]$ for all $g \in \mathrm{GL}(K)$. All agents converge to the same magnitude $\|\mu_i\| = \mu^*$ (vacuum state). Observations break this symmetry, forcing agents to specialize---this is learning.
...
This equivalence is exact and derived from first principles. The $\mathrm{GL}(K)$ gauge invariance theorem (Theorem~\ref{thm:glk_invariance}) is essential: it guarantees that the learned projections $W_Q, W_K$ are valid gauge transformations, explaining why transformers can learn arbitrary (invertible) attention patterns. \textbf{Standard machine learning implements $\mathrm{GL}(K)$ gauge-theoretic inference with gauge structure absorbed into learned parameters.}
...
To make this concrete, consider the compact subgroup $G = \mathrm{SO}(3) \subset \mathrm{GL}(3)$. For $\mathrm{SO}(3)$, the representation theory is well-understood: every finite-dimensional representation decomposes into irreducible representations (irreps) labeled by angular momentum $\ell \geq 0$:
\begin{equation}
\rho = \bigoplus_{\ell=0}^{L} n_\ell \cdot D^\ell, \quad \dim(D^\ell) = 2\ell + 1,
\end{equation}
where $n_\ell$ is the multiplicity of irrep $\ell$. The irreps have clear physical meaning:
\begin{itemize}
  \item $\ell = 0$ (dimension 1): \textbf{Scalars}---rotationally invariant quantities
  \item $\ell = 1$ (dimension 3): \textbf{Vectors}---transform as ordinary 3-vectors under rotation
  \item $\ell = 2$ (dimension 5): \textbf{Symmetric traceless tensors}---quadrupole moments
  \item $\ell = 3$ (dimension 7): \textbf{Octupole tensors}, and so on
\end{itemize}

Under this decomposition, gauge transformations act block-diagonally:
\begin{equation}
\rho(R) = \begin{pmatrix}
D^0(R) & 0 & 0 & \cdots \\
0 & D^1(R) & 0 & \cdots \\
...
\end{pmatrix}, \quad R \in \mathrm{SO}(3).
\end{equation}

Each irrep block corresponds naturally to an attention head with \textbf{intrinsic geometric meaning}: the scalar head captures rotationally-invariant features, the vector head captures directional information, and higher-$\ell$ heads capture increasingly complex angular structure.

The $\mathrm{SO}(3)$ Lie algebra $\mathfrak{so}(3)$ has dimension 3, with generators $\{L_x, L_y, L_z\}$ corresponding to infinitesimal rotations about each axis. Within each irrep, these generators have specific matrix representations determined by the Wigner $D$-matrices.

\paragraph{Contrast: Learned vs.\ Geometric Head Structure.}

In standard multi-head attention:
\begin{itemize}
  \item Head dimensions are uniform: $d_{\text{head}} = d_k/H$
  \item Head structure is \textbf{learned} from data with no geometric constraint
  \item The projection matrices $W_Q^a, W_K^a$ can encode arbitrary $\mathrm{GL}(d_{\text{head}})$ transformations
\end{itemize}

In gauge-equivariant attention with a compact group like $\mathrm{SO}(3)$:
\begin{itemize}
  \item Head dimensions are \textbf{non-uniform}: $\{1, 3, 5, 7, \ldots\}$ for $\mathrm{SO}(3)$ irreps
  \item Head structure has \textbf{intrinsic geometric meaning} from representation theory
  \item Transformations are constrained to respect equivariance within each irrep
\end{itemize}

The $\mathrm{GL}(K)$ framework encompasses both: standard transformers learn unconstrained $\mathrm{GL}(d_{\text{head}})$ per head, while equivariant architectures restrict to compact subgroups with richer geometric structure.

\paragraph{Implications for Architecture Design.}

This perspective suggests design principles for geometrically-aware transformers:

\begin{enumerate}
  \item \textbf{Non-Uniform Head Dimensions}: For tasks with known symmetries, head dimensions should match irrep sizes rather than being uniform. For $\mathrm{SO}(3)$-equivariant attention: heads of dimension $1, 3, 5, \ldots$

  \item \textbf{Equivariance Constraints}: Within each irrep block, attention should respect transformation properties. Vector channels ($\ell=1$) should transform covariantly under rotations.

  \item \textbf{Physical Inductive Biases}: For domains with known symmetries (molecular dynamics, 3D vision, physics), gauge-equivariant multi-head attention provides strong inductive biases matching problem structure.
\end{enumerate}

\paragraph{Summary.}

Multi-head attention implements a block-diagonal restriction of the full $\mathrm{GL}(d_k)$ gauge group to $\mathrm{GL}(d_{\text{head}})^H$. For compact subgroups like $\mathrm{SO}(3)$, this block structure has natural geometric interpretation via irreducible representations---each head captures a distinct angular momentum sector with specific transformation properties.

Standard transformers learn the block-diagonal $\mathrm{GL}(d_{\text{head}})$ structure from data, while geometrically-constrained architectures can leverage the richer structure of compact gauge groups. This reveals multi-head attention as a general mechanism for decomposing the embedding space into geometrically coherent subspaces, with the specific decomposition either learned ($\mathrm{GL}$) or prescribed by symmetry (compact subgroups).
...
To validate the complete gauge-theoretic framework beyond attention mechanism correspondence, we trained $\mathrm{GL}(30)$ gauge transformers on WikiText-103 language modeling. These experiments demonstrate variational free energy minimization driving actual language learning, with the gauge structure ($\mu$, $\Sigma$, $\phi$) evolving jointly during training.

\subsubsection{Experimental Configuration}

We trained two $\mathrm{GL}(30)$ configurations on WikiText-103 (102M tokens):
\begin{itemize}
  \item \textbf{3-layer, trivial gauge}: $d_{\text{embed}} = 30$, $d_{\text{gauge}} = 15$, batch size 24
  \item \textbf{1-layer, learned gauge}: $d_{\text{embed}} = 30$, $d_{\text{gauge}} = 30$, batch size 32
\end{itemize}

Both models use VFE-dynamic mode with evolving covariance ($\Sigma$) and gauge frame ($\phi$), diagonal covariance structure, learnable per-component learning rates, and 50,000 training steps. Total parameters: $\sim$50M per model.
...
\begin{enumerate}
  \item \textbf{Stable Convergence}: Both models achieve stable convergence with perplexity dropping from $\sim$2000 (random) to $\sim$125--165 (trained), representing 300--400$\times$ improvement over random chance.
...
Standard transformers require explicit positional encoding---sinusoidal functions or learned vectors added to embeddings~\citep{vaswani2017attention}. The gauge VFE operates \textbf{without any positional encoding}. The causal attention mask (agent $i$ attends only to $j < i$) provides implicit positional information, and the inherent asymmetry of KL divergence may encode relative relationships differently than symmetric dot-product attention. Our results (300--400$\times$ improvement over random) demonstrate that explicit positional encoding is not necessary for meaningful language modeling in this framework.
...
A central finding of this work is that gauge-theoretic transformers \textbf{without neural networks} achieve meaningful language modeling at production scale. Our $\mathrm{GL}(30)$ gauge VFE achieves test perplexity of 135 on WikiText-103---a $370\times$ improvement over random chance (PPL 50,257)---using only geometric operations on probability distributions. These results validate that transformer-like behavior emerges from information-geometric principles alone.

Our empirical results establish several key findings:

\begin{enumerate}
  \item \textbf{Gauge-theoretic attention works at scale:} The framework successfully processes a vocabulary of 50,257 tokens with 64-token context windows, achieving substantial compression of language statistics through geometric structure alone.

  \item \textbf{Minimal neural components:} The gauge VFE contains \emph{zero} MLPs, activation functions, or learned attention projections ($W_Q$, $W_K$, $W_V$). Only a linear output projection to vocabulary logits is retained. All other computation derives from KL divergences, parallel transport via $\Omega = \exp(\phi)$, and natural gradient descent on the Fisher-Rao manifold.

  \item \textbf{No positional encoding:} Unlike standard transformers that require explicit positional information, the gauge architecture operates without any positional encoding. The causal attention mask and inherent KL asymmetry provide sufficient sequential structure.

  \item \textbf{Depth vs.\ gauge expressiveness trade-off:} The 3-layer trivial-gauge model (PPL 135) generalizes better than the 1-layer learned-gauge model (PPL 152), suggesting that architectural depth and gauge expressiveness are partially substitutable. Both substantially outperform random, demonstrating that the geometric framework captures meaningful language structure.

  \item \textbf{Transport geometry over attention weighting:} A striking observation from related experiments is that attention weights can converge to approximately uniform distributions while performance continues to improve. This suggests that contextual information is encoded primarily in the transport operators $\Omega_{ij}$---not in selective attention weights. The gauge framework thus reveals that standard transformer attention may succeed because learned projections $W_Q W_K^\top$ approximate geometric transport, rather than because of the weighting mechanism per se.
\end{enumerate}

These findings support the view that attention and transformer-like computation are not artifacts of neural network engineering but rather natural consequences of variational inference in gauge geometry. The success of the gauge VFE with minimal components suggests that the essential computational structure of transformers lies in the information-geometric relationships between token representations, not in the particular parameterization used to implement them.
...
The central interpretive insight of this work concerns the $\mathrm{GL}(K)$ gauge structure of transformers. The well-known invariance of KL divergence (and all $f$-divergences) under invertible linear transformations has a profound implication for understanding attention: the learned projection matrices $W_Q, W_K$ are themselves gauge transformations in $\mathrm{GL}(d_k)$, with the composite $\Omega = W_Q W_K^\top$ serving as a learned gauge transport. Standard transformers thus already implement $\mathrm{GL}(K)$ gauge-theoretic attention, but with the gauge structure absorbed into learned parameters rather than specified geometrically. This explains why transformers can learn arbitrary (invertible) attention patterns: the variational free energy structure permits any $\Omega \in \mathrm{GL}(K)$, and backpropagation discovers the optimal gauge transformation for the task.

Beyond this theoretical unification, we have demonstrated that gauge-theoretic transformers \emph{without neural networks} achieve meaningful language modeling. Our $\mathrm{GL}(30)$ models (containing no MLPs, activation functions, or learned attention projections) achieve 370$\times$ improvement over random chance on WikiText-103. The learned gauge frames exhibit emergent semantic structure, with tokens clustering by linguistic category in both belief space and transport space. This provides evidence that the essential computational structure of transformers lies in information-geometric relationships, not neural network engineering.

The full framework possesses a vacuum state where all agents flow toward an average belief and gauge frame, mirroring an untrained network. Observations break this $\mathrm{GL}(K)$ symmetry, forcing agents to specialize: this is learning as spontaneous symmetry breaking.

Our framework suggests a novel approach toward unifying the variational free energy principle with machine learning architectures. The $\mathrm{GL}(K)$ gauge symmetry of attention is maximal, encompassing all invertible linear transformations, providing a mathematical foundation for understanding why transformers are so expressive. We anticipate this geometric perspective will find application not only in machine learning but also in understanding biological cognition, collective intelligence, and general informational systems where distributed agents must communicate under geometric constraints.
...
For gauge transport operators $\Omega_{ij}$, numerical monitoring ensures invertibility (the only theoretical requirement under $\mathrm{GL}(K)$). In our simulations using the compact subgroup $\mathrm{SO}(3)$, the function \texttt{safe\_omega\_inv} additionally verifies approximate orthogonality by checking $\|\Omega^\top \Omega - I\|_F$ against a tolerance; if excessive, the matrix is re-orthogonalized via QR decomposition. This orthogonality enforcement is a simulation choice for compact subgroups, not a theoretical necessity---as established in Theorem~\ref{thm:glk_invariance}, KL divergence is invariant under the full $\mathrm{GL}(K)$.
