...
For the purposes of attention computation we note that only the differences in KL values matter (e.g. softmax is invariant). The terms involving $\Omega_{ij}\Omega_{ij}^\top$ will cancel under softmax, leaving the essential structure as:
...
Next, we define the rescaled KL divergence as:
...
The term $\frac{1}{2\sigma^2}\|\mu_i\|^2$ is independent of $j$. Therefore it factors out and cancels between numerator and denominator:
...
The second term $-\frac{1}{2\sigma^2}\|\Omega\mu_j\|^2$ depends only on key $j$ and acts as a key-dependent bias. For general $\Omega \in \mathrm{GL}(d_k)$, this bias becomes approximately constant via two mechanisms:

\textbf{(1) Learned Absorption:}
For $\Omega = W_Q W_K^\top \in \mathrm{GL}(d_k)$, the key bias $\|\Omega\mu_j\|^2 = \|W_Q W_K^\top \mu_j\|^2$ is absorbed into the learned projections. The transformer learns $W_Q, W_K$ such that this term either approximately constant across keys, or provides useful key-dependent modulation. Unlike orthogonal groups where $\|\Omega\mu_j\|^2 = \|\mu_j\|^2$ by construction, $\mathrm{GL}(K)$ allows the transformer to learn the optimal transformation of key norms.
...
Having established that attention emerges from the variational free energy, we now show that gradient descent on this free energy recovers the standard transformer training update, providing a variational interpretation of each component.
