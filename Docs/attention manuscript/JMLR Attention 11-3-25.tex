...
We validate the full theory by training a $\mathrm{GL}(K)$ gauge transformer on WikiText-103 using only KL divergences, geometry, and gauge transport without any learned attention projections, MLPs, or activation functions. The model achieves test perplexity 121 with emergent semantic structure in the learned gauge frames and beliefs.
\end{abstract}
...
To place these results in broader context, Table~\ref{tab:literature_comparison} compares both models against published WikiText-103 benchmarks spanning several architectural families.

\begin{table}[h]
\centering
\begin{tabular}{llcc}
\hline
\textbf{Model} & \textbf{Architecture} & \textbf{Params} & \textbf{Test PPL} \\
\hline
LSTM \citep{grave2017improving} & Recurrent & --- & 48.7 \\
LSTM + Neural Cache \citep{grave2017improving} & Recurrent & --- & 40.8 \\
Gated CNN \citep{dauphin2017language} & Convolutional & --- & 37.2 \\
LSTM + Hebbian \citep{rae2018fast} & Recurrent & 159M & 34.3 \\
AWD-QRNN \citep{merity2018analysis} & Quasi-recurrent & --- & 33.0 \\
Transformer-XL \citep{dai2019transformerxl} & Transformer & 151M & 24.0 \\
Adaptive Input \citep{baevski2019adaptive} & Transformer & 247M & 20.5 \\
Transformer-XL Large \citep{dai2019transformerxl} & Transformer & 257M & 18.3 \\
\hline
\textbf{Standard transformer (ours)} & \textbf{Transformer} & \textbf{17.6M} & \textbf{65.0} \\
\textbf{Gauge VFE (ours)} & \textbf{VFE dynamics} & \textbf{92.5M} & \textbf{121.1} \\
\hline
\end{tabular}
\caption{WikiText-103 test perplexity across architectural families. Published results are drawn from the respective papers. Our gauge VFE model achieves non-trivial perplexity despite using no neural network components beyond a linear output projection. The performance gap relative to optimized LSTMs and transformers reflects both the proof-of-concept nature of these experiments and the architectural constraints of the pure gauge-theoretic framework (no MLPs, no learned attention projections, no positional encoding).}
\label{tab:literature_comparison}
\end{table}

The gauge VFE model's test perplexity of 121.1 is notable given that the model contains \emph{no neural network components}: no MLPs, no activation functions, no learned attention projections, and no positional encoding. All computation derives from KL divergences, matrix exponentials for gauge transport, and natural gradient descent on the Gaussian manifold. The gap relative to optimized LSTMs ($\sim$34) and transformers ($\sim$18--24) reflects the proof-of-concept scale of these experiments. Closing this gap is not the goal of this work; rather, these results establish that gauge-covariant variational inference is a \emph{viable} computational substrate for language modeling, supporting the explanatory thesis that transformers succeed because they approximate this underlying mathematical structure.
