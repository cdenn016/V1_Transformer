...
A crucial observation, which significantly strengthens the theoretical foundation of our framework, is that the Kullback-Leibler divergence possesses a much larger gauge symmetry than initially apparent. While we have presented the framework using $SO(K)$ gauge groups for concreteness, the variational free energy is in fact invariant under the \emph{full general linear group} $\mathrm{GL}(K)$.
