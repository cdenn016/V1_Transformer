...
Finally, we mathematically show that modern machine learning backpropagation and attention transformers are identically the deterministic, constant-gauge limit of a more general gauge-equivariant theory under natural gradient descent of the variational free energy functional. Natural gradient descent on the Fisher metric offers theoretical exponential convergence guarantees compared to Euclidean gradient descent; whether this translates to practical speedup in deep learning requires controlled empirical comparison, which we leave to future work.
...
Our BERT validation (Section~5.2) demonstrated strong quantitative agreement between gauge-aligned and standard transformer attention across 144 heads (mean $r = 0.821$, median $r = 0.889$ at $\tau = 19.0$). The 19\% temperature deviation from the theoretical prediction $\tau = 16$ and the observed key-norm bias ($\rho = -0.352$) are both quantitative validations of the finite-dimensional corrections our framework explicitly predicts.

\subsection{Language Modeling Without Neural Networks}

As detailed in Section~5.3, our $\mathrm{GL}(30)$ gauge VFE achieves test perplexity of 135 on WikiText-103---a $370\times$ improvement over random chance---using \emph{zero} MLPs, activation functions, learned attention projections, or positional encodings. Only a linear output projection to vocabulary logits is retained. The depth vs.\ gauge expressiveness trade-off (Section~5.3.3) reveals that architectural depth and gauge expressiveness are partially substitutable.
...
The central interpretive insight is the $\mathrm{GL}(K)$ gauge structure of transformers: the learned projection matrices $W_Q, W_K$ are themselves gauge transformations with $\Omega = W_Q W_K^\top$ serving as a learned gauge transport (Theorem~\ref{thm:glk_invariance}). Standard transformers thus already implement gauge-theoretic attention with gauge structure absorbed into learned parameters. This explains why transformers can learn arbitrary (invertible) attention patterns, and why backpropagation discovers the optimal gauge transformation for the task.

Our $\mathrm{GL}(30)$ language modeling experiments validate that gauge-theoretic transformers---without MLPs, activation functions, or learned attention projections---achieve meaningful language modeling ($370\times$ improvement over random chance on WikiText-103) with emergent semantic structure in both belief and transport spaces.
