...
\begin{itemize}
  ...
  \item The self-coupling weight $\alpha_i$ can be derived as a state-dependent precision through Bayesian estimation of a latent precision variable (Section~\ref{sec:state_dependent_precision}), with the fixed-$\alpha$ case recovered as a degenerate limit.
\end{itemize}
...
In the variational free energy \eqref{eq:free_energy_final}, the self-coupling term $D_{\mathrm{KL}}(q_i \| p_i)$ carries unit coefficient. This implicitly assumes that the prior precision is known and fixed---that each agent trusts its prior with uniform strength regardless of context. We now show that relaxing this assumption, by treating the prior precision as a latent variable subject to variational inference, yields a state-dependent self-coupling weight $\alpha_i$ that emerges from the same variational principle that produces the alignment weights $\beta_{ij}$.

\subsubsection{Precision as a Latent Variable}

The hierarchical structure of the generative model (Eq.~\ref{eq:hierarchical_prior}) already implies that the effective prior precision depends on model uncertainty. We make this dependence explicit by augmenting the generative model for agent $i$ with a scalar precision parameter $\lambda_i > 0$ that modulates the prior's informativeness:
\begin{equation}
p(k_i \mid m_i, \lambda_i) = \mathcal{N}\!\left(k_i;\, \mu_{p,i},\, \lambda_i^{-1} \Sigma_{p,i}\right), \qquad
p(\lambda_i) = \mathrm{Gamma}(\lambda_i;\, a_0, b_0),
\label{eq:precision_augmented}
\end{equation}
where $a_0 > 0$ and $b_0 > 0$ are hyperparameters of the precision prior. The Gamma distribution is the conjugate prior for the precision of a Gaussian likelihood, ensuring closed-form variational updates. The product $\lambda_i^{-1} \Sigma_{p,i}$ rescales the prior covariance: high $\lambda_i$ corresponds to a tight (informative) prior, low $\lambda_i$ to a diffuse (uninformative) prior.

\subsubsection{Variational Update for Precision}

Under the mean-field factorization $q(k_i, \lambda_i) = q_i(k_i)\,q(\lambda_i)$, the optimal variational posterior over precision takes the conjugate form $q^*(\lambda_i) = \mathrm{Gamma}(\lambda_i;\, a_i, b_i)$ with updated parameters:
\begin{equation}
a_i = a_0 + \frac{d}{2}, \qquad
b_i = b_0 + \frac{1}{2}\left[
\delta_i^\top \Sigma_{p,i}^{-1} \delta_i
+ \mathrm{tr}\!\left(\Sigma_{p,i}^{-1} \Sigma_{q,i}\right)
\right],
\label{eq:precision_posterior_params}
\end{equation}
where $\delta_i = \mu_{q,i} - \mu_{p,i}$ is the belief--prior mismatch and $d = \dim(k_i)$. This is the standard Gamma--Normal conjugate update \citep{bishop2006pattern}. The expected precision under this posterior defines the state-dependent self-coupling weight:
\begin{equation}
\boxed{
\alpha_i \;\equiv\; \langle \lambda_i \rangle_{q(\lambda_i)}
\;=\; \frac{a_0 + d/2}{\displaystyle b_0 + \tfrac{1}{2}\!\left[
\delta_i^\top \Sigma_{p,i}^{-1} \delta_i
+ \mathrm{tr}\!\left(\Sigma_{p,i}^{-1} \Sigma_{q,i}\right)
\right]}.
}
...
\begin{itemize}
\item \textbf{Close to prior} ($\delta_i \approx 0$, $\Sigma_{q,i} \approx \Sigma_{p,i}$): $\alpha_i \approx (a_0 + d/2)/(b_0 + d/2)$, a high value determined by the hyperparameters. The agent trusts its prior and resists displacement by the alignment term.

\item \textbf{Far from prior} (large $\|\delta_i\|_{\Sigma_p^{-1}}^2$): $\alpha_i \to 0$. The prior's influence wanes and the alignment and observation terms dominate. The agent ``releases'' from its prior anchor, allowing contextual information to reshape its belief.
\end{itemize}

The hyperparameter $b_0$ controls the sensitivity of this transition: large $b_0$ yields approximately constant $\alpha_i$ (stiff, uniformly informative prior), while small $b_0$ yields $\alpha_i$ that is highly responsive to belief--prior mismatch (elastic, context-sensitive prior). The hyperparameters $a_0$ and $b_0$ may themselves be learned via empirical Bayes, introducing only two scalar parameters.

\subsubsection{Modified Free Energy with Adaptive Precision}

Substituting the expected precision into the variational free energy, the self-coupling term for agent $i$ becomes:
\begin{equation}
\mathcal{F}_{i,\mathrm{self}} = \alpha_i\, D_{\mathrm{KL}}(q_i \| p_i) + D_{\mathrm{KL}}\!\left(q(\lambda_i) \,\|\, p(\lambda_i)\right),
\label{eq:modified_self_coupling}
\end{equation}
where the second term---the KL divergence between the variational and prior Gamma distributions---penalizes deviation from the prior precision belief, preventing the system from trivially setting $\alpha_i = 0$ to eliminate the self-coupling cost. The full single-agent free energy becomes:
\begin{equation}
\mathcal{F}_i = \alpha_i\,D_{\mathrm{KL}}(q_i \| p_i)
+ \sum_j \beta_{ij}\,D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j)
+ D_{\mathrm{KL}}(q(\lambda_i) \| p(\lambda_i))
...
\end{equation}
where, under the variational EM interpretation, $\alpha_i$ is held fixed during the belief update (coordinate ascent on the joint objective over $(q_i, q(\lambda_i))$): first update $\alpha_i$ from \eqref{eq:state_dependent_alpha} (E-step for precision), then update beliefs with $\alpha_i$ fixed (M-step for beliefs). This separation prevents the pathological $1/\|\delta\|^3$ gradient that would arise from differentiating through $\alpha_i$, while preserving the adaptive precision behavior.
...
In the limit of known (deterministic) precision---$\mathrm{Var}_{q(\lambda_i)}(\lambda_i) \to 0$ with $\langle \lambda_i \rangle = \alpha$ held fixed---the Gamma prior concentrates to a point mass. Concretely, taking $a_0 \to \infty$ and $b_0 \to \infty$ with $a_0/b_0 = \alpha$:
\begin{equation}
\alpha_i \;\xrightarrow{\;\mathrm{Var}(\lambda) \to 0\;}\; \alpha = \text{const},
\label{eq:fixed_alpha_limit}
\end{equation}
recovering the unit-coefficient self-coupling of \eqref{eq:free_energy_final} (with $\alpha = 1$) or, more generally, the fixed scalar weight used in the implementation. In this limit, $D_{\mathrm{KL}}(q(\lambda_i) \| p(\lambda_i)) \to 0$ and the precision KL term vanishes from the free energy.
...
$\alpha_i(\delta_i, \Sigma_{q,i})$ & $\mathrm{Var}(\lambda) \to 0$ & Fixed residual connection \\
...
\end{equation}
where the state-dependent precision \eqref{eq:state_dependent_alpha} simplifies in the deterministic limit to
\begin{equation}
\alpha_i = \frac{a_0 + d/2}{\displaystyle b_0 + \frac{1}{2\sigma_p^2}\|\mu_i - \mu_{\text{prior}}\|^2}.
