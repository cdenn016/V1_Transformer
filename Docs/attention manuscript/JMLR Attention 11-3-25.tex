...
\paragraph{The unreduced framework as a language model.} As detailed in Section~5.3, our $\mathrm{GL}(20)$ gauge VFE achieves test perplexity 121 on WikiText-103 (Table~\ref{tab:gl30_results}) using zero MLPs, activation functions, learned attention projections, or positional encodings---retaining only a linear output projection to vocabulary logits. Under matched BPE tokenization, this surpasses the modified Kneser-Ney 5-gram baseline (test PPL 134.8) by 13.7 PPL, confirming that gauge-covariant variational inference outperforms the strongest classical statistical method in a controlled comparison. This experiment implements the full framework with non-isotropic covariances, non-trivial gauge transport, and KL-divergence attention rather than the degenerate limits that reduce to standard attention. That this unreduced gauge-theoretic structure alone achieves non-trivial language modeling---surpassing the best non-neural baseline without any neural components---supports the explanatory claim: if language is a dynamic informational system, and gauge-covariant VFE is the natural mathematics for such systems, then a model built directly from that mathematics should function as a language model.
