...
\begin{abstract}
We show that transformer attention is variational inference over information sources. Modeling each token as a Gaussian agent on a statistical fiber bundle, we derive the generalized attention weight $\beta_{ij} = \operatorname{softmax}(-D_{\mathrm{KL}}[q_i \| \Omega_{ij} q_j] / \tau)$ from a single variational principle: minimizing the free energy of a mixture-of-sources generative model in which each agent infers which gauge-transported neighbor generated its state. The KL divergence arises exactly---not as an approximation---and the softmax follows from constrained optimization over the categorical source-selection posterior. Non-uniform attention priors $\pi_j$ naturally yield causal masking and positional biases.

Three degenerate limits---isotropic covariances, flat gauge connection, and learned projections---recover the standard transformer rule $\beta_{ij} \propto \operatorname{softmax}(Q_i K_j^\top / \sqrt{d_k})$, identifying attention, temperature scaling, and layer normalization as consequences of variational geometry. The $\mathrm{GL}(K)$ invariance of KL divergence implies that the learned projections $W_Q, W_K$ are gauge transformations, with $\Omega = W_Q W_K^\top$ a learned gauge transport; standard transformers already implement gauge-theoretic attention. Gradient descent on the free energy recovers standard backpropagation, providing a variational interpretation of transformer training.

We validate the full (non-degenerate) theory by training a $\mathrm{GL}(30)$ gauge transformer on WikiText-103 using only KL divergences and gauge transport---no learned attention projections, MLPs, or activation functions. This architecture achieves test perplexity $\sim$135, demonstrating that the gauge-theoretic structure alone carries meaningful computational content. The learned gauge frames develop emergent semantic structure, with tokens clustering by linguistic category.
\end{abstract}
