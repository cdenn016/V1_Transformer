...
The general solution \eqref{eq:mixture_softmax_general} with non-uniform priors $\pi_k$ has several immediate applications. Setting $\pi_k = 0$ for $k > i$ prohibits agent $i$ from attending to future agents, recovering the standard causal attention mask. This arises naturally as a prior constraint on which sources are available, rather than as an ad hoc modification of the softmax. Similarly, a prior $\pi_k \propto \xi(|i - k|)$ (for some decreasing function $\xi$) encodes a preference for attending to nearby agents, providing a mechanism for positional information. Furthermore, making $\pi_k$ a learnable parameter yields an additive bias $\log \pi_k$ in the pre-softmax logits which is equivalent to relative position biases used in architectures such as ALiBi and T5. In this manner we find several ad-hoc architectural choices have principled information geometric origin.
...
\item $\mathcal{F}$ (free energy) $\leftrightarrow$ $\mathcal{L}$ (loss + regularization), 
\item attention weights $\beta_{ij}$ $\leftrightarrow$ transformer attention $\alpha_{ij}$, 
\item observations $-\log p(o \mid \mu)$ $\leftrightarrow$ loss function, and symmetry breaking $\leftrightarrow$ training.
\end{itemize}

This correspondence provides a complete variational interpretation of transformer training. The $\mathrm{GL}(K)$ gauge invariance theorem (Theorem~\ref{thm:glk_invariance}) is essential as it guarantees that the learned projections $W_Q, W_K$ are valid gauge transformations, explaining why transformers can learn arbitrary (invertible) attention patterns.
