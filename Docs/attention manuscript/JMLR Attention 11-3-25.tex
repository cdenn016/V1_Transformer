...
The validation proceeds in three phases: (1) single-passage analysis at the natural temperature $\tau = 1.0$, establishing baseline agreement; (2) multi-passage corpus validation across all 105 passages at $\tau = 1.0$, quantifying cross-passage stability via standard errors and confidence intervals for each of the 144 attention heads; and (3) a temperature sweep across the full corpus at 10 values of $\tau$ from 0.1 to 50, identifying the empirically optimal $\tau$ and its consistency with the theoretical prediction $\tau_{\text{opt}} = 2\sqrt{d}$.
...
We validated the gauge-aligned KL attention against standard dot-product attention across all 144 heads of BERT-base-uncased on a corpus of 105 diverse passages. Figure~\ref{fig:temperature_sweep} shows the temperature sweep across the full corpus. The empirical optimum occurs at $\tau = 19.0$, where the grand mean Pearson correlation (averaged across heads and passages) is $\bar{r} = 0.804$ with 95\% confidence interval $[0.771, 0.838]$ and standard error $\mathrm{SE} = 0.015$.

At the natural temperature $\tau = 1.0$ (no rescaling), the multi-passage validation yields grand mean $\bar{r} = 0.686 \pm 0.230$ and grand median $\tilde{r} = 0.776$ across all 144 heads and 105 passages. Even without temperature optimization, 66 of 144 heads (45.8\%) exceed mean $r > 0.8$ and 21 heads (14.6\%) exceed mean $r > 0.9$, with the range of per-head mean correlations spanning $[0.055, 0.999]$.

\paragraph{Cross-passage stability.} The robustness of these results across the 105-passage corpus is a central finding. Standard errors for each head's correlation are uniformly small: mean $\mathrm{SE} = 0.006$, median $\mathrm{SE} = 0.005$, and maximum $\mathrm{SE} = 0.018$, with all 144 heads achieving $\mathrm{SE} < 0.05$ (Figure~\ref{fig:cross_passage_stability}). This demonstrates that the $\alpha$--$\beta$ agreement is a robust property of BERT's learned representations, not an artifact of any particular input passage.

The distribution of per-head correlations (Figure~\ref{fig:correlation_distribution}) reveals substantial heterogeneity consistent with functional specialization. Some heads achieve near-perfect agreement (e.g., Layer~2, Head~0: mean $r = 0.999$, $\mathrm{SE} = 0.0004$), while a minority of heads show weaker correspondence ($r < 0.3$), typically those implementing positional or syntactic attention patterns orthogonal to the KL-based distance metric. All 144 heads achieve $p < 0.001$ significance on 100\% of passages at the optimal temperature, confirming that the agreement is statistically robust across the full head population.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.95\textwidth]{tau_sweep_across_passages.png}
  \caption{
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{correlation_distribution_all_heads.png}
  \caption{
    \textbf{Distribution of per-head correlations across 105 passages.}
    Each head's correlation is its mean $r(\alpha,\beta)$ across the full 105-passage corpus.  The grand mean is $\bar{r} = 0.686$ and grand median is $\tilde{r} = 0.776$ at $\tau = 1.0$; 66 of 144 heads (45.8\%) exceed $r > 0.8$ and 21 heads (14.6\%) exceed $r > 0.9$ even without temperature optimization.  The rightward skew reflects that the majority of heads show strong agreement, with a tail of weakly correlated heads exhibiting specialized (e.g., positional) attention patterns.
  }
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{cross_passage_stability.png}
  \caption{
    \textbf{Cross-passage stability of per-head correlations.}
    Standard errors across the 105-passage corpus are uniformly small (mean $\mathrm{SE} = 0.006$, median $= 0.005$, max $= 0.018$), with all 144 heads achieving $\mathrm{SE} < 0.05$.  This demonstrates that the $\alpha$--$\beta$ agreement is a reproducible property of BERT's learned representations rather than an artifact of specific input text.
  }
...
\begin{tabular}{rccc}
\hline
$\tau$ & Grand mean $\bar{r}$ & 95\% CI & SE \\
\hline
0.1  & 0.654 & $[0.611, 0.692]$ & 0.019 \\
0.5  & 0.672 & $[0.633, 0.707]$ & 0.019 \\
1.0  & 0.686 & $[0.640, 0.719]$ & 0.019 \\
2.0  & 0.705 & $[0.667, 0.741]$ & 0.019 \\
5.0  & 0.743 & $[0.704, 0.777]$ & 0.018 \\
10.0 & 0.783 & $[0.749, 0.813]$ & 0.016 \\
15.0 & 0.800 & $[0.777, 0.829]$ & 0.016 \\
\textbf{19.0} & \textbf{0.804} & $\mathbf{[0.771, 0.838]}$ & \textbf{0.015} \\
25.0 & 0.801 & $[0.773, 0.825]$ & 0.015 \\
50.0 & 0.749 & $[0.718, 0.779]$ & 0.014 \\
\hline
\end{tabular}
\caption{Temperature sweep results across 105 passages and 144 heads. The grand mean correlation $\bar{r}$ is the mean across all heads of the per-head cross-passage mean correlation. The optimum at $\tau = 19.0$ is consistent with the theoretical prediction $\tau_{\text{opt}} = 2\sqrt{d} = 16$ for $d = 64$. Note the broad plateau from $\tau = 15$ to $\tau = 25$, indicating the optimum is not sharply tuned.}
\label{tab:tau_sweep}
\end{table}
...
The multi-passage analysis confirms this effect (Figure~\ref{fig:key_norm_bias}). For dot-product attention ($\alpha$), the mean absolute correlation between key norms $\|K_j\|^2$ and per-token attention weight is $\overline{|\rho|} = 0.139$ across all 144 heads, while for KL-based attention ($\beta$) at $\tau = 1.0$, the corresponding value is $\overline{|\rho|} = 0.164$. Both metrics aggregate across the full 105-passage corpus, providing robust cross-passage estimates. Although these corpus-averaged magnitudes are modest, individual heads show strong key-norm effects (e.g., Layer~1, Head~6 has $\overline{\rho}_\alpha = 0.889$, reflecting a head where key-norm variation dominates the attention pattern), and the cross-passage per-head key-norm correlations (Figure~\ref{fig:keynorm_cross_passage}) confirm that the sign and magnitude of the bias are reproducible across inputs.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.92\textwidth]{key_norm_bias_analysis.png}
  \caption{
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{keynorm_cross_passage.png}
  \caption{
...
Table~\ref{tab:gl30_results} presents our best $\mathrm{GL}(K)$ gauge VFE model alongside a standard transformer baseline trained on the same data.
...
\hline
\end{tabular}
\caption{Language modeling on WikiText-103 (vocab 50{,}257; random-chance PPL $\approx$ 50{,}000). The gauge VFE model uses KL-divergence attention with no MLPs, activation functions, or learned attention projections ($W_Q$, $W_K$, $W_V$); only a linear output projection to vocabulary logits is retained. The standard transformer baseline uses a conventional architecture with dot-product attention and MLP layers.}
\label{tab:gl30_results}
\end{table}

The gauge VFE model achieves test perplexity 121.1 (a 415$\times$ improvement over random chance), demonstrating that the unreduced gauge-theoretic framework---with KL-divergence attention, gauge transport, and natural gradient dynamics on the statistical manifold---is sufficient for non-trivial language modeling. The standard transformer baseline achieves test PPL 65.0, approximately $1.9\times$ lower, reflecting the computational advantages of dot-product attention with learned projections and MLP sublayers.

To contextualize the gauge VFE result, we note that the strongest purely non-neural baseline on WikiText-103 is the Kneser-Ney 5-gram model (KenLM), which achieves test perplexity $\sim$153--156 \citep{merity2017pointer}.\footnote{The KN-5 perplexity reported by \citet{merity2017pointer} uses word-level tokenization with a $\sim$267K vocabulary, whereas our model uses byte-pair encoding with a 50,257-token vocabulary. Because perplexity is computed per-token and a larger vocabulary makes next-token prediction harder (random-chance PPL scales with vocabulary size), the KN-5 figure evaluated on its larger vocabulary is not directly commensurable with ours. If anything, this difference makes the comparison \emph{conservative}: re-evaluating KN-5 on the same BPE vocabulary would likely yield a lower perplexity for the $n$-gram baseline, narrowing the gap. The qualitative conclusion---that gauge VFE outperforms the strongest non-neural method---would need to be confirmed under matched tokenization for a precise quantitative claim.} The gauge VFE model (PPL 121.1) \emph{surpasses} this classical baseline despite sharing its key constraint: neither model contains neural network components (MLPs, activation functions, or learned projections). The gauge VFE achieves this improvement through an entirely different mechanism---variational inference over gauge-transported Gaussian beliefs rather than count-based smoothing---suggesting that the geometric structure of the framework captures statistical regularities inaccessible to $n$-gram models. For reference, neural language models on WikiText-103 range from PPL $\sim$49 for basic LSTMs \citep{grave2017improving} to PPL $\sim$18 for large optimized transformers \citep{dai2019transformerxl}, establishing the performance ceiling for architectures with full neural components.

All gauge VFE computation derives from KL divergences, matrix exponentials for gauge transport, and natural gradient descent on the Gaussian manifold. Closing the gap to neural baselines is not the goal of this work; rather, these results establish that gauge-covariant variational inference is a \emph{viable} computational substrate for language modeling---one that already outperforms the best classical statistical methods---supporting the explanatory thesis that transformers succeed because they approximate this underlying mathematical structure.
...
\paragraph{Standard attention as a degenerate limit.} Our BERT validation (Section~5.2), conducted across 105 diverse passages and all 144 heads of BERT-base-uncased, demonstrates quantitative agreement between gauge-aligned KL attention and standard dot-product attention. At the optimal temperature $\tau = 19.0$, the grand mean Pearson correlation is $\bar{r} = 0.804$ (95\% CI $[0.771, 0.838]$), and even at the natural temperature $\tau = 1.0$, the grand median reaches $\tilde{r} = 0.776$ with 66/144 heads exceeding $r > 0.8$. The cross-passage standard errors are uniformly small (mean SE $= 0.006$, max SE $= 0.018$), confirming that these results are reproducible across diverse inputs. While the overall correlation partly reflects the mathematical relationship between squared Euclidean distance and dot product under approximately constant norms, the framework generates non-trivial predictions that are confirmed: the optimal temperature $\tau = 19.0$ matches the dimensional prediction $\tau \approx 2\sqrt{d_k} = 16$ to within 19\%, the key-norm bias has the predicted (predominantly negative) sign for KL-based attention, and the per-layer correlation structure reveals that deeper layers (8--11) exhibit higher correlations with KL attention, consistent with observations that semantic representations become more structured in deeper transformer layers.

\paragraph{The unreduced framework as a language model.} As detailed in Section~5.3, our $\mathrm{GL}(20)$ gauge VFE achieves test perplexity 121 on WikiText-103 (Table~\ref{tab:gl30_results}) using zero MLPs, activation functions, learned attention projections, or positional encodings---retaining only a linear output projection to vocabulary logits. This experiment implements the full framework with non-isotropic covariances, non-trivial gauge transport, and KL-divergence attention rather than the degenerate limits that reduce to standard attention. That this unreduced gauge-theoretic structure alone achieves non-trivial language modeling supports the explanatory claim: if language is a dynamic informational system, and gauge-covariant VFE is the natural mathematics for such systems, then a model built directly from that mathematics should function as a language model.
...
Our BERT validation tested the flat bundle limit where all frames are globally aligned and the connection is trivial, validated across 105 diverse English passages with cross-passage standard errors confirming reproducibility. This is the appropriate limit for comparison with standard transformers, which lack explicit gauge structure, but it does not probe the regime where the framework is most distinctive. While the multi-passage corpus provides substantial statistical power (all 144 heads with $\mathrm{SE} < 0.02$), the validation is currently restricted to a single model (BERT-base-uncased) and English-language text; extending to other architectures (GPT, LLaMA), model scales, and languages remains future work.
