...
For $\Omega = W_Q W_K^\top \in \mathrm{GL}(d_k)$, the key bias $\|\Omega\mu_j\|^2 = \|W_Q W_K^\top \mu_j\|^2$ is absorbed into the learned projections. The network learns $W_Q, W_K$ such that this term either approximately constant across keys, or provides useful key-dependent modulation. Unlike orthogonal groups where $\|\Omega\mu_j\|^2 = \|\mu_j\|^2$ by construction, $\mathrm{GL}(K)$ allows the network to learn the optimal transformation of key norms.
...
\end{equation}
where $\mathfrak{sl}(K)$ consists of traceless matrices (volume-preserving transformations) and $\mathbb{R}$ is the trace component (overall scaling). The key-norm bias $\|\Omega\mu_j\|^2$ is sensitive to the $\mathbb{R}$ component such that if $\Omega$ has a nontrivial scaling factor $\det(\Omega) \neq 1$, then $\|\Omega\mu_j\|$ varies with $\|\mu_j\|$, introducing a $j$-dependent bias. Layer normalization, which centers and rescales $\mu_j$ to have zero mean and unit variance per component, effectively projects the representation onto a  sub-manifold where the $\mathbb{R}$ (trace/scale) degree of freedom has been quotiented out. This restricts the effective gauge group from $\mathrm{GL}(K)$ to $\mathrm{SL}(K)$ (modulo translations) for the purpose of attention computation, eliminating the source of key-norm bias at its algebraic origin. In this view, layer normalization is not merely an empirical stabilization technique but a projection onto the appropriate gauge subgroup for frame-independent attention.
