...
A central theoretical contribution is the $\mathrm{GL}(K)$ gauge invariance theorem (Theorem~\ref{thm:glk_invariance}), which establishes that KL divergence---and all $f$-divergences---are invariant under the full general linear group, not merely orthogonal subgroups. This result has profound implications for understanding transformer attention: the learned projection matrices $W_Q, W_K$ are themselves gauge transformations in $\mathrm{GL}(d_k)$, with the composite $\Omega = W_Q W_K^\top$ serving as a learned gauge transport. Standard transformers thus already implement $\mathrm{GL}(K)$ gauge-theoretic attention, but with the gauge structure absorbed into learned parameters rather than specified geometrically. This explains why transformers can learn arbitrary (invertible) attention patterns: the variational free energy structure permits any $\Omega \in \mathrm{GL}(K)$, and backpropagation discovers the optimal gauge transformation for the task.

The full framework possesses a vacuum state where all agents flow toward an average belief and gauge frame (embedding), mirroring machine learning without training. Agent observations break this $\mathrm{GL}(K)$ symmetry, flowing to unique vectors $(\mu)$ that are equivalent under gauge transformation. Free energy principle observations behave as a machine learning loss function, and training is a variational natural gradient descent of the generalized free energy we have derived.
...
Our framework suggests a novel approach toward unifying the variational free energy principle with machine learning architectures by extending the free energy principle to include an agent-agent communication term, offering the potential for exponential speed-ups to convergence during training. The $\mathrm{GL}(K)$ invariance theorem provides the mathematical foundation for this unification: it reveals that the gauge symmetry of attention is maximal, encompassing all invertible linear transformations. We anticipate this approach will find application not only in machine learning and variational inference communities but also in linguistics, psychology, sociology, physics, philosophy, and other informational research domains.
