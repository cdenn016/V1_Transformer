...
Since we are considering 0-dimensional base manifolds there exist no gauge connections $A_\mu$ or field strengths $F_{\mu\nu}$, simplifying the geometry considerably. This is a significant reduction: curvature, holonomy, and all spatial derivative structure are absent. What remains is the algebraic structure of group-valued frame transformations $\Omega_{ij} = \exp(\phi_i)\exp(-\phi_j)$ acting on statistical fibers. We emphasize that this algebraic structure already suffices to recover transformer attention (as shown below), but the richer differential-geometric content---gauge curvature encoding position-dependent interactions, holonomy capturing path-dependent information transport---is available in the full theory on higher-dimensional base manifolds and represents a direction for future work (see Section~6.3).
