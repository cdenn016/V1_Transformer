...
\begin{figure*}[t]
  ...
  \end{subfigure}
  \caption{Training dynamics for exploratory $\mathrm{GL}(30)$ gauge transformer configurations on WikiText-103, illustrating representative convergence behavior. (a,b) Total loss and perplexity vs.\ training step. (c) Bits per token. (d) Gradient norms decomposed by component: $\mu$ (beliefs), $\Sigma$ (covariances), $\phi$ (gauge frames). The 3-layer trivial-gauge model converges faster (plateaus by step 25k) while the 1-layer learned-gauge model achieves lower validation perplexity (113.9 vs.\ 125.1) but requires longer training. Quantitative results in Table~\ref{tab:glk_results} are from the final $\mathrm{GL}(20)$, $K=80$ configuration.}
...
\begin{figure*}[t]
  ...
  \end{subfigure}
  \caption{Emergent semantic organization in learned representations from an exploratory $\mathrm{GL}(30)$ configuration. \textbf{(a)} PCA of belief means $\mu \in \mathbb{R}^{30}$ shows punctuation tokens (green) spreading along principal components while content words (orange) and letters (red) form tighter clusters. \textbf{(b)} PCA of gauge frame coefficients $\phi \in \mathfrak{gl}(30) \cong \mathbb{R}^{900}$ reveals clear separation between linguistic categories: punctuation clusters distinctly from content words and letters. This structure emerges purely from the language modeling objective---no category labels are provided during training. Qualitatively similar structure is observed in the $\mathrm{GL}(20)$ configuration reported in Table~\ref{tab:glk_results}.}
  \label{fig:glk_semantic}
\end{figure*}

\paragraph{Belief Embeddings.} In the $\mathrm{GL}(30)$ exploratory run (Figure~\ref{fig:glk_semantic}a), the learned belief means $\mu \in \mathbb{R}^{30}$ show that punctuation tokens spread along the principal components while content words form a tighter central cluster. The first three principal components capture approximately 49\% of the variance, indicating moderate low-dimensional structure.
...
\paragraph{Standard attention as a degenerate limit.} The BERT validation (Section~5.2) demonstrates quantitative agreement between gauge-aligned KL attention and standard dot-product attention across 105 diverse passages and all 144 heads ($\bar{r} = 0.804$ at optimal $\tau$; Table~\ref{tab:tau_sweep}). Beyond the correlation itself---which partly reflects the algebraic relationship between squared distance and dot product under approximately constant norms---the framework generates non-trivial predictions that are confirmed: the optimal temperature matches the dimensional prediction $\tau \approx 2\sqrt{d_k}$ to within 19\%, the key-norm bias has the predicted sign, and deeper layers exhibit stronger correlations consistent with increasingly structured representations.
