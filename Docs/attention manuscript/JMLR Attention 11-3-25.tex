...
This invariance has several important implications for our framework. First, it establishes that the natural gauge group for KL-based attention is the full $\mathrm{GL}(K)$, which is the maximal symmetry group preserving the divergence. Moreover, the proof extends to all $f$-divergences: for any convex function $f$ with $f(1) = 0$,
\begin{equation}
D_f(\Omega_* P \| \Omega_* Q) = D_f(P \| Q), \quad \forall\, \Omega \in \mathrm{GL}(K),
\end{equation}
since the Jacobian factors $|\det\Omega|$ from the density transformation cancel in the ratio $p(x)/q(x)$. From a practical standpoint, this means that transport operators need only satisfy $\det(\Omega_{ij}) \neq 0$ (invertibility), not $\Omega_{ij}^\top\Omega_{ij} = I$ (orthogonality), eliminating the need for computationally expensive re-orthogonalization procedures. Finally, as we shall demonstrate, the query and key projection matrices $W_Q, W_K \in \mathrm{GL}(d_k)$ in standard transformers are themselves gauge transformations. The ``constant gauge limit'' corresponds not to the absence of gauge structure, but to a constant gauge transformation $\Omega = W_Q W_K^\top$ applied uniformly across all token pairs.
...
$\Omega^q_{ij}$ & Connection $\mathcal{B}_{q_j} \to \mathcal{B}_{q_i}$ & $\mathrm{GL}(K_q)$ \\
$\tilde\Omega^p_{ij}$ & Connection $\mathcal{B}_{p_j} \to \mathcal{B}_{p_i}$ & $\mathrm{GL}(K_p)$ \\
...
These priors are \textbf{local}---they live in each agent's own gauge frame and need not be related across agents until transported via the gauge connection.
...
\end{equation}
mapping agent $j$'s latent into agent $i$'s frame. In the transformer limit, $\Omega_{ij} = \Omega = W_Q W_K^\top$ becomes a constant learned gauge transformation (see Section~\ref{sec:glk_invariance}).
...
We now derive the inter-agent alignment terms and their softmax attention weights from first principles, by formulating each agent's inference as source selection over gauge-transported neighbors.

\subsubsection{Mixture-of-Sources Generative Model}

Agent $i$ (the ``query'') seeks to update its belief $q_i(k)$ by attending to a set of source agents $j$ (the ``keys/values''). Agent $i$'s generative model assumes its latent state $k$ was drawn from one of the neighboring agents, selected by a categorical latent variable $z \in \{1, \ldots, N\}$:
\begin{equation}
P(k, z) = P(k \mid z)\, P(z),
\label{eq:mixture_joint}
\end{equation}
where $P(z = j) = \pi_j$ is the prior probability of attending to agent $j$ (the \emph{attention prior}), and $P(k \mid z = j) = \mathcal{N}(k;\, \Omega_{ij}\mu_j,\, \Omega_{ij}\Sigma_j\Omega_{ij}^\top)$ is agent $j$'s belief transported into agent $i$'s gauge frame via $\Omega_{ij}$.
...
We minimize $\mathcal{F}_{\text{align}}$ with respect to $\beta_{ij}$ subject to $\sum_j \beta_{ij} = 1$. The Lagrangian is:
\begin{equation}
...
A temperature parameter $\tau > 0$ is recovered by rescaling the alignment energy: $E_{ik} \to E_{ik}/\tau$, corresponding to scaling the coupling precision in the generative model. The forward KL divergence is the unique $f$-divergence that preserves exponential-family closure under this linear coupling structure and yields a consistent dual interpretation for the attention weights (Appendix~C).

\subsubsection{Non-Uniform Attention Priors and Masking}
\label{sec:nonuniform_priors}

The general solution \eqref{eq:mixture_softmax_general} with non-uniform priors $\pi_k$ has several immediate applications. Setting $\pi_k = 0$ for $k > i$ prohibits agent $i$ from attending to future agents, recovering the standard causal attention mask; this arises naturally as a prior constraint on which sources are available, rather than as an ad hoc modification of the softmax. A prior $\pi_k \propto f(|i - k|)$ for some decreasing function $f$ encodes a preference for attending to nearby agents, providing a principled mechanism for positional information without explicit positional encodings. Making $\pi_k$ a learnable parameter yields an additive bias $\log \pi_k$ in the pre-softmax logits, equivalent to relative position biases used in architectures such as ALiBi and T5. The gauge-theoretic framework reveals these as prior beliefs about source relevance.

\subsection{Full Variational Free Energy}
\label{sec:final_free_energy}

Combining the single-agent free energy \eqref{eq:single_agent_fep} with the alignment terms derived above, and including the model-channel analogue (with weights $\gamma_{ij}$ and transport $\tilde{\Omega}_{ij}$), we obtain the complete gauge-covariant variational free energy:
\begin{equation}
\boxed{
\begin{aligned}
\mathcal{F}[\{q_i\},\{s_i\}]
...
The first two terms are single-agent prior regularization (Section~3.3). The third and fourth terms are the alignment energies derived from the mixture-of-sources model, with softmax attention weights $\beta_{ij}$ and $\gamma_{ij}$. The final term is the observation likelihood. An alternative derivation of how these pairwise KL terms emerge from a normalized generative model with auxiliary agreement variables is given in Appendix~\ref{app:agreement_variables}.

In the full spatial formulation over base manifold $\mathcal{C}$, the attention weights take the form:
\begin{equation}
...
\end{equation}
where $\chi_{ij}(c) = \mathbb{I}_{\mathcal{U}_i \cap \mathcal{U}_j}(c)$ is the overlap indicator (the extreme case $\pi_j = 0$ for non-overlapping agent supports) and $\tau$ is the temperature parameter.
...
This result has a transparent interpretation. When an agent is close to its prior ($D_{\mathrm{KL}}(q_i \| p_i) \approx 0$), $\alpha_i \approx c_0/b_0$ takes a high value determined by the hyperparameters: the agent trusts its prior and resists displacement by the alignment term. Conversely, when an agent is far from its prior (large $D_{\mathrm{KL}}(q_i \| p_i)$), $\alpha_i \to 0$; the prior's influence wanes and the alignment and observation terms dominate. The agent ``releases'' from its prior anchor, allowing contextual information to reshape its belief.
...
Dynamic precision provides the microscopic mechanism through which observations drive the symmetry breaking described in Section~\ref{sec:symmetry_breaking}. The dynamics proceed in three phases. In the \emph{symmetric phase} (vacuum, $o = \emptyset$), all agents converge to $q_i = p_i$, giving $\delta_i = 0$ and maximal $\alpha_i$. The system is ``frozen''---agents are rigidly anchored to priors and resist perturbation. This is the high-precision, high-symmetry fixed point. During the \emph{transition} (when observations are introduced), observations push beliefs away from priors ($\delta_i > 0$), lowering $\alpha_i$, which reduces the restoring force toward the prior. This positive feedback allows the alignment and observation terms to ``capture'' the agent and pull it toward a new equilibrium in a different gauge orbit. Finally, in the \emph{broken phase} (steady state under data), agents settle into specialized configurations. On the slow timescale (Eq.~\ref{eq:model_dynamics}), priors update to match the new beliefs ($\mu_{p,i} \to \mu_{q,i}$, $\delta_i \to 0$), $\alpha_i$ rises again, and the agent ``freezes'' into its new semantic role with restored high precision.
...
We assume \textbf{isotropic covariances} $\Sigma_i = \Sigma_j = \sigma^2 I$ with $\sigma^2 > 0$. This implies that the log-determinant terms cancel ($\log\frac{|\Sigma_j|}{|\Sigma_i|} = 0$) and the trace term simplifies to $\mathrm{Tr}(\Sigma_j^{-1}\Sigma_i) = \mathrm{Tr}(I) = d$.
...
Each term admits a clear interpretation. The first term, $\lambda_p(\mu_i - \mu_{\text{prior}})$, corresponds to weight decay or L2 regularization; in the general theory, $\lambda_p \to \lambda_{p,i} = \alpha_i/\sigma_p^2$ (Eq.~\ref{eq:deterministic_free_energy_adaptive}), giving per-token adaptive regularization---the deterministic analog of the gated residual (Eq.~\ref{eq:gated_update}). The second term, $\sum_j \frac{\beta_{ij}}{\sigma^2}(\mu_i - \Omega\mu_j)$, is attention-weighted message aggregation that pulls $\mu_i$ toward gauge-transported neighbors. The third term, $\sum_j \frac{\partial \beta_{ij}}{\partial \mu_i}\|\mu_i - \Omega\mu_j\|^2/(2\sigma^2)$, represents gradient flow through the attention weights themselves (backpropagation through softmax). The fourth term, $-\frac{\partial \log p(o_i \mid \mu_i)}{\partial \mu_i}$, is the gradient of the loss function (the supervised learning signal).
...
This exact correspondence, strengthened by the $\mathrm{GL}(K)$ invariance theorem, reveals several insights. First, our framework invokes natural gradient descent rather than Euclidean gradient descent. Under strong geodesic convexity of the loss in the Fisher-Rao metric, natural gradient descent can achieve exponential convergence $k = O(\log(1/\varepsilon))$ compared to the $k = O(1/\varepsilon)$ rate of standard gradient descent \citep{amari1998natural}. However, strong geodesic convexity is not guaranteed for neural network losses in general, and the practical convergence rate depends on the specific geometry of the loss landscape. Realizing any speedup also requires (a) efficient computation of the Fisher metric or suitable approximations, and (b) that the additional overhead of KL terms in the variational free energy does not dominate. Our $\mathrm{GL}(30)$ language modeling experiments (Section~5.4) demonstrate stable training dynamics, but controlled comparison against standard backpropagation baselines is needed to quantify any practical advantage. Second, backward gradient flow admits a natural interpretation as variational message-passing: each layer's gradient update is driven by the discrepancy between its current beliefs and the predictions propagated from downstream layers. The chain rule itself is a general property of differentiable composition; the contribution here is the specific identification of each gradient term with a variational quantity (KL divergences, observation likelihoods, gauge-transported predictions). Third, transformer design choices (attention, layer norm, skip connections) can be understood as implementing gauge-theoretic principles---they preserve or restore geometric structure during propagation. Layer normalization, in particular, regulates key norms to satisfy the high-dimensional cancellation condition required for gauge-invariant attention (see Section~\ref{sec:glk_invariance}).
...
\paragraph{Empirical Evidence.} Figure~\ref{fig:key_norm_bias} provides strong confirmation. The Pearson correlation between key norms $\|K_j\|^2$ and average attention received is $\rho = -0.733$ ($p = 1.88 \times 10^{-30}$) for an example head (Layer 0, Head 0), indicating strong bias. The effect is pervasive: the distribution of correlations shows average $\rho = -0.352$ across all heads, with 133 out of 144 heads (92.4\%) exhibiting statistically significant key-norm bias at $p < 0.001$.
