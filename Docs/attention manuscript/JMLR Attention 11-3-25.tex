...
\subsubsection{Gaussian Fiber and $\mathrm{GL}(K)$ Gauge Symmetry}
We restrict to the exponential family of multivariate Gaussian distributions. The fundamental observation (Theorem~\ref{thm:glk_invariance}) is that the KL divergence---and hence the variational free energy---is invariant under the full general linear group $\mathrm{GL}(K)$, not merely orthogonal subgroups. The natural gauge structure is therefore:

\begin{itemize}
  \item Fiber: $\mathcal{B} = \{(\mu, \Sigma) : \mu \in \mathbb{R}^K, \Sigma \in \mathbb{R}^{K \times K}, \Sigma \succ 0\}$ (Gaussian manifold)
  \item Gauge group: $G = \mathrm{GL}(K)$ with action:
\begin{equation}
\rho(\Omega) \cdot (\mu, \Sigma) = (\Omega\mu, \Omega\Sigma\Omega^\top), \quad \Omega \in \mathrm{GL}(K)
\end{equation}
\end{itemize}

The Gaussian manifold has constant negative curvature under the Fisher-Rao metric. While $\mathrm{GL}(K)$ is non-compact and has dimension $K^2$, important compact subgroups include $\mathrm{SO}(K)$ (dimension $K(K-1)/2$) and $\mathrm{O}(K)$.

For computational tractability and physical interpretability, our simulations employ $G = \mathrm{SO}(3)$ acting via irreducible representations on higher-dimensional fibers. This choice is motivated by: (i) the well-understood representation theory of $\mathrm{SO}(3)$; (ii) compact group structure enabling Haar measure averaging; and (iii) physical relevance to rotation-equivariant systems \citep{nakahara2003geometry}\citep{hall2015lie}. However, the theoretical results---particularly the derivation of transformer attention---hold for the full $\mathrm{GL}(K)$ gauge group, which is essential for understanding how learned projection matrices implement gauge structure.
...
\subsubsection{Compact Subgroups and Simulations}
While $\mathrm{GL}(K)$ is the maximal gauge symmetry, compact subgroups such as $\mathrm{SO}(K)$ or $\mathrm{O}(K)$ are useful when:
\begin{itemize}
\item Haar measure averaging is required (e.g., gauge consensus)
\item Finite-dimensional irreducible representations are desired
\item Volume-preserving transformations are physically motivated
\end{itemize}

For computational tractability in our simulations, we employ $G = \mathrm{SO}(3)$ acting via irreducible representations on higher-dimensional fibers. Specifically, we used $d_q = d_p = 9$ with the spin-4 irrep:
\begin{equation}
\rho_q = \rho_p = \ell_4,
\quad
\text{(dimension $2 \cdot 4 + 1 = 9$)}.
\label{eq:simulation_irrep}
\end{equation}
However, all theoretical results---particularly the derivation of transformer attention---hold for the full $\mathrm{GL}(K)$.

\subsubsection{Gauge Structure}
The gauge structure of our framework is built via:

\begin{itemize}
  \item Gauge group: $G = \mathrm{GL}(K)$ (or compact subgroup for simulations)
  \item Frame fields: $\phi_i(c) \in \mathfrak{gl}(K)$ vary spatially
  \item Transport: $\Omega_{ij}(c) = e^{\phi_i(c)}e^{-\phi_j(c)} \in \mathrm{GL}(K)$
  \item Action on fiber: $\rho(\Omega_{ij}) \in \mathrm{GL}(d_K)$
\end{itemize}

In the transformer limit, the gauge structure simplifies in two key ways:

\textbf{(1) Constant gauge transformation:} Rather than agent-dependent $\Omega_{ij}$, transformers use a single learned gauge:
\begin{equation}
\Omega_{ij} = \Omega = W_Q W_K^\top \in \mathrm{GL}(d_k) \quad \forall\, i,j
\end{equation}

\textbf{(2) Flat connection:} The gauge fields become spatially constant:
\begin{equation}
\phi_i(c) = \phi = \text{const} \in \mathfrak{gl}(K) \quad \forall\, i, c
\end{equation}
...
\begin{itemize}
  ...
  \item Gauge transport absorbed into learned projections $W_Q, W_K$
\end{itemize}

Crucially, the transformer limit is \emph{not} gauge-free. The gauge structure persists as $\Omega = W_Q W_K^\top \in \mathrm{GL}(d_k)$---a learned, constant gauge transformation that backpropagation discovers from data.

\subsubsection{Learned Projections $W_Q, W_K$ as $\mathrm{GL}(K)$ Gauge Transformations}
\label{sec:wq_wk_gauge}

A central insight enabled by the $\mathrm{GL}(K)$ invariance theorem is that the learned projection matrices in standard transformers are not merely ``feature extractors''---they \emph{are} gauge transformations. This fundamentally changes our understanding of what transformers learn.

In standard transformer attention, query and key vectors are computed as:
\begin{equation}
Q_i = W_Q^\top \mu_i \in \mathbb{R}^{d_k}, \qquad K_j = W_K^\top \mu_j \in \mathbb{R}^{d_k},
\end{equation}
where $W_Q, W_K \in \mathbb{R}^{d \times d_k}$ are learned projection matrices. In practice, these matrices are full-rank (invertible when $d = d_k$), placing them in $\mathrm{GL}(d_k)$.

The attention score $Q_i K_j^\top = \mu_i^\top W_Q W_K^\top \mu_j$ can be written as:
\begin{equation}
Q_i K_j^\top = \mu_i^\top \,\Omega\, \mu_j, \quad \text{where} \quad \Omega := W_Q W_K^\top \in \mathrm{GL}(d_k).
\end{equation}

This matrix $\Omega$ is precisely a gauge transformation! In the language of our framework:
\begin{itemize}
\item The \emph{query projection} $W_Q$ transforms agent $i$'s embedding into its local gauge frame
\item The \emph{key projection} $W_K$ transforms agent $j$'s embedding into a comparison frame
\item The \emph{composite} $\Omega = W_Q W_K^\top$ is the learned gauge transport from $j$'s frame to $i$'s frame
\end{itemize}

By Theorem~\ref{thm:glk_invariance}, the KL divergence is invariant under such $\mathrm{GL}(K)$ transformations. This explains why transformers can learn arbitrary (invertible) projections: the variational free energy structure permits any $\Omega \in \mathrm{GL}(K)$, and backpropagation discovers the optimal gauge transformation for the task.

The ``trivial gauge limit'' of our framework does not correspond to ``no gauge structure.'' Rather, it corresponds to a \emph{constant} gauge transformation $\Omega = W_Q W_K^\top$ that is:
\begin{enumerate}
\item Independent of token position (same $\Omega$ for all pairs $i,j$)
\item Learned from data rather than derived from geometric principles
\item Applied in a flat (zero-curvature) bundle geometry
\end{enumerate}

This reframes the relationship between gauge theory and transformers: standard transformers \emph{already implement} $\mathrm{GL}(K)$ gauge-theoretic attention, but with the gauge structure absorbed into learned parameters rather than specified \emph{a priori}.
...
Each agent $i$ has a gauge frame field $\phi_i: \mathcal{U}_i \to \mathfrak{gl}(K)$ which may vary spatially. (In simulations using compact subgroups, $\phi_i \in \mathfrak{so}(3)$.) This induces a local connection:
...
A crucial observation underlying our entire framework is that the Kullback-Leibler divergence possesses a maximal gauge symmetry: it is invariant under the \emph{full general linear group} $\mathrm{GL}(K)$, not merely compact subgroups like $\mathrm{SO}(K)$.
...
\begin{enumerate}
\item \textbf{Maximal gauge symmetry}: The natural gauge group for KL-based attention is $\mathrm{GL}(K)$, not merely $SO(K)$. The orthogonal group $SO(K)$ is a \emph{subgroup} of $\mathrm{GL}(K)$; requiring $\Omega^\top\Omega = I$ is a sufficient but not necessary condition for gauge invariance.
...
\item \textbf{Learned projections as gauge transformations}: As we shall demonstrate, the query and key projection matrices $W_Q, W_K \in \mathrm{GL}(d_k)$ in standard transformers are themselves gauge transformations. The ``trivial gauge limit'' corresponds not to the absence of gauge structure, but to a \emph{constant} gauge transformation $\Omega = W_Q W_K^\top$ applied uniformly across all token pairs.
\end{enumerate}
...
This is precisely where the $\mathrm{GL}(K)$ gauge structure enters. We identify learned projection matrices $W_Q, W_K \in \mathbb{R}^{d \times d_k}$ such that:
\begin{equation}
...
$D_{\mathrm{KL}}(q_i \| \Omega_{ij}q_j)$ & $\mathrm{GL}(K)$-invariant & $\frac{1}{2}\|\mu_i - \Omega\mu_j\|^2$ \\
...
\hline
\end{tabular}
\caption{Complete correspondence between gauge-equivariant FEP and neural networks. The crucial insight is that transformers possess $\mathrm{GL}(K)$ gauge structure through learned projections $W_Q, W_K$.}
...
Under the successive limits---(i) deterministic beliefs ($\Sigma_i \to 0$), (ii) constant gauge ($\Omega_{ij} = \Omega$ for all pairs), and (iii) learned gauge via projections ($\Omega = W_Q W_K^\top \in \mathrm{GL}(d_k)$)---variational gradient descent on the gauge-equivariant free energy:
\begin{equation}
...
The infinitesimal structure of a gauge group is encoded in its Lie algebra $\mathfrak{g}$. For the full gauge group $G = \mathrm{GL}(K)$, the Lie algebra $\mathfrak{gl}(K)$ has dimension $K^2$ with generators spanning all $K \times K$ matrices. For compact subgroups like $G = \text{SO}(N)$, the Lie algebra $\mathfrak{so}(N)$ consists of $N(N-1)/2$ linearly independent skew-symmetric generators $\{G_a\}_{a=1}^{\dim \mathfrak{g}}$, each corresponding to an infinitesimal rotation in a specific 2-plane of $\mathbb{R}^N$.
